...
Provides thread-safe rate limiting with queue support for synchronous operations.
"""
...
logger = logging.getLogger("hfortix.rate_limiter")
...
class RateLimiter:
    """
    Token bucket rate limiter with FIFO queue support
    
    Implements token bucket algorithm for smooth rate limiting with burst support.
    Thread-safe for use in synchronous HTTP clients.
        ...
        with self._lock:
            ...
            try:
                if self._queue_overflow == "block":
                    ...
                    self._request_queue.put(True, timeout=self._queue_timeout)
