...
def validate_service_custom_name(name: Optional[str], operation: str = "operation") -> None:
    ...
    if isinstance(name, str) and len(name) > 79:
        raise ValueError(f"Service name cannot exceed 79 characters, got {len(name)}")


def validate_category(category: Optional[str]) -> None:
    """Validate service category."""
    if category is not None and isinstance(category, str) and len(category) > 63:
        raise ValueError(f"Service category cannot exceed 63 characters, got {len(category)}")
...
def validate_check_reset_range(check_reset_range: Optional[str]) -> None:
    ...
    if check_reset_range is not None and check_reset_range not in valid_values:
        raise ValueError(f"Invalid check-reset-range '{check_reset_range}'. Must be one of: {', '.join(valid_values)}")
...
def validate_app_service_type(app_service_type: Optional[str]) -> None:
    ...
    if app_service_type is not None and app_service_type not in valid_values:
        raise ValueError(f"Invalid app-service-type '{app_service_type}'. Must be one of: {', '.join(valid_values)}")
...
class ServiceCustom:
    ...
    def create(
        ...
        protocol: Optional[Literal["TCP/UDP/UDP-Lite/SCTP", "ICMP", "ICMP6", "IP", "HTTP", "FTP", "CONNECT", "SOCKS-TCP", "SOCKS-UDP", "ALL"]] = None,
        ...
        helper: Optional[Literal["auto", "disable", "ftp", "tftp", "ras", "h323", "tns", "mms", "sip", "pptp", "rtsp", "dns-udp", "dns-tcp", "pmap", "rsh", "dcerpc", "mgcp"]] = None,
        # Timeout parameters
        ...
        # Advanced parameters
        check_reset_range: Optional[Literal["disable", "strict", "default"]] = None,
        comment: Optional[str] = None,
        color: Optional[int] = None,
        app_service_type: Optional[Literal["disable", "app-id", "app-category"]] = None,
        ...
        payload = build_cmdb_payload_normalized(
            name=name, uuid=uuid, proxy=proxy, category=category, protocol=protocol,
            helper=helper, iprange=iprange, fqdn=fqdn, protocol_number=protocol_number,
            icmptype=icmptype, icmpcode=icmpcode, tcp_portrange=tcp_portrange,
            udp_portrange=udp_portrange, udplite_portrange=udplite_portrange,
            sctp_portrange=sctp_portrange, tcp_halfclose_timer=tcp_halfclose_timer,
            tcp_halfopen_timer=tcp_halfopen_timer, tcp_timewait_timer=tcp_timewait_timer,
            tcp_rst_timer=tcp_rst_timer, udp_idle_timer=udp_idle_timer,
            session_ttl=session_ttl, check_reset_range=check_reset_range,
            comment=comment, color=color, app_service_type=app_service_type,
            app_category=app_category, application=application,
            fabric_object=fabric_object, data=data,
        )
    ...
    def update(self, name: str, **params: Any) -> Union[Dict[str, Any], Coroutine[Any, Any, Dict[str, Any]]]:
