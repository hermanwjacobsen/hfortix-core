...
def validate_static_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'blackhole' in payload:
        ...
        if value and value not in VALID_BODY_BLACKHOLE:
            return (False, f"Invalid blackhole '{value}'. Must be one of: {', '.join(VALID_BODY_BLACKHOLE)}")
    ...
    if 'dynamic-gateway' in payload:
        ...
        if value and value not in VALID_BODY_DYNAMIC_GATEWAY:
            return (False, f"Invalid dynamic-gateway '{value}'. Must be one of: {', '.join(VALID_BODY_DYNAMIC_GATEWAY)}")
    ...
    if 'internet-service' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"internet-service must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"internet-service must be numeric, got: {value}")
    ...
    if 'internet-service-custom' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 64:
            return (False, f"internet-service-custom cannot exceed 64 characters")
    ...
    if 'internet-service-fortiguard' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 64:
            return (False, f"internet-service-fortiguard cannot exceed 64 characters")
    ...
    if 'link-monitor-exempt' in payload:
        ...
        if value and value not in VALID_BODY_LINK_MONITOR_EXEMPT:
            return (False, f"Invalid link-monitor-exempt '{value}'. Must be one of: {', '.join(VALID_BODY_LINK_MONITOR_EXEMPT)}")
