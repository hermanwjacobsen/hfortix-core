...
VALID_BODY_METRIC_STYLE = ['narrow', 'wide', 'transition', 'narrow-transition', 'narrow-transition-l1', 'narrow-transition-l2', 'wide-l1', 'wide-l2', 'wide-transition', 'wide-transition-l1', 'wide-transition-l2', 'transition-l1', 'transition-l2']
...
def validate_isis_put(
    ...
    if 'is-type' in payload:
        ...
        if value and value not in VALID_BODY_IS_TYPE:
            return (False, f"Invalid is-type '{value}'. Must be one of: {', '.join(VALID_BODY_IS_TYPE)}")
    ...
    if 'adv-passive-only' in payload:
        ...
        if value and value not in VALID_BODY_ADV_PASSIVE_ONLY:
            return (False, f"Invalid adv-passive-only '{value}'. Must be one of: {', '.join(VALID_BODY_ADV_PASSIVE_ONLY)}")
    ...
    if 'adv-passive-only6' in payload:
        ...
        if value and value not in VALID_BODY_ADV_PASSIVE_ONLY6:
            return (False, f"Invalid adv-passive-only6 '{value}'. Must be one of: {', '.join(VALID_BODY_ADV_PASSIVE_ONLY6)}")
    ...
    if 'auth-mode-l1' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_MODE_L1:
            return (False, f"Invalid auth-mode-l1 '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_MODE_L1)}")
    ...
    if 'auth-mode-l2' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_MODE_L2:
            return (False, f"Invalid auth-mode-l2 '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_MODE_L2)}")
    ...
    if 'auth-sendonly-l1' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SENDONLY_L1:
            return (False, f"Invalid auth-sendonly-l1 '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SENDONLY_L1)}")
    ...
    if 'auth-sendonly-l2' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SENDONLY_L2:
            return (False, f"Invalid auth-sendonly-l2 '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SENDONLY_L2)}")
    ...
    if 'ignore-lsp-errors' in payload:
        ...
        if value and value not in VALID_BODY_IGNORE_LSP_ERRORS:
            return (False, f"Invalid ignore-lsp-errors '{value}'. Must be one of: {', '.join(VALID_BODY_IGNORE_LSP_ERRORS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 120:
                    return (False, f"lsp-gen-interval-l1 must be between 1 and 120")
            except (ValueError, TypeError):
                return (False, f"lsp-gen-interval-l1 must be numeric, got: {value}")
    ...
    if 'lsp-gen-interval-l2' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 120:
                    return (False, f"lsp-gen-interval-l2 must be between 1 and 120")
            except (ValueError, TypeError):
                return (False, f"lsp-gen-interval-l2 must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"lsp-refresh-interval must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"lsp-refresh-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 350 or int_val > 65535:
                    return (False, f"max-lsp-lifetime must be between 350 and 65535")
            except (ValueError, TypeError):
                return (False, f"max-lsp-lifetime must be numeric, got: {value}")
    ...
    if 'dynamic-hostname' in payload:
        ...
        if value and value not in VALID_BODY_DYNAMIC_HOSTNAME:
            return (False, f"Invalid dynamic-hostname '{value}'. Must be one of: {', '.join(VALID_BODY_DYNAMIC_HOSTNAME)}")
    ...
    if 'adjacency-check' in payload:
        ...
        if value and value not in VALID_BODY_ADJACENCY_CHECK:
            return (False, f"Invalid adjacency-check '{value}'. Must be one of: {', '.join(VALID_BODY_ADJACENCY_CHECK)}")
    ...
    if 'adjacency-check6' in payload:
        ...
        if value and value not in VALID_BODY_ADJACENCY_CHECK6:
            return (False, f"Invalid adjacency-check6 '{value}'. Must be one of: {', '.join(VALID_BODY_ADJACENCY_CHECK6)}")
    ...
    if 'overload-bit' in payload:
        ...
        if value and value not in VALID_BODY_OVERLOAD_BIT:
            return (False, f"Invalid overload-bit '{value}'. Must be one of: {', '.join(VALID_BODY_OVERLOAD_BIT)}")
    ...
    if 'overload-bit-suppress' in payload:
        ...
        if value and value not in VALID_BODY_OVERLOAD_BIT_SUPPRESS:
            return (False, f"Invalid overload-bit-suppress '{value}'. Must be one of: {', '.join(VALID_BODY_OVERLOAD_BIT_SUPPRESS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 86400:
                    return (False, f"overload-bit-on-startup must be between 5 and 86400")
            except (ValueError, TypeError):
                return (False, f"overload-bit-on-startup must be numeric, got: {value}")
    ...
    if 'default-originate' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_ORIGINATE:
            return (False, f"Invalid default-originate '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_ORIGINATE)}")
    ...
    if 'default-originate6' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_ORIGINATE6:
            return (False, f"Invalid default-originate6 '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_ORIGINATE6)}")
    ...
    if 'metric-style' in payload:
        ...
        if value and value not in VALID_BODY_METRIC_STYLE:
            return (False, f"Invalid metric-style '{value}'. Must be one of: {', '.join(VALID_BODY_METRIC_STYLE)}")
    ...
    if 'redistribute-l1' in payload:
        ...
        if value and value not in VALID_BODY_REDISTRIBUTE_L1:
            return (False, f"Invalid redistribute-l1 '{value}'. Must be one of: {', '.join(VALID_BODY_REDISTRIBUTE_L1)}")
    ...
    if 'redistribute-l2' in payload:
        ...
        if value and value not in VALID_BODY_REDISTRIBUTE_L2:
            return (False, f"Invalid redistribute-l2 '{value}'. Must be one of: {', '.join(VALID_BODY_REDISTRIBUTE_L2)}")
    ...
    if 'redistribute6-l1' in payload:
        ...
        if value and value not in VALID_BODY_REDISTRIBUTE6_L1:
            return (False, f"Invalid redistribute6-l1 '{value}'. Must be one of: {', '.join(VALID_BODY_REDISTRIBUTE6_L1)}")
    
    # Validate redistribute6-l1-list if present
    if 'redistribute6-l1-list' in payload:
        value = payload.get('redistribute6-l1-list')
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"redistribute6-l1-list cannot exceed 35 characters")
    ...
    if 'redistribute6-l2' in payload:
        ...
        if value and value not in VALID_BODY_REDISTRIBUTE6_L2:
            return (False, f"Invalid redistribute6-l2 '{value}'. Must be one of: {', '.join(VALID_BODY_REDISTRIBUTE6_L2)}")
    
    # Validate redistribute6-l2-list if present
    if 'redistribute6-l2-list' in payload:
        value = payload.get('redistribute6-l2-list')
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"redistribute6-l2-list cannot exceed 35 characters")
