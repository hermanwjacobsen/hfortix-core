...
def validate_policy6_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'input-device-negate' in payload:
        ...
        if value and value not in VALID_BODY_INPUT_DEVICE_NEGATE:
            return (False, f"Invalid input-device-negate '{value}'. Must be one of: {', '.join(VALID_BODY_INPUT_DEVICE_NEGATE)}")
    ...
    if 'src-negate' in payload:
        ...
        if value and value not in VALID_BODY_SRC_NEGATE:
            return (False, f"Invalid src-negate '{value}'. Must be one of: {', '.join(VALID_BODY_SRC_NEGATE)}")
    ...
    if 'dst-negate' in payload:
        ...
        if value and value not in VALID_BODY_DST_NEGATE:
            return (False, f"Invalid dst-negate '{value}'. Must be one of: {', '.join(VALID_BODY_DST_NEGATE)}")
    ...
    if 'start-source-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"start-source-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"start-source-port must be numeric, got: {value}")
    ...
    if 'end-source-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"end-source-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"end-source-port must be numeric, got: {value}")
