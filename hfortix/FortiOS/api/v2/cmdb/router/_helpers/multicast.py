...
def validate_multicast_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 2147483647:
                    return (False, f"route-threshold must be between 1 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"route-threshold must be numeric, got: {value}")
    ...
    if 'route-limit' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 2147483647:
                    return (False, f"route-limit must be between 1 and 2147483647")
            except (ValueError, TypeError):
    ...
    if 'multicast-routing' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_ROUTING:
            return (False, f"Invalid multicast-routing '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_ROUTING)}")
