...
def validate_ospf_put(
    ...
    if 'abr-type' in payload:
        ...
        if value and value not in VALID_BODY_ABR_TYPE:
            return (False, f"Invalid abr-type '{value}'. Must be one of: {', '.join(VALID_BODY_ABR_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 1000000:
                    return (False, f"auto-cost-ref-bandwidth must be between 1 and 1000000")
            except (ValueError, TypeError):
                return (False, f"auto-cost-ref-bandwidth must be numeric, got: {value}")
    ...
    if 'distance-inter-area' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 255:
                    return (False, f"distance-inter-area must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"distance-inter-area must be numeric, got: {value}")
    ...
    if 'distance-intra-area' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 255:
                    return (False, f"distance-intra-area must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"distance-intra-area must be numeric, got: {value}")
    ...
    if 'database-overflow' in payload:
        ...
        if value and value not in VALID_BODY_DATABASE_OVERFLOW:
            return (False, f"Invalid database-overflow '{value}'. Must be one of: {', '.join(VALID_BODY_DATABASE_OVERFLOW)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"database-overflow-max-lsas must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"database-overflow-max-lsas must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"database-overflow-time-to-recover must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"database-overflow-time-to-recover must be numeric, got: {value}")
    ...
    if 'default-information-originate' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_INFORMATION_ORIGINATE:
            return (False, f"Invalid default-information-originate '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_INFORMATION_ORIGINATE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 16777214:
                    return (False, f"default-information-metric must be between 1 and 16777214")
            except (ValueError, TypeError):
                return (False, f"default-information-metric must be numeric, got: {value}")
    ...
    if 'default-information-metric-type' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_INFORMATION_METRIC_TYPE:
            return (False, f"Invalid default-information-metric-type '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_INFORMATION_METRIC_TYPE)}")
    ...
    if 'default-information-route-map' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"default-information-route-map cannot exceed 35 characters")
    ...
    if 'default-metric' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 16777214:
                    return (False, f"default-metric must be between 1 and 16777214")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 5:
                    return (False, f"lsa-refresh-interval must be between 0 and 5")
            except (ValueError, TypeError):
                return (False, f"lsa-refresh-interval must be numeric, got: {value}")
    ...
    if 'rfc1583-compatible' in payload:
        ...
        if value and value not in VALID_BODY_RFC1583_COMPATIBLE:
            return (False, f"Invalid rfc1583-compatible '{value}'. Must be one of: {', '.join(VALID_BODY_RFC1583_COMPATIBLE)}")
    ...
    if 'distribute-route-map-in' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"distribute-route-map-in cannot exceed 35 characters")
    ...
    if 'restart-mode' in payload:
        ...
        if value and value not in VALID_BODY_RESTART_MODE:
            return (False, f"Invalid restart-mode '{value}'. Must be one of: {', '.join(VALID_BODY_RESTART_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 3600:
                    return (False, f"restart-period must be between 1 and 3600")
            except (ValueError, TypeError):
    ...
    if 'restart-on-topology-change' in payload:
        ...
        if value and value not in VALID_BODY_RESTART_ON_TOPOLOGY_CHANGE:
            return (False, f"Invalid restart-on-topology-change '{value}'. Must be one of: {', '.join(VALID_BODY_RESTART_ON_TOPOLOGY_CHANGE)}")
