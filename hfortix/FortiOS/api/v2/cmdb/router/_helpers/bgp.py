...
def validate_bgp_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"keepalive-timer must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"keepalive-timer must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 3 or int_val > 65535:
                    return (False, f"holdtime-timer must be between 3 and 65535")
            except (ValueError, TypeError):
    ...
    if 'always-compare-med' in payload:
        ...
        if value and value not in VALID_BODY_ALWAYS_COMPARE_MED:
            return (False, f"Invalid always-compare-med '{value}'. Must be one of: {', '.join(VALID_BODY_ALWAYS_COMPARE_MED)}")
    ...
    if 'bestpath-as-path-ignore' in payload:
        ...
        if value and value not in VALID_BODY_BESTPATH_AS_PATH_IGNORE:
            return (False, f"Invalid bestpath-as-path-ignore '{value}'. Must be one of: {', '.join(VALID_BODY_BESTPATH_AS_PATH_IGNORE)}")
    ...
    if 'bestpath-cmp-confed-aspath' in payload:
        ...
        if value and value not in VALID_BODY_BESTPATH_CMP_CONFED_ASPATH:
            return (False, f"Invalid bestpath-cmp-confed-aspath '{value}'. Must be one of: {', '.join(VALID_BODY_BESTPATH_CMP_CONFED_ASPATH)}")
    ...
    if 'bestpath-cmp-routerid' in payload:
        ...
        if value and value not in VALID_BODY_BESTPATH_CMP_ROUTERID:
            return (False, f"Invalid bestpath-cmp-routerid '{value}'. Must be one of: {', '.join(VALID_BODY_BESTPATH_CMP_ROUTERID)}")
    ...
    if 'bestpath-med-confed' in payload:
        ...
        if value and value not in VALID_BODY_BESTPATH_MED_CONFED:
            return (False, f"Invalid bestpath-med-confed '{value}'. Must be one of: {', '.join(VALID_BODY_BESTPATH_MED_CONFED)}")
    ...
    if 'bestpath-med-missing-as-worst' in payload:
        ...
        if value and value not in VALID_BODY_BESTPATH_MED_MISSING_AS_WORST:
            return (False, f"Invalid bestpath-med-missing-as-worst '{value}'. Must be one of: {', '.join(VALID_BODY_BESTPATH_MED_MISSING_AS_WORST)}")
    ...
    if 'client-to-client-reflection' in payload:
        ...
        if value and value not in VALID_BODY_CLIENT_TO_CLIENT_REFLECTION:
            return (False, f"Invalid client-to-client-reflection '{value}'. Must be one of: {', '.join(VALID_BODY_CLIENT_TO_CLIENT_REFLECTION)}")
    ...
    if 'dampening' in payload:
        ...
        if value and value not in VALID_BODY_DAMPENING:
            return (False, f"Invalid dampening '{value}'. Must be one of: {', '.join(VALID_BODY_DAMPENING)}")
    ...
    if 'deterministic-med' in payload:
        ...
        if value and value not in VALID_BODY_DETERMINISTIC_MED:
            return (False, f"Invalid deterministic-med '{value}'. Must be one of: {', '.join(VALID_BODY_DETERMINISTIC_MED)}")
    ...
    if 'ebgp-multipath' in payload:
        ...
        if value and value not in VALID_BODY_EBGP_MULTIPATH:
            return (False, f"Invalid ebgp-multipath '{value}'. Must be one of: {', '.join(VALID_BODY_EBGP_MULTIPATH)}")
    ...
    if 'ibgp-multipath' in payload:
        ...
        if value and value not in VALID_BODY_IBGP_MULTIPATH:
            return (False, f"Invalid ibgp-multipath '{value}'. Must be one of: {', '.join(VALID_BODY_IBGP_MULTIPATH)}")
    ...
    if 'enforce-first-as' in payload:
        ...
        if value and value not in VALID_BODY_ENFORCE_FIRST_AS:
            return (False, f"Invalid enforce-first-as '{value}'. Must be one of: {', '.join(VALID_BODY_ENFORCE_FIRST_AS)}")
    ...
    if 'fast-external-failover' in payload:
        ...
        if value and value not in VALID_BODY_FAST_EXTERNAL_FAILOVER:
            return (False, f"Invalid fast-external-failover '{value}'. Must be one of: {', '.join(VALID_BODY_FAST_EXTERNAL_FAILOVER)}")
    ...
    if 'log-neighbour-changes' in payload:
        ...
        if value and value not in VALID_BODY_LOG_NEIGHBOUR_CHANGES:
            return (False, f"Invalid log-neighbour-changes '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_NEIGHBOUR_CHANGES)}")
    ...
    if 'network-import-check' in payload:
        ...
        if value and value not in VALID_BODY_NETWORK_IMPORT_CHECK:
            return (False, f"Invalid network-import-check '{value}'. Must be one of: {', '.join(VALID_BODY_NETWORK_IMPORT_CHECK)}")
    ...
    if 'ignore-optional-capability' in payload:
        ...
        if value and value not in VALID_BODY_IGNORE_OPTIONAL_CAPABILITY:
            return (False, f"Invalid ignore-optional-capability '{value}'. Must be one of: {', '.join(VALID_BODY_IGNORE_OPTIONAL_CAPABILITY)}")
    ...
    if 'additional-path' in payload:
        ...
        if value and value not in VALID_BODY_ADDITIONAL_PATH:
            return (False, f"Invalid additional-path '{value}'. Must be one of: {', '.join(VALID_BODY_ADDITIONAL_PATH)}")
    ...
    if 'additional-path6' in payload:
        ...
        if value and value not in VALID_BODY_ADDITIONAL_PATH6:
            return (False, f"Invalid additional-path6 '{value}'. Must be one of: {', '.join(VALID_BODY_ADDITIONAL_PATH6)}")
    ...
    if 'additional-path-vpnv4' in payload:
        ...
        if value and value not in VALID_BODY_ADDITIONAL_PATH_VPNV4:
            return (False, f"Invalid additional-path-vpnv4 '{value}'. Must be one of: {', '.join(VALID_BODY_ADDITIONAL_PATH_VPNV4)}")
    ...
    if 'additional-path-vpnv6' in payload:
        ...
        if value and value not in VALID_BODY_ADDITIONAL_PATH_VPNV6:
            return (False, f"Invalid additional-path-vpnv6 '{value}'. Must be one of: {', '.join(VALID_BODY_ADDITIONAL_PATH_VPNV6)}")
    ...
    if 'multipath-recursive-distance' in payload:
        ...
        if value and value not in VALID_BODY_MULTIPATH_RECURSIVE_DISTANCE:
            return (False, f"Invalid multipath-recursive-distance '{value}'. Must be one of: {', '.join(VALID_BODY_MULTIPATH_RECURSIVE_DISTANCE)}")
    ...
    if 'recursive-next-hop' in payload:
        ...
        if value and value not in VALID_BODY_RECURSIVE_NEXT_HOP:
            return (False, f"Invalid recursive-next-hop '{value}'. Must be one of: {', '.join(VALID_BODY_RECURSIVE_NEXT_HOP)}")
    ...
    if 'recursive-inherit-priority' in payload:
        ...
        if value and value not in VALID_BODY_RECURSIVE_INHERIT_PRIORITY:
            return (False, f"Invalid recursive-inherit-priority '{value}'. Must be one of: {', '.join(VALID_BODY_RECURSIVE_INHERIT_PRIORITY)}")
    ...
    if 'tag-resolve-mode' in payload:
        ...
        if value and value not in VALID_BODY_TAG_RESOLVE_MODE:
            return (False, f"Invalid tag-resolve-mode '{value}'. Must be one of: {', '.join(VALID_BODY_TAG_RESOLVE_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 4294967295:
                    return (False, f"confederation-identifier must be between 1 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"confederation-identifier must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 45:
                    return (False, f"dampening-reachability-half-life must be between 1 and 45")
            except (ValueError, TypeError):
                return (False, f"dampening-reachability-half-life must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 20000:
                    return (False, f"dampening-reuse must be between 1 and 20000")
            except (ValueError, TypeError):
                return (False, f"dampening-reuse must be numeric, got: {value}")
    ...
    if 'dampening-suppress' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 20000:
                    return (False, f"dampening-suppress must be between 1 and 20000")
            except (ValueError, TypeError):
                return (False, f"dampening-suppress must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 255:
                    return (False, f"dampening-max-suppress-time must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"dampening-max-suppress-time must be numeric, got: {value}")
    ...
    if 'dampening-unreachability-half-life' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 45:
                    return (False, f"dampening-unreachability-half-life must be between 1 and 45")
            except (ValueError, TypeError):
                return (False, f"dampening-unreachability-half-life must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"default-local-preference must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"default-local-preference must be numeric, got: {value}")
    ...
    if 'distance-external' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 255:
                    return (False, f"distance-external must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"distance-external must be numeric, got: {value}")
    ...
    if 'distance-internal' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 255:
                    return (False, f"distance-internal must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"distance-internal must be numeric, got: {value}")
    ...
    if 'synchronization' in payload:
        ...
        if value and value not in VALID_BODY_SYNCHRONIZATION:
            return (False, f"Invalid synchronization '{value}'. Must be one of: {', '.join(VALID_BODY_SYNCHRONIZATION)}")
    ...
    if 'graceful-restart' in payload:
        ...
        if value and value not in VALID_BODY_GRACEFUL_RESTART:
            return (False, f"Invalid graceful-restart '{value}'. Must be one of: {', '.join(VALID_BODY_GRACEFUL_RESTART)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 3600:
                    return (False, f"graceful-restart-time must be between 1 and 3600")
            except (ValueError, TypeError):
                return (False, f"graceful-restart-time must be numeric, got: {value}")
    ...
    if 'graceful-stalepath-time' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 3600:
                    return (False, f"graceful-stalepath-time must be between 1 and 3600")
            except (ValueError, TypeError):
                return (False, f"graceful-stalepath-time must be numeric, got: {value}")
    ...
    if 'graceful-update-delay' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 3600:
                    return (False, f"graceful-update-delay must be between 1 and 3600")
            except (ValueError, TypeError):
                return (False, f"graceful-update-delay must be numeric, got: {value}")
    ...
    if 'graceful-end-on-timer' in payload:
        ...
        if value and value not in VALID_BODY_GRACEFUL_END_ON_TIMER:
            return (False, f"Invalid graceful-end-on-timer '{value}'. Must be one of: {', '.join(VALID_BODY_GRACEFUL_END_ON_TIMER)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 255:
                    return (False, f"additional-path-select must be between 2 and 255")
            except (ValueError, TypeError):
                return (False, f"additional-path-select must be numeric, got: {value}")
    ...
    if 'additional-path-select6' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 2 or int_val > 255:
                    return (False, f"additional-path-select6 must be between 2 and 255")
            except (ValueError, TypeError):
                return (False, f"additional-path-select6 must be numeric, got: {value}")
    ...
    if 'additional-path-select-vpnv4' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 2 or int_val > 255:
                    return (False, f"additional-path-select-vpnv4 must be between 2 and 255")
            except (ValueError, TypeError):
                return (False, f"additional-path-select-vpnv4 must be numeric, got: {value}")
    ...
    if 'additional-path-select-vpnv6' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 2 or int_val > 255:
                    return (False, f"additional-path-select-vpnv6 must be between 2 and 255")
            except (ValueError, TypeError):
                return (False, f"additional-path-select-vpnv6 must be numeric, got: {value}")
    ...
    if 'cross-family-conditional-adv' in payload:
        ...
        if value and value not in VALID_BODY_CROSS_FAMILY_CONDITIONAL_ADV:
            return (False, f"Invalid cross-family-conditional-adv '{value}'. Must be one of: {', '.join(VALID_BODY_CROSS_FAMILY_CONDITIONAL_ADV)}")
