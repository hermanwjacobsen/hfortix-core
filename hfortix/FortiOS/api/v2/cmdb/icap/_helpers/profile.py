...
VALID_BODY_METHODS = ['delete', 'get', 'head', 'options', 'post', 'put', 'trace', 'connect', 'other']
...
def validate_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'request' in payload:
        ...
        if value and value not in VALID_BODY_REQUEST:
            return (False, f"Invalid request '{value}'. Must be one of: {', '.join(VALID_BODY_REQUEST)}")
    
    # Validate response if present
    if 'response' in payload:
        value = payload.get('response')
        if value and value not in VALID_BODY_RESPONSE:
            return (False, f"Invalid response '{value}'. Must be one of: {', '.join(VALID_BODY_RESPONSE)}")
    ...
    if 'file-transfer' in payload:
        ...
        if value and value not in VALID_BODY_FILE_TRANSFER:
            return (False, f"Invalid file-transfer '{value}'. Must be one of: {', '.join(VALID_BODY_FILE_TRANSFER)}")
    ...
    if 'streaming-content-bypass' in payload:
        ...
        if value and value not in VALID_BODY_STREAMING_CONTENT_BYPASS:
            return (False, f"Invalid streaming-content-bypass '{value}'. Must be one of: {', '.join(VALID_BODY_STREAMING_CONTENT_BYPASS)}")
    ...
    if 'ocr-only' in payload:
        ...
        if value and value not in VALID_BODY_OCR_ONLY:
            return (False, f"Invalid ocr-only '{value}'. Must be one of: {', '.join(VALID_BODY_OCR_ONLY)}")
    ...
    if '204-response' in payload:
        ...
        if value and value not in VALID_BODY_204_RESPONSE:
            return (False, f"Invalid 204-response '{value}'. Must be one of: {', '.join(VALID_BODY_204_RESPONSE)}")
    
    # Validate preview if present
    if 'preview' in payload:
        value = payload.get('preview')
        if value and value not in VALID_BODY_PREVIEW:
            return (False, f"Invalid preview '{value}'. Must be one of: {', '.join(VALID_BODY_PREVIEW)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4096:
                    return (False, f"preview-data-length must be between 0 and 4096")
            except (ValueError, TypeError):
                return (False, f"preview-data-length must be numeric, got: {value}")
    ...
    if 'request-failure' in payload:
        ...
        if value and value not in VALID_BODY_REQUEST_FAILURE:
            return (False, f"Invalid request-failure '{value}'. Must be one of: {', '.join(VALID_BODY_REQUEST_FAILURE)}")
    ...
    if 'response-failure' in payload:
        ...
        if value and value not in VALID_BODY_RESPONSE_FAILURE:
            return (False, f"Invalid response-failure '{value}'. Must be one of: {', '.join(VALID_BODY_RESPONSE_FAILURE)}")
    ...
    if 'file-transfer-failure' in payload:
        ...
        if value and value not in VALID_BODY_FILE_TRANSFER_FAILURE:
            return (False, f"Invalid file-transfer-failure '{value}'. Must be one of: {', '.join(VALID_BODY_FILE_TRANSFER_FAILURE)}")
    ...
    if 'methods' in payload:
        ...
        if value and value not in VALID_BODY_METHODS:
            return (False, f"Invalid methods '{value}'. Must be one of: {', '.join(VALID_BODY_METHODS)}")
    ...
    if 'response-req-hdr' in payload:
        ...
        if value and value not in VALID_BODY_RESPONSE_REQ_HDR:
            return (False, f"Invalid response-req-hdr '{value}'. Must be one of: {', '.join(VALID_BODY_RESPONSE_REQ_HDR)}")
    ...
    if 'respmod-default-action' in payload:
        ...
        if value and value not in VALID_BODY_RESPMOD_DEFAULT_ACTION:
            return (False, f"Invalid respmod-default-action '{value}'. Must be one of: {', '.join(VALID_BODY_RESPMOD_DEFAULT_ACTION)}")
    ...
    if 'icap-block-log' in payload:
        ...
        if value and value not in VALID_BODY_ICAP_BLOCK_LOG:
            return (False, f"Invalid icap-block-log '{value}'. Must be one of: {', '.join(VALID_BODY_ICAP_BLOCK_LOG)}")
    ...
    if 'chunk-encap' in payload:
        ...
        if value and value not in VALID_BODY_CHUNK_ENCAP:
            return (False, f"Invalid chunk-encap '{value}'. Must be one of: {', '.join(VALID_BODY_CHUNK_ENCAP)}")
    ...
    if 'extension-feature' in payload:
        ...
        if value and value not in VALID_BODY_EXTENSION_FEATURE:
            return (False, f"Invalid extension-feature '{value}'. Must be one of: {', '.join(VALID_BODY_EXTENSION_FEATURE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 30:
                    return (False, f"scan-progress-interval must be between 5 and 30")
            except (ValueError, TypeError):
                return (False, f"scan-progress-interval must be numeric, got: {value}")
