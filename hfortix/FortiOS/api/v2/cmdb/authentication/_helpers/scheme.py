...
VALID_BODY_METHOD = ['ntlm', 'basic', 'digest', 'form', 'negotiate', 'fsso', 'rsso', 'ssh-publickey', 'cert', 'saml', 'entra-sso']
...
def validate_scheme_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'negotiate-ntlm' in payload:
        ...
        if value and value not in VALID_BODY_NEGOTIATE_NTLM:
            return (False, f"Invalid negotiate-ntlm '{value}'. Must be one of: {', '.join(VALID_BODY_NEGOTIATE_NTLM)}")
    ...
    if 'require-tfa' in payload:
        ...
        if value and value not in VALID_BODY_REQUIRE_TFA:
            return (False, f"Invalid require-tfa '{value}'. Must be one of: {', '.join(VALID_BODY_REQUIRE_TFA)}")
    ...
    if 'fsso-guest' in payload:
        ...
        if value and value not in VALID_BODY_FSSO_GUEST:
            return (False, f"Invalid fsso-guest '{value}'. Must be one of: {', '.join(VALID_BODY_FSSO_GUEST)}")
    ...
    if 'user-cert' in payload:
        ...
        if value and value not in VALID_BODY_USER_CERT:
            return (False, f"Invalid user-cert '{value}'. Must be one of: {', '.join(VALID_BODY_USER_CERT)}")
    ...
    if 'cert-http-header' in payload:
        ...
        if value and value not in VALID_BODY_CERT_HTTP_HEADER:
            return (False, f"Invalid cert-http-header '{value}'. Must be one of: {', '.join(VALID_BODY_CERT_HTTP_HEADER)}")
    ...
    if 'group-attr-type' in payload:
        ...
        if value and value not in VALID_BODY_GROUP_ATTR_TYPE:
            return (False, f"Invalid group-attr-type '{value}'. Must be one of: {', '.join(VALID_BODY_GROUP_ATTR_TYPE)}")
    ...
    if 'digest-algo' in payload:
        ...
        if value and value not in VALID_BODY_DIGEST_ALGO:
            return (False, f"Invalid digest-algo '{value}'. Must be one of: {', '.join(VALID_BODY_DIGEST_ALGO)}")
    ...
    if 'digest-rfc2069' in payload:
        ...
        if value and value not in VALID_BODY_DIGEST_RFC2069:
            return (False, f"Invalid digest-rfc2069 '{value}'. Must be one of: {', '.join(VALID_BODY_DIGEST_RFC2069)}")
