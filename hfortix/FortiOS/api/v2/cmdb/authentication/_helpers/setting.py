...
def validate_setting_put(
    ...
    if 'persistent-cookie' in payload:
        ...
        if value and value not in VALID_BODY_PERSISTENT_COOKIE:
            return (False, f"Invalid persistent-cookie '{value}'. Must be one of: {', '.join(VALID_BODY_PERSISTENT_COOKIE)}")
    ...
    if 'ip-auth-cookie' in payload:
        ...
        if value and value not in VALID_BODY_IP_AUTH_COOKIE:
            return (False, f"Invalid ip-auth-cookie '{value}'. Must be one of: {', '.join(VALID_BODY_IP_AUTH_COOKIE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 30 or int_val > 10080:
                    return (False, f"cookie-max-age must be between 30 and 10080")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 4:
                    return (False, f"cookie-refresh-div must be between 2 and 4")
            except (ValueError, TypeError):
                return (False, f"cookie-refresh-div must be numeric, got: {value}")
    ...
    if 'captive-portal-type' in payload:
        ...
        if value and value not in VALID_BODY_CAPTIVE_PORTAL_TYPE:
            return (False, f"Invalid captive-portal-type '{value}'. Must be one of: {', '.join(VALID_BODY_CAPTIVE_PORTAL_TYPE)}")
    ...
    if 'cert-auth' in payload:
        ...
        if value and value not in VALID_BODY_CERT_AUTH:
            return (False, f"Invalid cert-auth '{value}'. Must be one of: {', '.join(VALID_BODY_CERT_AUTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"cert-captive-portal-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"cert-captive-portal-port must be numeric, got: {value}")
    ...
    if 'captive-portal-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"captive-portal-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"captive-portal-port must be numeric, got: {value}")
    ...
    if 'auth-https' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_HTTPS:
            return (False, f"Invalid auth-https '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_HTTPS)}")
    ...
    if 'captive-portal-ssl-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"captive-portal-ssl-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"captive-portal-ssl-port must be numeric, got: {value}")
