...
VALID_BODY_CONTROL_MESSAGE_OFFLOAD = ['ebp-frame', 'aeroscout-tag', 'ap-list', 'sta-list', 'sta-cap-list', 'stats', 'aeroscout-mu', 'sta-health', 'spectral-analysis']
...
VALID_BODY_POE_MODE = ['auto', '8023af', '8023at', 'power-adapter', 'full', 'high', 'low']
...
VALID_BODY_INDOOR_OUTDOOR_DEPLOYMENT = ['platform-determined', 'outdoor', 'indoor']
...
VALID_BODY_APCFG_AUTO_CERT_CRYPTO_ALGO = ['rsa-1024', 'rsa-1536', 'rsa-2048', 'rsa-4096', 'ec-secp256r1', 'ec-secp384r1', 'ec-secp521r1']
VALID_BODY_APCFG_AUTO_CERT_SCEP_KEYTYPE = ['rsa', 'ec']
VALID_BODY_APCFG_AUTO_CERT_SCEP_KEYSIZE = ['1024', '1536', '2048', '4096']
VALID_BODY_APCFG_AUTO_CERT_SCEP_EC_NAME = ['secp256r1', 'secp384r1', 'secp521r1']
...
def validate_wtp_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'control-message-offload' in payload:
        ...
        if value and value not in VALID_BODY_CONTROL_MESSAGE_OFFLOAD:
            return (False, f"Invalid control-message-offload '{value}'. Must be one of: {', '.join(VALID_BODY_CONTROL_MESSAGE_OFFLOAD)}")
    ...
    if 'apcfg-mesh' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_MESH:
            return (False, f"Invalid apcfg-mesh '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_MESH)}")
    ...
    if 'apcfg-mesh-ap-type' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_MESH_AP_TYPE:
            return (False, f"Invalid apcfg-mesh-ap-type '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_MESH_AP_TYPE)}")
    ...
    if 'apcfg-mesh-eth-bridge' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_MESH_ETH_BRIDGE:
            return (False, f"Invalid apcfg-mesh-eth-bridge '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_MESH_ETH_BRIDGE)}")
    ...
    if 'wan-port-mode' in payload:
        ...
        if value and value not in VALID_BODY_WAN_PORT_MODE:
            return (False, f"Invalid wan-port-mode '{value}'. Must be one of: {', '.join(VALID_BODY_WAN_PORT_MODE)}")
    ...
    if 'energy-efficient-ethernet' in payload:
        ...
        if value and value not in VALID_BODY_ENERGY_EFFICIENT_ETHERNET:
            return (False, f"Invalid energy-efficient-ethernet '{value}'. Must be one of: {', '.join(VALID_BODY_ENERGY_EFFICIENT_ETHERNET)}")
    ...
    if 'led-state' in payload:
        ...
        if value and value not in VALID_BODY_LED_STATE:
            return (False, f"Invalid led-state '{value}'. Must be one of: {', '.join(VALID_BODY_LED_STATE)}")
    ...
    if 'dtls-policy' in payload:
        ...
        if value and value not in VALID_BODY_DTLS_POLICY:
            return (False, f"Invalid dtls-policy '{value}'. Must be one of: {', '.join(VALID_BODY_DTLS_POLICY)}")
    ...
    if 'dtls-in-kernel' in payload:
        ...
        if value and value not in VALID_BODY_DTLS_IN_KERNEL:
            return (False, f"Invalid dtls-in-kernel '{value}'. Must be one of: {', '.join(VALID_BODY_DTLS_IN_KERNEL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 60:
                    return (False, f"handoff-sta-thresh must be between 5 and 60")
            except (ValueError, TypeError):
                return (False, f"handoff-sta-thresh must be numeric, got: {value}")
    ...
    if 'handoff-roaming' in payload:
        ...
        if value and value not in VALID_BODY_HANDOFF_ROAMING:
            return (False, f"Invalid handoff-roaming '{value}'. Must be one of: {', '.join(VALID_BODY_HANDOFF_ROAMING)}")
    ...
    if 'ap-country' in payload:
        ...
        if value and value not in VALID_BODY_AP_COUNTRY:
            return (False, f"Invalid ap-country '{value}'. Must be one of: {', '.join(VALID_BODY_AP_COUNTRY)}")
    ...
    if 'ip-fragment-preventing' in payload:
        ...
        if value and value not in VALID_BODY_IP_FRAGMENT_PREVENTING:
            return (False, f"Invalid ip-fragment-preventing '{value}'. Must be one of: {', '.join(VALID_BODY_IP_FRAGMENT_PREVENTING)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 576 or int_val > 1500:
                    return (False, f"tun-mtu-uplink must be between 576 and 1500")
            except (ValueError, TypeError):
    ...
    if 'tun-mtu-downlink' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 576 or int_val > 1500:
                    return (False, f"tun-mtu-downlink must be between 576 and 1500")
            except (ValueError, TypeError):
                return (False, f"tun-mtu-downlink must be numeric, got: {value}")
    ...
    if 'split-tunneling-acl-path' in payload:
        ...
        if value and value not in VALID_BODY_SPLIT_TUNNELING_ACL_PATH:
            return (False, f"Invalid split-tunneling-acl-path '{value}'. Must be one of: {', '.join(VALID_BODY_SPLIT_TUNNELING_ACL_PATH)}")
    ...
    if 'split-tunneling-acl-local-ap-subnet' in payload:
        ...
        if value and value not in VALID_BODY_SPLIT_TUNNELING_ACL_LOCAL_AP_SUBNET:
            return (False, f"Invalid split-tunneling-acl-local-ap-subnet '{value}'. Must be one of: {', '.join(VALID_BODY_SPLIT_TUNNELING_ACL_LOCAL_AP_SUBNET)}")
    ...
    if 'login-passwd-change' in payload:
        ...
        if value and value not in VALID_BODY_LOGIN_PASSWD_CHANGE:
            return (False, f"Invalid login-passwd-change '{value}'. Must be one of: {', '.join(VALID_BODY_LOGIN_PASSWD_CHANGE)}")
    
    # Validate lldp if present
    if 'lldp' in payload:
        value = payload.get('lldp')
        if value and value not in VALID_BODY_LLDP:
            return (False, f"Invalid lldp '{value}'. Must be one of: {', '.join(VALID_BODY_LLDP)}")
    ...
    if 'poe-mode' in payload:
        ...
        if value and value not in VALID_BODY_POE_MODE:
            return (False, f"Invalid poe-mode '{value}'. Must be one of: {', '.join(VALID_BODY_POE_MODE)}")
    ...
    if 'usb-port' in payload:
        ...
        if value and value not in VALID_BODY_USB_PORT:
            return (False, f"Invalid usb-port '{value}'. Must be one of: {', '.join(VALID_BODY_USB_PORT)}")
    ...
    if 'frequency-handoff' in payload:
        ...
        if value and value not in VALID_BODY_FREQUENCY_HANDOFF:
            return (False, f"Invalid frequency-handoff '{value}'. Must be one of: {', '.join(VALID_BODY_FREQUENCY_HANDOFF)}")
    ...
    if 'ap-handoff' in payload:
        ...
        if value and value not in VALID_BODY_AP_HANDOFF:
            return (False, f"Invalid ap-handoff '{value}'. Must be one of: {', '.join(VALID_BODY_AP_HANDOFF)}")
    ...
    if 'default-mesh-root' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_MESH_ROOT:
            return (False, f"Invalid default-mesh-root '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_MESH_ROOT)}")
    ...
    if 'ext-info-enable' in payload:
        ...
        if value and value not in VALID_BODY_EXT_INFO_ENABLE:
            return (False, f"Invalid ext-info-enable '{value}'. Must be one of: {', '.join(VALID_BODY_EXT_INFO_ENABLE)}")
    ...
    if 'indoor-outdoor-deployment' in payload:
        ...
        if value and value not in VALID_BODY_INDOOR_OUTDOOR_DEPLOYMENT:
            return (False, f"Invalid indoor-outdoor-deployment '{value}'. Must be one of: {', '.join(VALID_BODY_INDOOR_OUTDOOR_DEPLOYMENT)}")
    ...
    if 'console-login' in payload:
        ...
        if value and value not in VALID_BODY_CONSOLE_LOGIN:
            return (False, f"Invalid console-login '{value}'. Must be one of: {', '.join(VALID_BODY_CONSOLE_LOGIN)}")
    ...
    if 'wan-port-auth' in payload:
        ...
        if value and value not in VALID_BODY_WAN_PORT_AUTH:
            return (False, f"Invalid wan-port-auth '{value}'. Must be one of: {', '.join(VALID_BODY_WAN_PORT_AUTH)}")
    ...
    if 'wan-port-auth-usrname' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 63:
            return (False, f"wan-port-auth-usrname cannot exceed 63 characters")
    ...
    if 'wan-port-auth-methods' in payload:
        ...
        if value and value not in VALID_BODY_WAN_PORT_AUTH_METHODS:
            return (False, f"Invalid wan-port-auth-methods '{value}'. Must be one of: {', '.join(VALID_BODY_WAN_PORT_AUTH_METHODS)}")
    ...
    if 'wan-port-auth-macsec' in payload:
        ...
        if value and value not in VALID_BODY_WAN_PORT_AUTH_MACSEC:
            return (False, f"Invalid wan-port-auth-macsec '{value}'. Must be one of: {', '.join(VALID_BODY_WAN_PORT_AUTH_MACSEC)}")
    ...
    if 'apcfg-auto-cert' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_AUTO_CERT:
            return (False, f"Invalid apcfg-auto-cert '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_AUTO_CERT)}")
    ...
    if 'apcfg-auto-cert-enroll-protocol' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_AUTO_CERT_ENROLL_PROTOCOL:
            return (False, f"Invalid apcfg-auto-cert-enroll-protocol '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_AUTO_CERT_ENROLL_PROTOCOL)}")
    ...
    if 'apcfg-auto-cert-crypto-algo' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_AUTO_CERT_CRYPTO_ALGO:
            return (False, f"Invalid apcfg-auto-cert-crypto-algo '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_AUTO_CERT_CRYPTO_ALGO)}")
    ...
    if 'apcfg-auto-cert-est-server' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"apcfg-auto-cert-est-server cannot exceed 255 characters")
    ...
    if 'apcfg-auto-cert-est-ca-id' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"apcfg-auto-cert-est-ca-id cannot exceed 255 characters")
    ...
    if 'apcfg-auto-cert-est-http-username' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 63:
            return (False, f"apcfg-auto-cert-est-http-username cannot exceed 63 characters")
    ...
    if 'apcfg-auto-cert-est-subject' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 127:
            return (False, f"apcfg-auto-cert-est-subject cannot exceed 127 characters")
    ...
    if 'apcfg-auto-cert-est-subject-alt-name' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 127:
            return (False, f"apcfg-auto-cert-est-subject-alt-name cannot exceed 127 characters")
    ...
    if 'apcfg-auto-cert-auto-regen-days' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"apcfg-auto-cert-auto-regen-days must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"apcfg-auto-cert-auto-regen-days must be numeric, got: {value}")
    ...
    if 'apcfg-auto-cert-est-https-ca' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 79:
            return (False, f"apcfg-auto-cert-est-https-ca cannot exceed 79 characters")
    ...
    if 'apcfg-auto-cert-scep-keytype' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_AUTO_CERT_SCEP_KEYTYPE:
            return (False, f"Invalid apcfg-auto-cert-scep-keytype '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_AUTO_CERT_SCEP_KEYTYPE)}")
    ...
    if 'apcfg-auto-cert-scep-keysize' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_AUTO_CERT_SCEP_KEYSIZE:
            return (False, f"Invalid apcfg-auto-cert-scep-keysize '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_AUTO_CERT_SCEP_KEYSIZE)}")
    ...
    if 'apcfg-auto-cert-scep-ec-name' in payload:
        ...
        if value and value not in VALID_BODY_APCFG_AUTO_CERT_SCEP_EC_NAME:
            return (False, f"Invalid apcfg-auto-cert-scep-ec-name '{value}'. Must be one of: {', '.join(VALID_BODY_APCFG_AUTO_CERT_SCEP_EC_NAME)}")
    ...
    if 'apcfg-auto-cert-scep-sub-fully-dn' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"apcfg-auto-cert-scep-sub-fully-dn cannot exceed 255 characters")
    ...
    if 'apcfg-auto-cert-scep-url' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"apcfg-auto-cert-scep-url cannot exceed 255 characters")
    ...
    if 'apcfg-auto-cert-scep-ca-id' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"apcfg-auto-cert-scep-ca-id cannot exceed 255 characters")
    ...
    if 'apcfg-auto-cert-scep-subject-alt-name' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 127:
            return (False, f"apcfg-auto-cert-scep-subject-alt-name cannot exceed 127 characters")
    ...
    if 'apcfg-auto-cert-scep-https-ca' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 79:
            return (False, f"apcfg-auto-cert-scep-https-ca cannot exceed 79 characters")
    ...
    if 'unii-4-5ghz-band' in payload:
        ...
        if value and value not in VALID_BODY_UNII_4_5GHZ_BAND:
            return (False, f"Invalid unii-4-5ghz-band '{value}'. Must be one of: {', '.join(VALID_BODY_UNII_4_5GHZ_BAND)}")
...
def validate_wtp_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
