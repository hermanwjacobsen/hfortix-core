...
VALID_BODY_IPV4_ADDRESS_TYPE = ['not-available', 'public', 'port-restricted', 'single-NATed-private', 'double-NATed-private', 'port-restricted-and-single-NATed', 'port-restricted-and-double-NATed', 'not-known']
...
def validate_hotspot20_anqp_ip_address_type_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'ipv6-address-type' in payload:
        ...
        if value and value not in VALID_BODY_IPV6_ADDRESS_TYPE:
            return (False, f"Invalid ipv6-address-type '{value}'. Must be one of: {', '.join(VALID_BODY_IPV6_ADDRESS_TYPE)}")
    ...
    if 'ipv4-address-type' in payload:
        ...
        if value and value not in VALID_BODY_IPV4_ADDRESS_TYPE:
            return (False, f"Invalid ipv4-address-type '{value}'. Must be one of: {', '.join(VALID_BODY_IPV4_ADDRESS_TYPE)}")
...
def validate_hotspot20_anqp_ip_address_type_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
