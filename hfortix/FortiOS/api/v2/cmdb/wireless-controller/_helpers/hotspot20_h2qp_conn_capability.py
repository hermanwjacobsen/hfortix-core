...
def validate_hotspot20_h2qp_conn_capability_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'icmp-port' in payload:
        ...
        if value and value not in VALID_BODY_ICMP_PORT:
            return (False, f"Invalid icmp-port '{value}'. Must be one of: {', '.join(VALID_BODY_ICMP_PORT)}")
    ...
    if 'ftp-port' in payload:
        ...
        if value and value not in VALID_BODY_FTP_PORT:
            return (False, f"Invalid ftp-port '{value}'. Must be one of: {', '.join(VALID_BODY_FTP_PORT)}")
    ...
    if 'ssh-port' in payload:
        ...
        if value and value not in VALID_BODY_SSH_PORT:
            return (False, f"Invalid ssh-port '{value}'. Must be one of: {', '.join(VALID_BODY_SSH_PORT)}")
    ...
    if 'http-port' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_PORT:
            return (False, f"Invalid http-port '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_PORT)}")
    ...
    if 'tls-port' in payload:
        ...
        if value and value not in VALID_BODY_TLS_PORT:
            return (False, f"Invalid tls-port '{value}'. Must be one of: {', '.join(VALID_BODY_TLS_PORT)}")
    ...
    if 'pptp-vpn-port' in payload:
        ...
        if value and value not in VALID_BODY_PPTP_VPN_PORT:
            return (False, f"Invalid pptp-vpn-port '{value}'. Must be one of: {', '.join(VALID_BODY_PPTP_VPN_PORT)}")
    ...
    if 'voip-tcp-port' in payload:
        ...
        if value and value not in VALID_BODY_VOIP_TCP_PORT:
            return (False, f"Invalid voip-tcp-port '{value}'. Must be one of: {', '.join(VALID_BODY_VOIP_TCP_PORT)}")
    ...
    if 'voip-udp-port' in payload:
        ...
        if value and value not in VALID_BODY_VOIP_UDP_PORT:
            return (False, f"Invalid voip-udp-port '{value}'. Must be one of: {', '.join(VALID_BODY_VOIP_UDP_PORT)}")
    ...
    if 'ikev2-port' in payload:
        ...
        if value and value not in VALID_BODY_IKEV2_PORT:
            return (False, f"Invalid ikev2-port '{value}'. Must be one of: {', '.join(VALID_BODY_IKEV2_PORT)}")
    ...
    if 'ikev2-xx-port' in payload:
        ...
        if value and value not in VALID_BODY_IKEV2_XX_PORT:
            return (False, f"Invalid ikev2-xx-port '{value}'. Must be one of: {', '.join(VALID_BODY_IKEV2_XX_PORT)}")
    ...
    if 'esp-port' in payload:
        ...
        if value and value not in VALID_BODY_ESP_PORT:
            return (False, f"Invalid esp-port '{value}'. Must be one of: {', '.join(VALID_BODY_ESP_PORT)}")
...
def validate_hotspot20_h2qp_conn_capability_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
