...
def validate_qos_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'downlink-sta' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2097152:
                    return (False, f"downlink-sta must be between 0 and 2097152")
            except (ValueError, TypeError):
    ...
    if 'burst' in payload:
        ...
        if value and value not in VALID_BODY_BURST:
            return (False, f"Invalid burst '{value}'. Must be one of: {', '.join(VALID_BODY_BURST)}")
    
    # Validate wmm if present
    if 'wmm' in payload:
        value = payload.get('wmm')
        if value and value not in VALID_BODY_WMM:
            return (False, f"Invalid wmm '{value}'. Must be one of: {', '.join(VALID_BODY_WMM)}")
    ...
    if 'wmm-uapsd' in payload:
        ...
        if value and value not in VALID_BODY_WMM_UAPSD:
            return (False, f"Invalid wmm-uapsd '{value}'. Must be one of: {', '.join(VALID_BODY_WMM_UAPSD)}")
    ...
    if 'call-admission-control' in payload:
        ...
        if value and value not in VALID_BODY_CALL_ADMISSION_CONTROL:
            return (False, f"Invalid call-admission-control '{value}'. Must be one of: {', '.join(VALID_BODY_CALL_ADMISSION_CONTROL)}")
    ...
    if 'bandwidth-admission-control' in payload:
        ...
        if value and value not in VALID_BODY_BANDWIDTH_ADMISSION_CONTROL:
            return (False, f"Invalid bandwidth-admission-control '{value}'. Must be one of: {', '.join(VALID_BODY_BANDWIDTH_ADMISSION_CONTROL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 600000:
                    return (False, f"bandwidth-capacity must be between 1 and 600000")
            except (ValueError, TypeError):
                return (False, f"bandwidth-capacity must be numeric, got: {value}")
    ...
    if 'dscp-wmm-mapping' in payload:
        ...
        if value and value not in VALID_BODY_DSCP_WMM_MAPPING:
            return (False, f"Invalid dscp-wmm-mapping '{value}'. Must be one of: {', '.join(VALID_BODY_DSCP_WMM_MAPPING)}")
    ...
    if 'wmm-dscp-marking' in payload:
        ...
        if value and value not in VALID_BODY_WMM_DSCP_MARKING:
            return (False, f"Invalid wmm-dscp-marking '{value}'. Must be one of: {', '.join(VALID_BODY_WMM_DSCP_MARKING)}")
...
def validate_qos_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
