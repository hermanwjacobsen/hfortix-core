...
def validate_inter_controller_put(
    ...
    if 'inter-controller-mode' in payload:
        ...
        if value and value not in VALID_BODY_INTER_CONTROLLER_MODE:
            return (False, f"Invalid inter-controller-mode '{value}'. Must be one of: {', '.join(VALID_BODY_INTER_CONTROLLER_MODE)}")
    ...
    if 'inter-controller-pri' in payload:
        ...
        if value and value not in VALID_BODY_INTER_CONTROLLER_PRI:
            return (False, f"Invalid inter-controller-pri '{value}'. Must be one of: {', '.join(VALID_BODY_INTER_CONTROLLER_PRI)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 3 or int_val > 64:
                    return (False, f"fast-failover-max must be between 3 and 64")
            except (ValueError, TypeError):
                return (False, f"fast-failover-max must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 86400:
                    return (False, f"fast-failover-wait must be between 10 and 86400")
            except (ValueError, TypeError):
                return (False, f"fast-failover-wait must be numeric, got: {value}")
