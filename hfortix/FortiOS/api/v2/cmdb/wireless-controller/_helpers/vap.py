...
VALID_BODY_SECURITY = ['open', 'wep64', 'wep128', 'wpa-personal', 'wpa-enterprise', 'wpa-only-personal', 'wpa-only-enterprise', 'wpa2-only-personal', 'wpa2-only-enterprise', 'wpa3-enterprise', 'wpa3-only-enterprise', 'wpa3-enterprise-transition', 'wpa3-sae', 'wpa3-sae-transition', 'owe', 'osen']
...
VALID_BODY_EXTERNAL_WEB_FORMAT = ['auto-detect', 'no-query-string', 'partial-query-string']
VALID_BODY_MAC_USERNAME_DELIMITER = ['hyphen', 'single-hyphen', 'colon', 'none']
VALID_BODY_MAC_PASSWORD_DELIMITER = ['hyphen', 'single-hyphen', 'colon', 'none']
VALID_BODY_MAC_CALLING_STATION_DELIMITER = ['hyphen', 'single-hyphen', 'colon', 'none']
VALID_BODY_MAC_CALLED_STATION_DELIMITER = ['hyphen', 'single-hyphen', 'colon', 'none']
...
VALID_BODY_PORTAL_TYPE = ['auth', 'auth+disclaimer', 'disclaimer', 'email-collect', 'cmcc', 'cmcc-macauth', 'auth-mac', 'external-auth', 'external-macauth']
...
VALID_BODY_BROADCAST_SUPPRESSION = ['dhcp-up', 'dhcp-down', 'dhcp-starvation', 'dhcp-ucast', 'arp-known', 'arp-unknown', 'arp-reply', 'arp-poison', 'arp-proxy', 'netbios-ns', 'netbios-ds', 'ipv6', 'all-other-mc', 'all-other-bc']
VALID_BODY_IPV6_RULES = ['drop-icmp6ra', 'drop-icmp6rs', 'drop-llmnr6', 'drop-icmp6mld2', 'drop-dhcp6s', 'drop-dhcp6c', 'ndp-proxy', 'drop-ns-dad', 'drop-ns-nondad']
...
VALID_BODY_DHCP_OPTION82_CIRCUIT_ID_INSERTION = ['style-1', 'style-2', 'style-3', 'disable']
...
VALID_BODY_RATES_11A = ['6', '6-basic', '9', '9-basic', '12', '12-basic', '18', '18-basic', '24', '24-basic', '36', '36-basic', '48', '48-basic', '54', '54-basic']
VALID_BODY_RATES_11BG = ['1', '1-basic', '2', '2-basic', '5.5', '5.5-basic', '11', '11-basic', '6', '6-basic', '9', '9-basic', '12', '12-basic', '18', '18-basic', '24', '24-basic', '36', '36-basic', '48', '48-basic', '54', '54-basic']
VALID_BODY_RATES_11N_SS12 = ['mcs0/1', 'mcs1/1', 'mcs2/1', 'mcs3/1', 'mcs4/1', 'mcs5/1', 'mcs6/1', 'mcs7/1', 'mcs8/2', 'mcs9/2', 'mcs10/2', 'mcs11/2', 'mcs12/2', 'mcs13/2', 'mcs14/2', 'mcs15/2']
VALID_BODY_RATES_11N_SS34 = ['mcs16/3', 'mcs17/3', 'mcs18/3', 'mcs19/3', 'mcs20/3', 'mcs21/3', 'mcs22/3', 'mcs23/3', 'mcs24/4', 'mcs25/4', 'mcs26/4', 'mcs27/4', 'mcs28/4', 'mcs29/4', 'mcs30/4', 'mcs31/4']
...
def validate_vap_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'pre-auth' in payload:
        ...
        if value and value not in VALID_BODY_PRE_AUTH:
            return (False, f"Invalid pre-auth '{value}'. Must be one of: {', '.join(VALID_BODY_PRE_AUTH)}")
    ...
    if 'external-pre-auth' in payload:
        ...
        if value and value not in VALID_BODY_EXTERNAL_PRE_AUTH:
            return (False, f"Invalid external-pre-auth '{value}'. Must be one of: {', '.join(VALID_BODY_EXTERNAL_PRE_AUTH)}")
    ...
    if 'mesh-backhaul' in payload:
        ...
        if value and value not in VALID_BODY_MESH_BACKHAUL:
            return (False, f"Invalid mesh-backhaul '{value}'. Must be one of: {', '.join(VALID_BODY_MESH_BACKHAUL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-clients must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'max-clients-ap' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-clients-ap must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'broadcast-ssid' in payload:
        ...
        if value and value not in VALID_BODY_BROADCAST_SSID:
            return (False, f"Invalid broadcast-ssid '{value}'. Must be one of: {', '.join(VALID_BODY_BROADCAST_SSID)}")
    ...
    if 'security' in payload:
        ...
        if value and value not in VALID_BODY_SECURITY:
            return (False, f"Invalid security '{value}'. Must be one of: {', '.join(VALID_BODY_SECURITY)}")
    
    # Validate pmf if present
    if 'pmf' in payload:
        value = payload.get('pmf')
        if value and value not in VALID_BODY_PMF:
            return (False, f"Invalid pmf '{value}'. Must be one of: {', '.join(VALID_BODY_PMF)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 20:
                    return (False, f"pmf-assoc-comeback-timeout must be between 1 and 20")
            except (ValueError, TypeError):
                return (False, f"pmf-assoc-comeback-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 5:
                    return (False, f"pmf-sa-query-retry-timeout must be between 1 and 5")
            except (ValueError, TypeError):
                return (False, f"pmf-sa-query-retry-timeout must be numeric, got: {value}")
    ...
    if 'beacon-protection' in payload:
        ...
        if value and value not in VALID_BODY_BEACON_PROTECTION:
            return (False, f"Invalid beacon-protection '{value}'. Must be one of: {', '.join(VALID_BODY_BEACON_PROTECTION)}")
    
    # Validate okc if present
    if 'okc' in payload:
        value = payload.get('okc')
        if value and value not in VALID_BODY_OKC:
            return (False, f"Invalid okc '{value}'. Must be one of: {', '.join(VALID_BODY_OKC)}")
    
    # Validate mbo if present
    if 'mbo' in payload:
        value = payload.get('mbo')
        if value and value not in VALID_BODY_MBO:
            return (False, f"Invalid mbo '{value}'. Must be one of: {', '.join(VALID_BODY_MBO)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 100 or int_val > 10000:
                    return (False, f"gas-comeback-delay must be between 100 and 10000")
            except (ValueError, TypeError):
                return (False, f"gas-comeback-delay must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 512 or int_val > 4096:
                    return (False, f"gas-fragmentation-limit must be between 512 and 4096")
            except (ValueError, TypeError):
                return (False, f"gas-fragmentation-limit must be numeric, got: {value}")
    ...
    if 'mbo-cell-data-conn-pref' in payload:
        ...
        if value and value not in VALID_BODY_MBO_CELL_DATA_CONN_PREF:
            return (False, f"Invalid mbo-cell-data-conn-pref '{value}'. Must be one of: {', '.join(VALID_BODY_MBO_CELL_DATA_CONN_PREF)}")
    
    # Validate 80211k if present
    if '80211k' in payload:
        value = payload.get('80211k')
        if value and value not in VALID_BODY_80211K:
            return (False, f"Invalid 80211k '{value}'. Must be one of: {', '.join(VALID_BODY_80211K)}")
    
    # Validate 80211v if present
    if '80211v' in payload:
        value = payload.get('80211v')
        if value and value not in VALID_BODY_80211V:
            return (False, f"Invalid 80211v '{value}'. Must be one of: {', '.join(VALID_BODY_80211V)}")
    ...
    if 'neighbor-report-dual-band' in payload:
        ...
        if value and value not in VALID_BODY_NEIGHBOR_REPORT_DUAL_BAND:
            return (False, f"Invalid neighbor-report-dual-band '{value}'. Must be one of: {', '.join(VALID_BODY_NEIGHBOR_REPORT_DUAL_BAND)}")
    ...
    if 'fast-bss-transition' in payload:
        ...
        if value and value not in VALID_BODY_FAST_BSS_TRANSITION:
            return (False, f"Invalid fast-bss-transition '{value}'. Must be one of: {', '.join(VALID_BODY_FAST_BSS_TRANSITION)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"ft-mobility-domain must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"ft-mobility-domain must be numeric, got: {value}")
    ...
    if 'ft-r0-key-lifetime' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"ft-r0-key-lifetime must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"ft-r0-key-lifetime must be numeric, got: {value}")
    ...
    if 'ft-over-ds' in payload:
        ...
        if value and value not in VALID_BODY_FT_OVER_DS:
            return (False, f"Invalid ft-over-ds '{value}'. Must be one of: {', '.join(VALID_BODY_FT_OVER_DS)}")
    ...
    if 'sae-groups' in payload:
        ...
        if value and value not in VALID_BODY_SAE_GROUPS:
            return (False, f"Invalid sae-groups '{value}'. Must be one of: {', '.join(VALID_BODY_SAE_GROUPS)}")
    ...
    if 'owe-groups' in payload:
        ...
        if value and value not in VALID_BODY_OWE_GROUPS:
            return (False, f"Invalid owe-groups '{value}'. Must be one of: {', '.join(VALID_BODY_OWE_GROUPS)}")
    ...
    if 'owe-transition' in payload:
        ...
        if value and value not in VALID_BODY_OWE_TRANSITION:
            return (False, f"Invalid owe-transition '{value}'. Must be one of: {', '.join(VALID_BODY_OWE_TRANSITION)}")
    ...
    if 'additional-akms' in payload:
        ...
        if value and value not in VALID_BODY_ADDITIONAL_AKMS:
            return (False, f"Invalid additional-akms '{value}'. Must be one of: {', '.join(VALID_BODY_ADDITIONAL_AKMS)}")
    ...
    if 'eapol-key-retries' in payload:
        ...
        if value and value not in VALID_BODY_EAPOL_KEY_RETRIES:
            return (False, f"Invalid eapol-key-retries '{value}'. Must be one of: {', '.join(VALID_BODY_EAPOL_KEY_RETRIES)}")
    ...
    if 'tkip-counter-measure' in payload:
        ...
        if value and value not in VALID_BODY_TKIP_COUNTER_MEASURE:
            return (False, f"Invalid tkip-counter-measure '{value}'. Must be one of: {', '.join(VALID_BODY_TKIP_COUNTER_MEASURE)}")
    ...
    if 'external-web-format' in payload:
        ...
        if value and value not in VALID_BODY_EXTERNAL_WEB_FORMAT:
            return (False, f"Invalid external-web-format '{value}'. Must be one of: {', '.join(VALID_BODY_EXTERNAL_WEB_FORMAT)}")
    ...
    if 'mac-username-delimiter' in payload:
        ...
        if value and value not in VALID_BODY_MAC_USERNAME_DELIMITER:
            return (False, f"Invalid mac-username-delimiter '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_USERNAME_DELIMITER)}")
    ...
    if 'mac-password-delimiter' in payload:
        ...
        if value and value not in VALID_BODY_MAC_PASSWORD_DELIMITER:
            return (False, f"Invalid mac-password-delimiter '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_PASSWORD_DELIMITER)}")
    ...
    if 'mac-calling-station-delimiter' in payload:
        ...
        if value and value not in VALID_BODY_MAC_CALLING_STATION_DELIMITER:
            return (False, f"Invalid mac-calling-station-delimiter '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_CALLING_STATION_DELIMITER)}")
    ...
    if 'mac-called-station-delimiter' in payload:
        ...
        if value and value not in VALID_BODY_MAC_CALLED_STATION_DELIMITER:
            return (False, f"Invalid mac-called-station-delimiter '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_CALLED_STATION_DELIMITER)}")
    ...
    if 'mac-case' in payload:
        ...
        if value and value not in VALID_BODY_MAC_CASE:
            return (False, f"Invalid mac-case '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_CASE)}")
    ...
    if 'called-station-id-type' in payload:
        ...
        if value and value not in VALID_BODY_CALLED_STATION_ID_TYPE:
            return (False, f"Invalid called-station-id-type '{value}'. Must be one of: {', '.join(VALID_BODY_CALLED_STATION_ID_TYPE)}")
    ...
    if 'mac-auth-bypass' in payload:
        ...
        if value and value not in VALID_BODY_MAC_AUTH_BYPASS:
            return (False, f"Invalid mac-auth-bypass '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_AUTH_BYPASS)}")
    ...
    if 'radius-mac-auth' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_MAC_AUTH:
            return (False, f"Invalid radius-mac-auth '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_MAC_AUTH)}")
    ...
    if 'radius-mac-auth-server' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"radius-mac-auth-server cannot exceed 35 characters")
        ...
        if value is not None:
            try:
                ...
                if int_val < 30 or int_val > 864000:
                    return (False, f"radius-mac-auth-block-interval must be between 30 and 864000")
            except (ValueError, TypeError):
                return (False, f"radius-mac-auth-block-interval must be numeric, got: {value}")
    ...
    if 'radius-mac-mpsk-auth' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_MAC_MPSK_AUTH:
            return (False, f"Invalid radius-mac-mpsk-auth '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_MAC_MPSK_AUTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 300 or int_val > 864000:
                    return (False, f"radius-mac-mpsk-timeout must be between 300 and 864000")
            except (ValueError, TypeError):
                return (False, f"radius-mac-mpsk-timeout must be numeric, got: {value}")
    
    # Validate auth if present
    if 'auth' in payload:
        value = payload.get('auth')
        if value and value not in VALID_BODY_AUTH:
            return (False, f"Invalid auth '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH)}")
    
    # Validate encrypt if present
    if 'encrypt' in payload:
        value = payload.get('encrypt')
        if value and value not in VALID_BODY_ENCRYPT:
            return (False, f"Invalid encrypt '{value}'. Must be one of: {', '.join(VALID_BODY_ENCRYPT)}")
    ...
    if 'sae-h2e-only' in payload:
        ...
        if value and value not in VALID_BODY_SAE_H2E_ONLY:
            return (False, f"Invalid sae-h2e-only '{value}'. Must be one of: {', '.join(VALID_BODY_SAE_H2E_ONLY)}")
    ...
    if 'sae-hnp-only' in payload:
        ...
        if value and value not in VALID_BODY_SAE_HNP_ONLY:
            return (False, f"Invalid sae-hnp-only '{value}'. Must be one of: {', '.join(VALID_BODY_SAE_HNP_ONLY)}")
    
    # Validate sae-pk if present
    if 'sae-pk' in payload:
        value = payload.get('sae-pk')
        if value and value not in VALID_BODY_SAE_PK:
            return (False, f"Invalid sae-pk '{value}'. Must be one of: {', '.join(VALID_BODY_SAE_PK)}")
    ...
    if 'akm24-only' in payload:
        ...
        if value and value not in VALID_BODY_AKM24_ONLY:
            return (False, f"Invalid akm24-only '{value}'. Must be one of: {', '.join(VALID_BODY_AKM24_ONLY)}")
    ...
    if 'nas-filter-rule' in payload:
        ...
        if value and value not in VALID_BODY_NAS_FILTER_RULE:
            return (False, f"Invalid nas-filter-rule '{value}'. Must be one of: {', '.join(VALID_BODY_NAS_FILTER_RULE)}")
    ...
    if 'domain-name-stripping' in payload:
        ...
        if value and value not in VALID_BODY_DOMAIN_NAME_STRIPPING:
            return (False, f"Invalid domain-name-stripping '{value}'. Must be one of: {', '.join(VALID_BODY_DOMAIN_NAME_STRIPPING)}")
    
    # Validate mlo if present
    if 'mlo' in payload:
        value = payload.get('mlo')
        if value and value not in VALID_BODY_MLO:
            return (False, f"Invalid mlo '{value}'. Must be one of: {', '.join(VALID_BODY_MLO)}")
    ...
    if 'local-standalone' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_STANDALONE:
            return (False, f"Invalid local-standalone '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_STANDALONE)}")
    ...
    if 'local-standalone-nat' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_STANDALONE_NAT:
            return (False, f"Invalid local-standalone-nat '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_STANDALONE_NAT)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 300 or int_val > 8640000:
                    return (False, f"dhcp-lease-time must be between 300 and 8640000")
            except (ValueError, TypeError):
                return (False, f"dhcp-lease-time must be numeric, got: {value}")
    ...
    if 'local-standalone-dns' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_STANDALONE_DNS:
            return (False, f"Invalid local-standalone-dns '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_STANDALONE_DNS)}")
    ...
    if 'local-lan-partition' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_LAN_PARTITION:
            return (False, f"Invalid local-lan-partition '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_LAN_PARTITION)}")
    ...
    if 'local-bridging' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_BRIDGING:
            return (False, f"Invalid local-bridging '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_BRIDGING)}")
    ...
    if 'local-lan' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_LAN:
            return (False, f"Invalid local-lan '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_LAN)}")
    ...
    if 'local-authentication' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_AUTHENTICATION:
            return (False, f"Invalid local-authentication '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_AUTHENTICATION)}")
    ...
    if 'captive-portal' in payload:
        ...
        if value and value not in VALID_BODY_CAPTIVE_PORTAL:
            return (False, f"Invalid captive-portal '{value}'. Must be one of: {', '.join(VALID_BODY_CAPTIVE_PORTAL)}")
    ...
    if 'captive-network-assistant-bypass' in payload:
        ...
        if value and value not in VALID_BODY_CAPTIVE_NETWORK_ASSISTANT_BYPASS:
            return (False, f"Invalid captive-network-assistant-bypass '{value}'. Must be one of: {', '.join(VALID_BODY_CAPTIVE_NETWORK_ASSISTANT_BYPASS)}")
    ...
    if 'portal-message-override-group' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"portal-message-override-group cannot exceed 35 characters")
    ...
    if 'portal-type' in payload:
        ...
        if value and value not in VALID_BODY_PORTAL_TYPE:
            return (False, f"Invalid portal-type '{value}'. Must be one of: {', '.join(VALID_BODY_PORTAL_TYPE)}")
    ...
    if 'intra-vap-privacy' in payload:
        ...
        if value and value not in VALID_BODY_INTRA_VAP_PRIVACY:
            return (False, f"Invalid intra-vap-privacy '{value}'. Must be one of: {', '.join(VALID_BODY_INTRA_VAP_PRIVACY)}")
    
    # Validate ldpc if present
    if 'ldpc' in payload:
        value = payload.get('ldpc')
        if value and value not in VALID_BODY_LDPC:
            return (False, f"Invalid ldpc '{value}'. Must be one of: {', '.join(VALID_BODY_LDPC)}")
    ...
    if 'high-efficiency' in payload:
        ...
        if value and value not in VALID_BODY_HIGH_EFFICIENCY:
            return (False, f"Invalid high-efficiency '{value}'. Must be one of: {', '.join(VALID_BODY_HIGH_EFFICIENCY)}")
    ...
    if 'target-wake-time' in payload:
        ...
        if value and value not in VALID_BODY_TARGET_WAKE_TIME:
            return (False, f"Invalid target-wake-time '{value}'. Must be one of: {', '.join(VALID_BODY_TARGET_WAKE_TIME)}")
    ...
    if 'port-macauth' in payload:
        ...
        if value and value not in VALID_BODY_PORT_MACAUTH:
            return (False, f"Invalid port-macauth '{value}'. Must be one of: {', '.join(VALID_BODY_PORT_MACAUTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 65535:
                    return (False, f"port-macauth-timeout must be between 60 and 65535")
            except (ValueError, TypeError):
                return (False, f"port-macauth-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 120 or int_val > 65535:
                    return (False, f"port-macauth-reauth-timeout must be between 120 and 65535")
            except (ValueError, TypeError):
                return (False, f"port-macauth-reauth-timeout must be numeric, got: {value}")
    ...
    if 'bss-color-partial' in payload:
        ...
        if value and value not in VALID_BODY_BSS_COLOR_PARTIAL:
            return (False, f"Invalid bss-color-partial '{value}'. Must be one of: {', '.join(VALID_BODY_BSS_COLOR_PARTIAL)}")
    ...
    if 'split-tunneling' in payload:
        ...
        if value and value not in VALID_BODY_SPLIT_TUNNELING:
            return (False, f"Invalid split-tunneling '{value}'. Must be one of: {', '.join(VALID_BODY_SPLIT_TUNNELING)}")
    
    # Validate nac if present
    if 'nac' in payload:
        value = payload.get('nac')
        if value and value not in VALID_BODY_NAC:
            return (False, f"Invalid nac '{value}'. Must be one of: {', '.join(VALID_BODY_NAC)}")
    ...
    if 'vlan-auto' in payload:
        ...
        if value and value not in VALID_BODY_VLAN_AUTO:
            return (False, f"Invalid vlan-auto '{value}'. Must be one of: {', '.join(VALID_BODY_VLAN_AUTO)}")
    ...
    if 'dynamic-vlan' in payload:
        ...
        if value and value not in VALID_BODY_DYNAMIC_VLAN:
            return (False, f"Invalid dynamic-vlan '{value}'. Must be one of: {', '.join(VALID_BODY_DYNAMIC_VLAN)}")
    ...
    if 'captive-portal-fw-accounting' in payload:
        ...
        if value and value not in VALID_BODY_CAPTIVE_PORTAL_FW_ACCOUNTING:
            return (False, f"Invalid captive-portal-fw-accounting '{value}'. Must be one of: {', '.join(VALID_BODY_CAPTIVE_PORTAL_FW_ACCOUNTING)}")
    
    # Validate captive-portal-ac-name if present
    if 'captive-portal-ac-name' in payload:
        value = payload.get('captive-portal-ac-name')
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"captive-portal-ac-name cannot exceed 35 characters")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 864000:
                    return (False, f"captive-portal-auth-timeout must be between 0 and 864000")
            except (ValueError, TypeError):
                return (False, f"captive-portal-auth-timeout must be numeric, got: {value}")
    ...
    if 'multicast-rate' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_RATE:
            return (False, f"Invalid multicast-rate '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_RATE)}")
    ...
    if 'multicast-enhance' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_ENHANCE:
            return (False, f"Invalid multicast-enhance '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_ENHANCE)}")
    ...
    if 'igmp-snooping' in payload:
        ...
        if value and value not in VALID_BODY_IGMP_SNOOPING:
            return (False, f"Invalid igmp-snooping '{value}'. Must be one of: {', '.join(VALID_BODY_IGMP_SNOOPING)}")
    ...
    if 'dhcp-address-enforcement' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_ADDRESS_ENFORCEMENT:
            return (False, f"Invalid dhcp-address-enforcement '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_ADDRESS_ENFORCEMENT)}")
    ...
    if 'broadcast-suppression' in payload:
        ...
        if value and value not in VALID_BODY_BROADCAST_SUPPRESSION:
            return (False, f"Invalid broadcast-suppression '{value}'. Must be one of: {', '.join(VALID_BODY_BROADCAST_SUPPRESSION)}")
    ...
    if 'ipv6-rules' in payload:
        ...
        if value and value not in VALID_BODY_IPV6_RULES:
            return (False, f"Invalid ipv6-rules '{value}'. Must be one of: {', '.join(VALID_BODY_IPV6_RULES)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 256:
                    return (False, f"me-disable-thresh must be between 2 and 256")
            except (ValueError, TypeError):
                return (False, f"me-disable-thresh must be numeric, got: {value}")
    
    # Validate mu-mimo if present
    if 'mu-mimo' in payload:
        value = payload.get('mu-mimo')
        if value and value not in VALID_BODY_MU_MIMO:
            return (False, f"Invalid mu-mimo '{value}'. Must be one of: {', '.join(VALID_BODY_MU_MIMO)}")
    ...
    if 'probe-resp-suppression' in payload:
        ...
        if value and value not in VALID_BODY_PROBE_RESP_SUPPRESSION:
            return (False, f"Invalid probe-resp-suppression '{value}'. Must be one of: {', '.join(VALID_BODY_PROBE_RESP_SUPPRESSION)}")
    ...
    if 'radio-sensitivity' in payload:
        ...
        if value and value not in VALID_BODY_RADIO_SENSITIVITY:
            return (False, f"Invalid radio-sensitivity '{value}'. Must be one of: {', '.join(VALID_BODY_RADIO_SENSITIVITY)}")
    ...
    if 'quarantine' in payload:
        ...
        if value and value not in VALID_BODY_QUARANTINE:
            return (False, f"Invalid quarantine '{value}'. Must be one of: {', '.join(VALID_BODY_QUARANTINE)}")
    ...
    if 'vlan-pooling' in payload:
        ...
        if value and value not in VALID_BODY_VLAN_POOLING:
            return (False, f"Invalid vlan-pooling '{value}'. Must be one of: {', '.join(VALID_BODY_VLAN_POOLING)}")
    ...
    if 'dhcp-option43-insertion' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION43_INSERTION:
            return (False, f"Invalid dhcp-option43-insertion '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION43_INSERTION)}")
    ...
    if 'dhcp-option82-insertion' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION82_INSERTION:
            return (False, f"Invalid dhcp-option82-insertion '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION82_INSERTION)}")
    ...
    if 'dhcp-option82-circuit-id-insertion' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION82_CIRCUIT_ID_INSERTION:
            return (False, f"Invalid dhcp-option82-circuit-id-insertion '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION82_CIRCUIT_ID_INSERTION)}")
    ...
    if 'dhcp-option82-remote-id-insertion' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION82_REMOTE_ID_INSERTION:
            return (False, f"Invalid dhcp-option82-remote-id-insertion '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION82_REMOTE_ID_INSERTION)}")
    ...
    if 'ptk-rekey' in payload:
        ...
        if value and value not in VALID_BODY_PTK_REKEY:
            return (False, f"Invalid ptk-rekey '{value}'. Must be one of: {', '.join(VALID_BODY_PTK_REKEY)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 600 or int_val > 864000:
                    return (False, f"ptk-rekey-intv must be between 600 and 864000")
            except (ValueError, TypeError):
    ...
    if 'gtk-rekey' in payload:
        ...
        if value and value not in VALID_BODY_GTK_REKEY:
            return (False, f"Invalid gtk-rekey '{value}'. Must be one of: {', '.join(VALID_BODY_GTK_REKEY)}")
    ...
    if 'gtk-rekey-intv' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 600 or int_val > 864000:
                    return (False, f"gtk-rekey-intv must be between 600 and 864000")
            except (ValueError, TypeError):
    ...
    if 'eap-reauth' in payload:
        ...
        if value and value not in VALID_BODY_EAP_REAUTH:
            return (False, f"Invalid eap-reauth '{value}'. Must be one of: {', '.join(VALID_BODY_EAP_REAUTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1800 or int_val > 864000:
                    return (False, f"eap-reauth-intv must be between 1800 and 864000")
            except (ValueError, TypeError):
                return (False, f"eap-reauth-intv must be numeric, got: {value}")
    ...
    if 'roaming-acct-interim-update' in payload:
        ...
        if value and value not in VALID_BODY_ROAMING_ACCT_INTERIM_UPDATE:
            return (False, f"Invalid roaming-acct-interim-update '{value}'. Must be one of: {', '.join(VALID_BODY_ROAMING_ACCT_INTERIM_UPDATE)}")
    ...
    if 'secondary-wag-profile' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"secondary-wag-profile cannot exceed 35 characters")
    ...
    if 'tunnel-echo-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"tunnel-echo-interval must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"tunnel-echo-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"tunnel-fallback-interval must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"tunnel-fallback-interval must be numeric, got: {value}")
    ...
    if 'rates-11a' in payload:
        ...
        if value and value not in VALID_BODY_RATES_11A:
            return (False, f"Invalid rates-11a '{value}'. Must be one of: {', '.join(VALID_BODY_RATES_11A)}")
    ...
    if 'rates-11bg' in payload:
        ...
        if value and value not in VALID_BODY_RATES_11BG:
            return (False, f"Invalid rates-11bg '{value}'. Must be one of: {', '.join(VALID_BODY_RATES_11BG)}")
    ...
    if 'rates-11n-ss12' in payload:
        ...
        if value and value not in VALID_BODY_RATES_11N_SS12:
            return (False, f"Invalid rates-11n-ss12 '{value}'. Must be one of: {', '.join(VALID_BODY_RATES_11N_SS12)}")
    ...
    if 'rates-11n-ss34' in payload:
        ...
        if value and value not in VALID_BODY_RATES_11N_SS34:
            return (False, f"Invalid rates-11n-ss34 '{value}'. Must be one of: {', '.join(VALID_BODY_RATES_11N_SS34)}")
    ...
    if 'rates-11be-mcs-map-160' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 15:
            return (False, f"rates-11be-mcs-map-160 cannot exceed 15 characters")
    
    # Validate rates-11be-mcs-map-320 if present
    if 'rates-11be-mcs-map-320' in payload:
        value = payload.get('rates-11be-mcs-map-320')
        if value and isinstance(value, str) and len(value) > 15:
            return (False, f"rates-11be-mcs-map-320 cannot exceed 15 characters")
    ...
    if 'utm-log' in payload:
        ...
        if value and value not in VALID_BODY_UTM_LOG:
            return (False, f"Invalid utm-log '{value}'. Must be one of: {', '.join(VALID_BODY_UTM_LOG)}")
    ...
    if 'scan-botnet-connections' in payload:
        ...
        if value and value not in VALID_BODY_SCAN_BOTNET_CONNECTIONS:
            return (False, f"Invalid scan-botnet-connections '{value}'. Must be one of: {', '.join(VALID_BODY_SCAN_BOTNET_CONNECTIONS)}")
    ...
    if 'address-group-policy' in payload:
        ...
        if value and value not in VALID_BODY_ADDRESS_GROUP_POLICY:
            return (False, f"Invalid address-group-policy '{value}'. Must be one of: {', '.join(VALID_BODY_ADDRESS_GROUP_POLICY)}")
    ...
    if 'sticky-client-remove' in payload:
        ...
        if value and value not in VALID_BODY_STICKY_CLIENT_REMOVE:
            return (False, f"Invalid sticky-client-remove '{value}'. Must be one of: {', '.join(VALID_BODY_STICKY_CLIENT_REMOVE)}")
    ...
    if 'sticky-client-threshold-5g' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 7:
            return (False, f"sticky-client-threshold-5g cannot exceed 7 characters")
    ...
    if 'sticky-client-threshold-2g' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 7:
            return (False, f"sticky-client-threshold-2g cannot exceed 7 characters")
    ...
    if 'sticky-client-threshold-6g' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 7:
            return (False, f"sticky-client-threshold-6g cannot exceed 7 characters")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 2000:
                    return (False, f"bstm-rssi-disassoc-timer must be between 1 and 2000")
            except (ValueError, TypeError):
                return (False, f"bstm-rssi-disassoc-timer must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"bstm-load-balancing-disassoc-timer must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"bstm-load-balancing-disassoc-timer must be numeric, got: {value}")
    ...
    if 'bstm-disassociation-imminent' in payload:
        ...
        if value and value not in VALID_BODY_BSTM_DISASSOCIATION_IMMINENT:
            return (False, f"Invalid bstm-disassociation-imminent '{value}'. Must be one of: {', '.join(VALID_BODY_BSTM_DISASSOCIATION_IMMINENT)}")
    ...
    if 'beacon-advertising' in payload:
        ...
        if value and value not in VALID_BODY_BEACON_ADVERTISING:
            return (False, f"Invalid beacon-advertising '{value}'. Must be one of: {', '.join(VALID_BODY_BEACON_ADVERTISING)}")
    
    # Validate osen if present
    if 'osen' in payload:
        value = payload.get('osen')
        if value and value not in VALID_BODY_OSEN:
            return (False, f"Invalid osen '{value}'. Must be one of: {', '.join(VALID_BODY_OSEN)}")
    ...
    if 'application-detection-engine' in payload:
        ...
        if value and value not in VALID_BODY_APPLICATION_DETECTION_ENGINE:
            return (False, f"Invalid application-detection-engine '{value}'. Must be one of: {', '.join(VALID_BODY_APPLICATION_DETECTION_ENGINE)}")
    ...
    if 'application-dscp-marking' in payload:
        ...
        if value and value not in VALID_BODY_APPLICATION_DSCP_MARKING:
            return (False, f"Invalid application-dscp-marking '{value}'. Must be one of: {', '.join(VALID_BODY_APPLICATION_DSCP_MARKING)}")
    ...
    if 'application-report-intv' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 30 or int_val > 864000:
                    return (False, f"application-report-intv must be between 30 and 864000")
            except (ValueError, TypeError):
                return (False, f"application-report-intv must be numeric, got: {value}")
    ...
    if 'l3-roaming' in payload:
        ...
        if value and value not in VALID_BODY_L3_ROAMING:
            return (False, f"Invalid l3-roaming '{value}'. Must be one of: {', '.join(VALID_BODY_L3_ROAMING)}")
    ...
    if 'l3-roaming-mode' in payload:
        ...
        if value and value not in VALID_BODY_L3_ROAMING_MODE:
            return (False, f"Invalid l3-roaming-mode '{value}'. Must be one of: {', '.join(VALID_BODY_L3_ROAMING_MODE)}")
