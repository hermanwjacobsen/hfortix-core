...
def validate_global__put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 255:
                    return (False, f"acd-process-count must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"acd-process-count must be numeric, got: {value}")
    ...
    if 'wpad-process-count' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 255:
                    return (False, f"wpad-process-count must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"wpad-process-count must be numeric, got: {value}")
    ...
    if 'image-download' in payload:
        ...
        if value and value not in VALID_BODY_IMAGE_DOWNLOAD:
            return (False, f"Invalid image-download '{value}'. Must be one of: {', '.join(VALID_BODY_IMAGE_DOWNLOAD)}")
    ...
    if 'rolling-wtp-upgrade' in payload:
        ...
        if value and value not in VALID_BODY_ROLLING_WTP_UPGRADE:
            return (False, f"Invalid rolling-wtp-upgrade '{value}'. Must be one of: {', '.join(VALID_BODY_ROLLING_WTP_UPGRADE)}")
    ...
    if 'rolling-wtp-upgrade-threshold' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 7:
            return (False, f"rolling-wtp-upgrade-threshold cannot exceed 7 characters")
    ...
    if 'data-ethernet-II' in payload:
        ...
        if value and value not in VALID_BODY_DATA_ETHERNET_II:
            return (False, f"Invalid data-ethernet-II '{value}'. Must be one of: {', '.join(VALID_BODY_DATA_ETHERNET_II)}")
    ...
    if 'link-aggregation' in payload:
        ...
        if value and value not in VALID_BODY_LINK_AGGREGATION:
            return (False, f"Invalid link-aggregation '{value}'. Must be one of: {', '.join(VALID_BODY_LINK_AGGREGATION)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"mesh-eth-type must be between 0 and 65535")
            except (ValueError, TypeError):
    ...
    if 'fiapp-eth-type' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"fiapp-eth-type must be between 0 and 65535")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 31:
                    return (False, f"rogue-scan-mac-adjacency must be between 0 and 31")
            except (ValueError, TypeError):
                return (False, f"rogue-scan-mac-adjacency must be numeric, got: {value}")
    ...
    if 'wtp-share' in payload:
        ...
        if value and value not in VALID_BODY_WTP_SHARE:
            return (False, f"Invalid wtp-share '{value}'. Must be one of: {', '.join(VALID_BODY_WTP_SHARE)}")
    ...
    if 'tunnel-mode' in payload:
        ...
        if value and value not in VALID_BODY_TUNNEL_MODE:
            return (False, f"Invalid tunnel-mode '{value}'. Must be one of: {', '.join(VALID_BODY_TUNNEL_MODE)}")
    ...
    if 'ap-log-server' in payload:
        ...
        if value and value not in VALID_BODY_AP_LOG_SERVER:
            return (False, f"Invalid ap-log-server '{value}'. Must be one of: {', '.join(VALID_BODY_AP_LOG_SERVER)}")
    ...
    if 'ap-log-server-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"ap-log-server-port must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"ap-log-server-port must be numeric, got: {value}")
    ...
    if 'max-sta-offline' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-sta-offline must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"max-sta-offline must be numeric, got: {value}")
    ...
    if 'max-sta-offline-ip2mac' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-sta-offline-ip2mac must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"max-sta-offline-ip2mac must be numeric, got: {value}")
    ...
    if 'max-sta-cap' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-sta-cap must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'max-sta-cap-wtp' in payload:
        ...
        if value is not None:
            try:
                ...
                return (False, f"max-sta-cap-wtp must be numeric, got: {value}")
    ...
    if 'max-rogue-ap' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-rogue-ap must be between 0 and 4294967295")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 16:
                    return (False, f"max-rogue-ap-wtp must be between 1 and 16")
            except (ValueError, TypeError):
                return (False, f"max-rogue-ap-wtp must be numeric, got: {value}")
    ...
    if 'max-rogue-sta' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-rogue-sta must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'max-wids-entry' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-wids-entry must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'max-ble-device' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"max-ble-device must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'dfs-lab-test' in payload:
        ...
        if value and value not in VALID_BODY_DFS_LAB_TEST:
            return (False, f"Invalid dfs-lab-test '{value}'. Must be one of: {', '.join(VALID_BODY_DFS_LAB_TEST)}")
