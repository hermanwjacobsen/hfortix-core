...
def validate_wtp_group_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'platform-type' in payload:
        ...
        if value and value not in VALID_BODY_PLATFORM_TYPE:
            return (False, f"Invalid platform-type '{value}'. Must be one of: {', '.join(VALID_BODY_PLATFORM_TYPE)}")
...
def validate_wtp_group_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
