...
def validate_hotspot20_h2qp_wan_metric_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'link-status' in payload:
        ...
        if value and value not in VALID_BODY_LINK_STATUS:
            return (False, f"Invalid link-status '{value}'. Must be one of: {', '.join(VALID_BODY_LINK_STATUS)}")
    ...
    if 'symmetric-wan-link' in payload:
        ...
        if value and value not in VALID_BODY_SYMMETRIC_WAN_LINK:
            return (False, f"Invalid symmetric-wan-link '{value}'. Must be one of: {', '.join(VALID_BODY_SYMMETRIC_WAN_LINK)}")
    ...
    if 'link-at-capacity' in payload:
        ...
        if value and value not in VALID_BODY_LINK_AT_CAPACITY:
            return (False, f"Invalid link-at-capacity '{value}'. Must be one of: {', '.join(VALID_BODY_LINK_AT_CAPACITY)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"uplink-speed must be between 0 and 4294967295")
    ...
    if 'downlink-speed' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"downlink-speed must be between 0 and 4294967295")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"load-measurement-duration must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"load-measurement-duration must be numeric, got: {value}")
...
def validate_hotspot20_h2qp_wan_metric_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
