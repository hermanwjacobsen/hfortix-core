...
VALID_BODY_ACCESS_NETWORK_TYPE = ['private-network', 'private-network-with-guest-access', 'chargeable-public-network', 'free-public-network', 'personal-device-network', 'emergency-services-only-network', 'test-or-experimental', 'wildcard']
...
VALID_BODY_VENUE_GROUP = ['unspecified', 'assembly', 'business', 'educational', 'factory', 'institutional', 'mercantile', 'residential', 'storage', 'utility', 'vehicular', 'outdoor']
VALID_BODY_VENUE_TYPE = ['unspecified', 'arena', 'stadium', 'passenger-terminal', 'amphitheater', 'amusement-park', 'place-of-worship', 'convention-center', 'library', 'museum', 'restaurant', 'theater', 'bar', 'coffee-shop', 'zoo-or-aquarium', 'emergency-center', 'doctor-office', 'bank', 'fire-station', 'police-station', 'post-office', 'professional-office', 'research-facility', 'attorney-office', 'primary-school', 'secondary-school', 'university-or-college', 'factory', 'hospital', 'long-term-care-facility', 'rehab-center', 'group-home', 'prison-or-jail', 'retail-store', 'grocery-market', 'auto-service-station', 'shopping-mall', 'gas-station', 'private', 'hotel-or-motel', 'dormitory', 'boarding-house', 'automobile', 'airplane', 'bus', 'ferry', 'ship-or-boat', 'train', 'motor-bike', 'muni-mesh-network', 'city-park', 'rest-area', 'traffic-control', 'bus-stop', 'kiosk']
...
def validate_hotspot20_hs_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'access-network-type' in payload:
        ...
        if value and value not in VALID_BODY_ACCESS_NETWORK_TYPE:
            return (False, f"Invalid access-network-type '{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_NETWORK_TYPE)}")
    ...
    if 'access-network-internet' in payload:
        ...
        if value and value not in VALID_BODY_ACCESS_NETWORK_INTERNET:
            return (False, f"Invalid access-network-internet '{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_NETWORK_INTERNET)}")
    ...
    if 'access-network-asra' in payload:
        ...
        if value and value not in VALID_BODY_ACCESS_NETWORK_ASRA:
            return (False, f"Invalid access-network-asra '{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_NETWORK_ASRA)}")
    ...
    if 'access-network-esr' in payload:
        ...
        if value and value not in VALID_BODY_ACCESS_NETWORK_ESR:
            return (False, f"Invalid access-network-esr '{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_NETWORK_ESR)}")
    ...
    if 'access-network-uesa' in payload:
        ...
        if value and value not in VALID_BODY_ACCESS_NETWORK_UESA:
            return (False, f"Invalid access-network-uesa '{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_NETWORK_UESA)}")
    ...
    if 'venue-group' in payload:
        ...
        if value and value not in VALID_BODY_VENUE_GROUP:
            return (False, f"Invalid venue-group '{value}'. Must be one of: {', '.join(VALID_BODY_VENUE_GROUP)}")
    ...
    if 'venue-type' in payload:
        ...
        if value and value not in VALID_BODY_VENUE_TYPE:
            return (False, f"Invalid venue-type '{value}'. Must be one of: {', '.join(VALID_BODY_VENUE_TYPE)}")
    ...
    if 'proxy-arp' in payload:
        ...
        if value and value not in VALID_BODY_PROXY_ARP:
            return (False, f"Invalid proxy-arp '{value}'. Must be one of: {', '.join(VALID_BODY_PROXY_ARP)}")
    
    # Validate l2tif if present
    if 'l2tif' in payload:
        value = payload.get('l2tif')
        if value and value not in VALID_BODY_L2TIF:
            return (False, f"Invalid l2tif '{value}'. Must be one of: {', '.join(VALID_BODY_L2TIF)}")
    
    # Validate pame-bi if present
    if 'pame-bi' in payload:
        value = payload.get('pame-bi')
        if value and value not in VALID_BODY_PAME_BI:
            return (False, f"Invalid pame-bi '{value}'. Must be one of: {', '.join(VALID_BODY_PAME_BI)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"anqp-domain-id must be between 0 and 65535")
            except (ValueError, TypeError):
    ...
    if 'dgaf' in payload:
        ...
        if value and value not in VALID_BODY_DGAF:
            return (False, f"Invalid dgaf '{value}'. Must be one of: {', '.join(VALID_BODY_DGAF)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 30 or int_val > 120:
                    return (False, f"deauth-request-timeout must be between 30 and 120")
            except (ValueError, TypeError):
                return (False, f"deauth-request-timeout must be numeric, got: {value}")
    ...
    if 'wnm-sleep-mode' in payload:
        ...
        if value and value not in VALID_BODY_WNM_SLEEP_MODE:
            return (False, f"Invalid wnm-sleep-mode '{value}'. Must be one of: {', '.join(VALID_BODY_WNM_SLEEP_MODE)}")
    ...
    if 'bss-transition' in payload:
        ...
        if value and value not in VALID_BODY_BSS_TRANSITION:
            return (False, f"Invalid bss-transition '{value}'. Must be one of: {', '.join(VALID_BODY_BSS_TRANSITION)}")
    ...
    if 'wba-open-roaming' in payload:
        ...
        if value and value not in VALID_BODY_WBA_OPEN_ROAMING:
            return (False, f"Invalid wba-open-roaming '{value}'. Must be one of: {', '.join(VALID_BODY_WBA_OPEN_ROAMING)}")
    ...
    if 'wba-financial-clearing-provider' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 127:
            return (False, f"wba-financial-clearing-provider cannot exceed 127 characters")
    ...
    if 'wba-data-clearing-provider' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 127:
            return (False, f"wba-data-clearing-provider cannot exceed 127 characters")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"wba-charging-rate must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"wba-charging-rate must be numeric, got: {value}")
...
def validate_hotspot20_hs_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
