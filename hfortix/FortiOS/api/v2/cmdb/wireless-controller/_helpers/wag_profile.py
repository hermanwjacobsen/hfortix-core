...
def validate_wag_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'tunnel-type' in payload:
        ...
        if value and value not in VALID_BODY_TUNNEL_TYPE:
            return (False, f"Invalid tunnel-type '{value}'. Must be one of: {', '.join(VALID_BODY_TUNNEL_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"ping-interval must be between 1 and 65535")
            except (ValueError, TypeError):
    ...
    if 'return-packet-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"return-packet-timeout must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"return-packet-timeout must be numeric, got: {value}")
...
def validate_wag_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
