...
def validate_forward_server_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'healthcheck' in payload:
        ...
        if value and value not in VALID_BODY_HEALTHCHECK:
            return (False, f"Invalid healthcheck '{value}'. Must be one of: {', '.join(VALID_BODY_HEALTHCHECK)}")
    ...
    if 'server-down-option' in payload:
        ...
        if value and value not in VALID_BODY_SERVER_DOWN_OPTION:
            return (False, f"Invalid server-down-option '{value}'. Must be one of: {', '.join(VALID_BODY_SERVER_DOWN_OPTION)}")
...
def validate_forward_server_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
