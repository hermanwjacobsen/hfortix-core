...
def validate_fast_fallback_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'connection-mode' in payload:
        ...
        if value and value not in VALID_BODY_CONNECTION_MODE:
            return (False, f"Invalid connection-mode '{value}'. Must be one of: {', '.join(VALID_BODY_CONNECTION_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 200 or int_val > 1800000:
                    return (False, f"connection-timeout must be between 200 and 1800000")
            except (ValueError, TypeError):
                return (False, f"connection-timeout must be numeric, got: {value}")
...
def validate_fast_fallback_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
