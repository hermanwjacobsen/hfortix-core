...
def validate_forward_server_group_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'affinity' in payload:
        ...
        if value and value not in VALID_BODY_AFFINITY:
            return (False, f"Invalid affinity '{value}'. Must be one of: {', '.join(VALID_BODY_AFFINITY)}")
    ...
    if 'group-down-option' in payload:
        ...
        if value and value not in VALID_BODY_GROUP_DOWN_OPTION:
            return (False, f"Invalid group-down-option '{value}'. Must be one of: {', '.join(VALID_BODY_GROUP_DOWN_OPTION)}")
...
def validate_forward_server_group_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
