...
VALID_BODY_LEARN_CLIENT_IP_FROM_HEADER = ['true-client-ip', 'x-real-ip', 'x-forwarded-for']
...
def validate_global__put(
    ...
    if 'fast-policy-match' in payload:
        ...
        if value and value not in VALID_BODY_FAST_POLICY_MATCH:
            return (False, f"Invalid fast-policy-match '{value}'. Must be one of: {', '.join(VALID_BODY_FAST_POLICY_MATCH)}")
    ...
    if 'ldap-user-cache' in payload:
        ...
        if value and value not in VALID_BODY_LDAP_USER_CACHE:
            return (False, f"Invalid ldap-user-cache '{value}'. Must be one of: {', '.join(VALID_BODY_LDAP_USER_CACHE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 64:
                    return (False, f"max-request-length must be between 2 and 64")
            except (ValueError, TypeError):
                return (False, f"max-request-length must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 16 or int_val > 256:
                    return (False, f"max-message-length must be between 16 and 256")
            except (ValueError, TypeError):
                return (False, f"max-message-length must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 65535 or int_val > 2147483647:
                    return (False, f"http2-client-window-size must be between 65535 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"http2-client-window-size must be numeric, got: {value}")
    ...
    if 'http2-server-window-size' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 65535 or int_val > 2147483647:
                    return (False, f"http2-server-window-size must be between 65535 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"http2-server-window-size must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 30 or int_val > 3600:
                    return (False, f"auth-sign-timeout must be between 30 and 3600")
            except (ValueError, TypeError):
                return (False, f"auth-sign-timeout must be numeric, got: {value}")
    ...
    if 'strict-web-check' in payload:
        ...
        if value and value not in VALID_BODY_STRICT_WEB_CHECK:
            return (False, f"Invalid strict-web-check '{value}'. Must be one of: {', '.join(VALID_BODY_STRICT_WEB_CHECK)}")
    ...
    if 'forward-proxy-auth' in payload:
        ...
        if value and value not in VALID_BODY_FORWARD_PROXY_AUTH:
            return (False, f"Invalid forward-proxy-auth '{value}'. Must be one of: {', '.join(VALID_BODY_FORWARD_PROXY_AUTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 6 or int_val > 60:
                    return (False, f"forward-server-affinity-timeout must be between 6 and 60")
            except (ValueError, TypeError):
                return (False, f"forward-server-affinity-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 1024:
                    return (False, f"max-waf-body-cache-length must be between 1 and 1024")
            except (ValueError, TypeError):
                return (False, f"max-waf-body-cache-length must be numeric, got: {value}")
    ...
    if 'learn-client-ip' in payload:
        ...
        if value and value not in VALID_BODY_LEARN_CLIENT_IP:
            return (False, f"Invalid learn-client-ip '{value}'. Must be one of: {', '.join(VALID_BODY_LEARN_CLIENT_IP)}")
    ...
    if 'always-learn-client-ip' in payload:
        ...
        if value and value not in VALID_BODY_ALWAYS_LEARN_CLIENT_IP:
            return (False, f"Invalid always-learn-client-ip '{value}'. Must be one of: {', '.join(VALID_BODY_ALWAYS_LEARN_CLIENT_IP)}")
    ...
    if 'learn-client-ip-from-header' in payload:
        ...
        if value and value not in VALID_BODY_LEARN_CLIENT_IP_FROM_HEADER:
            return (False, f"Invalid learn-client-ip-from-header '{value}'. Must be one of: {', '.join(VALID_BODY_LEARN_CLIENT_IP_FROM_HEADER)}")
    ...
    if 'policy-partial-match' in payload:
        ...
        if value and value not in VALID_BODY_POLICY_PARTIAL_MATCH:
            return (False, f"Invalid policy-partial-match '{value}'. Must be one of: {', '.join(VALID_BODY_POLICY_PARTIAL_MATCH)}")
    ...
    if 'log-policy-pending' in payload:
        ...
        if value and value not in VALID_BODY_LOG_POLICY_PENDING:
            return (False, f"Invalid log-policy-pending '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_POLICY_PENDING)}")
    ...
    if 'log-forward-server' in payload:
        ...
        if value and value not in VALID_BODY_LOG_FORWARD_SERVER:
            return (False, f"Invalid log-forward-server '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_FORWARD_SERVER)}")
    ...
    if 'log-app-id' in payload:
        ...
        if value and value not in VALID_BODY_LOG_APP_ID:
            return (False, f"Invalid log-app-id '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_APP_ID)}")
    ...
    if 'proxy-transparent-cert-inspection' in payload:
        ...
        if value and value not in VALID_BODY_PROXY_TRANSPARENT_CERT_INSPECTION:
            return (False, f"Invalid proxy-transparent-cert-inspection '{value}'. Must be one of: {', '.join(VALID_BODY_PROXY_TRANSPARENT_CERT_INSPECTION)}")
    ...
    if 'request-obs-fold' in payload:
        ...
        if value and value not in VALID_BODY_REQUEST_OBS_FOLD:
            return (False, f"Invalid request-obs-fold '{value}'. Must be one of: {', '.join(VALID_BODY_REQUEST_OBS_FOLD)}")
