...
def validate_url_match_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'cache-exemption' in payload:
        ...
        if value and value not in VALID_BODY_CACHE_EXEMPTION:
            return (False, f"Invalid cache-exemption '{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_EXEMPTION)}")
...
def validate_url_match_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
