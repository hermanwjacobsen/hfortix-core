...
VALID_BODY_SAFESEARCH = ['disable', 'url', 'header', 'translate', 'yt-pattern', 'yt-scan', 'yt-video', 'yt-channel']
...
def validate_search_engine_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'safesearch' in payload:
        ...
        if value and value not in VALID_BODY_SAFESEARCH:
            return (False, f"Invalid safesearch '{value}'. Must be one of: {', '.join(VALID_BODY_SAFESEARCH)}")
    
    # Validate charset if present
    if 'charset' in payload:
        value = payload.get('charset')
        if value and value not in VALID_BODY_CHARSET:
            return (False, f"Invalid charset '{value}'. Must be one of: {', '.join(VALID_BODY_CHARSET)}")
...
def validate_search_engine_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
