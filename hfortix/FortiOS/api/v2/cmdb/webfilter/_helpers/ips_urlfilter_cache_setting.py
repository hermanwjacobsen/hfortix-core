        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2147483:
                    return (False, f"dns-retry-interval must be between 0 and 2147483")
            except (ValueError, TypeError):
                return (False, f"dns-retry-interval must be numeric, got: {value}")
    ...
    if 'extended-ttl' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2147483:
                    return (False, f"extended-ttl must be between 0 and 2147483")
            except (ValueError, TypeError):
