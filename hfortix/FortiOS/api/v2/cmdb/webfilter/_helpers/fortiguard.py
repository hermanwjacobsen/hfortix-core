...
def validate_fortiguard_put(
    ...
    if 'cache-mode' in payload:
        ...
        if value and value not in VALID_BODY_CACHE_MODE:
            return (False, f"Invalid cache-mode '{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_MODE)}")
    ...
    if 'cache-prefix-match' in payload:
        ...
        if value and value not in VALID_BODY_CACHE_PREFIX_MATCH:
            return (False, f"Invalid cache-prefix-match '{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_PREFIX_MATCH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 150:
                    return (False, f"cache-mem-permille must be between 1 and 150")
            except (ValueError, TypeError):
                return (False, f"cache-mem-permille must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"ovrd-auth-port-http must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"ovrd-auth-port-http must be numeric, got: {value}")
    ...
    if 'ovrd-auth-port-https' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"ovrd-auth-port-https must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"ovrd-auth-port-https must be numeric, got: {value}")
    ...
    if 'ovrd-auth-port-https-flow' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"ovrd-auth-port-https-flow must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"ovrd-auth-port-https-flow must be numeric, got: {value}")
    ...
    if 'ovrd-auth-port-warning' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"ovrd-auth-port-warning must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"ovrd-auth-port-warning must be numeric, got: {value}")
    ...
    if 'ovrd-auth-https' in payload:
        ...
        if value and value not in VALID_BODY_OVRD_AUTH_HTTPS:
            return (False, f"Invalid ovrd-auth-https '{value}'. Must be one of: {', '.join(VALID_BODY_OVRD_AUTH_HTTPS)}")
    ...
    if 'warn-auth-https' in payload:
        ...
        if value and value not in VALID_BODY_WARN_AUTH_HTTPS:
            return (False, f"Invalid warn-auth-https '{value}'. Must be one of: {', '.join(VALID_BODY_WARN_AUTH_HTTPS)}")
    ...
    if 'close-ports' in payload:
        ...
        if value and value not in VALID_BODY_CLOSE_PORTS:
            return (False, f"Invalid close-ports '{value}'. Must be one of: {', '.join(VALID_BODY_CLOSE_PORTS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 576 or int_val > 10000:
                    return (False, f"request-packet-size-limit must be between 576 and 10000")
            except (ValueError, TypeError):
                return (False, f"request-packet-size-limit must be numeric, got: {value}")
    ...
    if 'embed-image' in payload:
        ...
        if value and value not in VALID_BODY_EMBED_IMAGE:
            return (False, f"Invalid embed-image '{value}'. Must be one of: {', '.join(VALID_BODY_EMBED_IMAGE)}")
