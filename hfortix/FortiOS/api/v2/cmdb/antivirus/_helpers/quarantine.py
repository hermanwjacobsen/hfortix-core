...
VALID_BODY_DROP_INFECTED = ['imap', 'smtp', 'pop3', 'http', 'ftp', 'nntp', 'imaps', 'smtps', 'pop3s', 'https', 'ftps', 'mapi', 'cifs', 'ssh']
VALID_BODY_STORE_INFECTED = ['imap', 'smtp', 'pop3', 'http', 'ftp', 'nntp', 'imaps', 'smtps', 'pop3s', 'https', 'ftps', 'mapi', 'cifs', 'ssh']
VALID_BODY_DROP_MACHINE_LEARNING = ['imap', 'smtp', 'pop3', 'http', 'ftp', 'nntp', 'imaps', 'smtps', 'pop3s', 'https', 'ftps', 'mapi', 'cifs', 'ssh']
VALID_BODY_STORE_MACHINE_LEARNING = ['imap', 'smtp', 'pop3', 'http', 'ftp', 'nntp', 'imaps', 'smtps', 'pop3s', 'https', 'ftps', 'mapi', 'cifs', 'ssh']
...
def validate_quarantine_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"quarantine-quota must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"quarantine-quota must be numeric, got: {value}")
    ...
    if 'drop-infected' in payload:
        ...
        if value and value not in VALID_BODY_DROP_INFECTED:
            return (False, f"Invalid drop-infected '{value}'. Must be one of: {', '.join(VALID_BODY_DROP_INFECTED)}")
    ...
    if 'store-infected' in payload:
        ...
        if value and value not in VALID_BODY_STORE_INFECTED:
            return (False, f"Invalid store-infected '{value}'. Must be one of: {', '.join(VALID_BODY_STORE_INFECTED)}")
    ...
    if 'drop-machine-learning' in payload:
        ...
        if value and value not in VALID_BODY_DROP_MACHINE_LEARNING:
            return (False, f"Invalid drop-machine-learning '{value}'. Must be one of: {', '.join(VALID_BODY_DROP_MACHINE_LEARNING)}")
    ...
    if 'store-machine-learning' in payload:
        ...
        if value and value not in VALID_BODY_STORE_MACHINE_LEARNING:
            return (False, f"Invalid store-machine-learning '{value}'. Must be one of: {', '.join(VALID_BODY_STORE_MACHINE_LEARNING)}")
    ...
    if 'lowspace' in payload:
        ...
        if value and value not in VALID_BODY_LOWSPACE:
            return (False, f"Invalid lowspace '{value}'. Must be one of: {', '.join(VALID_BODY_LOWSPACE)}")
    
    # Validate destination if present
    if 'destination' in payload:
        value = payload.get('destination')
        if value and value not in VALID_BODY_DESTINATION:
            return (False, f"Invalid destination '{value}'. Must be one of: {', '.join(VALID_BODY_DESTINATION)}")
