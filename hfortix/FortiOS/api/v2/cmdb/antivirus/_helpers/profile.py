...
def validate_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'fortisandbox-mode' in payload:
        ...
        if value and value not in VALID_BODY_FORTISANDBOX_MODE:
            return (False, f"Invalid fortisandbox-mode '{value}'. Must be one of: {', '.join(VALID_BODY_FORTISANDBOX_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 193:
                    return (False, f"fortisandbox-max-upload must be between 1 and 193")
            except (ValueError, TypeError):
                return (False, f"fortisandbox-max-upload must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"analytics-ignore-filetype must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"analytics-ignore-filetype must be numeric, got: {value}")
    ...
    if 'analytics-accept-filetype' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"analytics-accept-filetype must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"analytics-accept-filetype must be numeric, got: {value}")
    ...
    if 'analytics-db' in payload:
        ...
        if value and value not in VALID_BODY_ANALYTICS_DB:
            return (False, f"Invalid analytics-db '{value}'. Must be one of: {', '.join(VALID_BODY_ANALYTICS_DB)}")
    ...
    if 'mobile-malware-db' in payload:
        ...
        if value and value not in VALID_BODY_MOBILE_MALWARE_DB:
            return (False, f"Invalid mobile-malware-db '{value}'. Must be one of: {', '.join(VALID_BODY_MOBILE_MALWARE_DB)}")
    ...
    if 'outbreak-prevention-archive-scan' in payload:
        ...
        if value and value not in VALID_BODY_OUTBREAK_PREVENTION_ARCHIVE_SCAN:
            return (False, f"Invalid outbreak-prevention-archive-scan '{value}'. Must be one of: {', '.join(VALID_BODY_OUTBREAK_PREVENTION_ARCHIVE_SCAN)}")
    ...
    if 'external-blocklist-enable-all' in payload:
        ...
        if value and value not in VALID_BODY_EXTERNAL_BLOCKLIST_ENABLE_ALL:
            return (False, f"Invalid external-blocklist-enable-all '{value}'. Must be one of: {', '.join(VALID_BODY_EXTERNAL_BLOCKLIST_ENABLE_ALL)}")
    ...
    if 'ems-threat-feed' in payload:
        ...
        if value and value not in VALID_BODY_EMS_THREAT_FEED:
            return (False, f"Invalid ems-threat-feed '{value}'. Must be one of: {', '.join(VALID_BODY_EMS_THREAT_FEED)}")
    ...
    if 'av-virus-log' in payload:
        ...
        if value and value not in VALID_BODY_AV_VIRUS_LOG:
            return (False, f"Invalid av-virus-log '{value}'. Must be one of: {', '.join(VALID_BODY_AV_VIRUS_LOG)}")
    ...
    if 'scan-mode' in payload:
        ...
        if value and value not in VALID_BODY_SCAN_MODE:
            return (False, f"Invalid scan-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SCAN_MODE)}")
