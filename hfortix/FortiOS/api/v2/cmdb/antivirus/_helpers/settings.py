...
def validate_settings_put(
    ...
    if 'use-extreme-db' in payload:
        ...
        if value and value not in VALID_BODY_USE_EXTREME_DB:
            return (False, f"Invalid use-extreme-db '{value}'. Must be one of: {', '.join(VALID_BODY_USE_EXTREME_DB)}")
    ...
    if 'grayware' in payload:
        ...
        if value and value not in VALID_BODY_GRAYWARE:
            return (False, f"Invalid grayware '{value}'. Must be one of: {', '.join(VALID_BODY_GRAYWARE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 30 or int_val > 3600:
                    return (False, f"override-timeout must be between 30 and 3600")
            except (ValueError, TypeError):
                return (False, f"override-timeout must be numeric, got: {value}")
    ...
    if 'cache-infected-result' in payload:
        ...
        if value and value not in VALID_BODY_CACHE_INFECTED_RESULT:
            return (False, f"Invalid cache-infected-result '{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_INFECTED_RESULT)}")
