...
VALID_BODY_DHCP_OPTION82_CIRCUIT_ID = ['intfname', 'vlan', 'hostname', 'mode', 'description']
...
VALID_BODY_UPDATE_USER_DEVICE = ['mac-cache', 'lldp', 'dhcp-snooping', 'l2-db', 'l3-db']
...
def validate_global__put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 1000000:
                    return (False, f"mac-aging-interval must be between 10 and 1000000")
            except (ValueError, TypeError):
                return (False, f"mac-aging-interval must be numeric, got: {value}")
    ...
    if 'https-image-push' in payload:
        ...
        if value and value not in VALID_BODY_HTTPS_IMAGE_PUSH:
            return (False, f"Invalid https-image-push '{value}'. Must be one of: {', '.join(VALID_BODY_HTTPS_IMAGE_PUSH)}")
    ...
    if 'vlan-all-mode' in payload:
        ...
        if value and value not in VALID_BODY_VLAN_ALL_MODE:
            return (False, f"Invalid vlan-all-mode '{value}'. Must be one of: {', '.join(VALID_BODY_VLAN_ALL_MODE)}")
    ...
    if 'vlan-optimization' in payload:
        ...
        if value and value not in VALID_BODY_VLAN_OPTIMIZATION:
            return (False, f"Invalid vlan-optimization '{value}'. Must be one of: {', '.join(VALID_BODY_VLAN_OPTIMIZATION)}")
    ...
    if 'vlan-identity' in payload:
        ...
        if value and value not in VALID_BODY_VLAN_IDENTITY:
            return (False, f"Invalid vlan-identity '{value}'. Must be one of: {', '.join(VALID_BODY_VLAN_IDENTITY)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 168:
                    return (False, f"mac-retention-period must be between 0 and 168")
            except (ValueError, TypeError):
                return (False, f"mac-retention-period must be numeric, got: {value}")
    ...
    if 'default-virtual-switch-vlan' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 15:
            return (False, f"default-virtual-switch-vlan cannot exceed 15 characters")
    ...
    if 'dhcp-option82-format' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION82_FORMAT:
            return (False, f"Invalid dhcp-option82-format '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION82_FORMAT)}")
    ...
    if 'dhcp-option82-circuit-id' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION82_CIRCUIT_ID:
            return (False, f"Invalid dhcp-option82-circuit-id '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION82_CIRCUIT_ID)}")
    ...
    if 'dhcp-option82-remote-id' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_OPTION82_REMOTE_ID:
            return (False, f"Invalid dhcp-option82-remote-id '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_OPTION82_REMOTE_ID)}")
    ...
    if 'dhcp-snoop-client-req' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_SNOOP_CLIENT_REQ:
            return (False, f"Invalid dhcp-snoop-client-req '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_SNOOP_CLIENT_REQ)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 300 or int_val > 259200:
                    return (False, f"dhcp-snoop-client-db-exp must be between 300 and 259200")
            except (ValueError, TypeError):
                return (False, f"dhcp-snoop-client-db-exp must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2048:
                    return (False, f"dhcp-snoop-db-per-port-learn-limit must be between 0 and 2048")
            except (ValueError, TypeError):
                return (False, f"dhcp-snoop-db-per-port-learn-limit must be numeric, got: {value}")
    ...
    if 'log-mac-limit-violations' in payload:
        ...
        if value and value not in VALID_BODY_LOG_MAC_LIMIT_VIOLATIONS:
            return (False, f"Invalid log-mac-limit-violations '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_MAC_LIMIT_VIOLATIONS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"mac-violation-timer must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"mac-violation-timer must be numeric, got: {value}")
    ...
    if 'sn-dns-resolution' in payload:
        ...
        if value and value not in VALID_BODY_SN_DNS_RESOLUTION:
            return (False, f"Invalid sn-dns-resolution '{value}'. Must be one of: {', '.join(VALID_BODY_SN_DNS_RESOLUTION)}")
    ...
    if 'mac-event-logging' in payload:
        ...
        if value and value not in VALID_BODY_MAC_EVENT_LOGGING:
            return (False, f"Invalid mac-event-logging '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_EVENT_LOGGING)}")
    ...
    if 'bounce-quarantined-link' in payload:
        ...
        if value and value not in VALID_BODY_BOUNCE_QUARANTINED_LINK:
            return (False, f"Invalid bounce-quarantined-link '{value}'. Must be one of: {', '.join(VALID_BODY_BOUNCE_QUARANTINED_LINK)}")
    ...
    if 'quarantine-mode' in payload:
        ...
        if value and value not in VALID_BODY_QUARANTINE_MODE:
            return (False, f"Invalid quarantine-mode '{value}'. Must be one of: {', '.join(VALID_BODY_QUARANTINE_MODE)}")
    ...
    if 'update-user-device' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_USER_DEVICE:
            return (False, f"Invalid update-user-device '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_USER_DEVICE)}")
    ...
    if 'fips-enforce' in payload:
        ...
        if value and value not in VALID_BODY_FIPS_ENFORCE:
            return (False, f"Invalid fips-enforce '{value}'. Must be one of: {', '.join(VALID_BODY_FIPS_ENFORCE)}")
    ...
    if 'firmware-provision-on-authorization' in payload:
        ...
        if value and value not in VALID_BODY_FIRMWARE_PROVISION_ON_AUTHORIZATION:
            return (False, f"Invalid firmware-provision-on-authorization '{value}'. Must be one of: {', '.join(VALID_BODY_FIRMWARE_PROVISION_ON_AUTHORIZATION)}")
    ...
    if 'switch-on-deauth' in payload:
        ...
        if value and value not in VALID_BODY_SWITCH_ON_DEAUTH:
            return (False, f"Invalid switch-on-deauth '{value}'. Must be one of: {', '.join(VALID_BODY_SWITCH_ON_DEAUTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 1440:
                    return (False, f"firewall-auth-user-hold-period must be between 5 and 1440")
            except (ValueError, TypeError):
                return (False, f"firewall-auth-user-hold-period must be numeric, got: {value}")
