...
def validate_storm_control_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'storm-control-mode' in payload:
        ...
        if value and value not in VALID_BODY_STORM_CONTROL_MODE:
            return (False, f"Invalid storm-control-mode '{value}'. Must be one of: {', '.join(VALID_BODY_STORM_CONTROL_MODE)}")
    ...
    if 'burst-size-level' in payload:
        ...
        if value is not None:
            try:
                ...
                return (False, f"burst-size-level must be numeric, got: {value}")
    ...
    if 'unknown-unicast' in payload:
        ...
        if value and value not in VALID_BODY_UNKNOWN_UNICAST:
            return (False, f"Invalid unknown-unicast '{value}'. Must be one of: {', '.join(VALID_BODY_UNKNOWN_UNICAST)}")
    ...
    if 'unknown-multicast' in payload:
        ...
        if value and value not in VALID_BODY_UNKNOWN_MULTICAST:
            return (False, f"Invalid unknown-multicast '{value}'. Must be one of: {', '.join(VALID_BODY_UNKNOWN_MULTICAST)}")
    ...
    if 'broadcast' in payload:
        ...
        if value and value not in VALID_BODY_BROADCAST:
            return (False, f"Invalid broadcast '{value}'. Must be one of: {', '.join(VALID_BODY_BROADCAST)}")
...
def validate_storm_control_policy_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
