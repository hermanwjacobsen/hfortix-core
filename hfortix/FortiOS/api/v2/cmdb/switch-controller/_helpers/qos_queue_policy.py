...
def validate_qos_queue_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'rate-by' in payload:
        ...
        if value and value not in VALID_BODY_RATE_BY:
            return (False, f"Invalid rate-by '{value}'. Must be one of: {', '.join(VALID_BODY_RATE_BY)}")
...
def validate_qos_queue_policy_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
