...
VALID_BODY_PDELAY_REQ_INTERVAL = ['1sec', '2sec', '4sec', '8sec', '16sec', '32sec']
...
def validate_ptp_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'ptp-profile' in payload:
        ...
        if value and value not in VALID_BODY_PTP_PROFILE:
            return (False, f"Invalid ptp-profile '{value}'. Must be one of: {', '.join(VALID_BODY_PTP_PROFILE)}")
    ...
    if 'pdelay-req-interval' in payload:
        ...
        if value and value not in VALID_BODY_PDELAY_REQ_INTERVAL:
            return (False, f"Invalid pdelay-req-interval '{value}'. Must be one of: {', '.join(VALID_BODY_PDELAY_REQ_INTERVAL)}")
...
def validate_ptp_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
