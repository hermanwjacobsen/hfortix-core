...
def validate_mac_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'count' in payload:
        ...
        if value and value not in VALID_BODY_COUNT:
            return (False, f"Invalid count '{value}'. Must be one of: {', '.join(VALID_BODY_COUNT)}")
    ...
    if 'bounce-port-link' in payload:
        ...
        if value and value not in VALID_BODY_BOUNCE_PORT_LINK:
            return (False, f"Invalid bounce-port-link '{value}'. Must be one of: {', '.join(VALID_BODY_BOUNCE_PORT_LINK)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"bounce-port-duration must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"bounce-port-duration must be numeric, got: {value}")
    ...
    if 'poe-reset' in payload:
        ...
        if value and value not in VALID_BODY_POE_RESET:
            return (False, f"Invalid poe-reset '{value}'. Must be one of: {', '.join(VALID_BODY_POE_RESET)}")
...
def validate_mac_policy_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
