...
def validate_ip_source_guard_log_put(
    ...
    if 'log-violations' in payload:
        ...
        if value and value not in VALID_BODY_LOG_VIOLATIONS:
            return (False, f"Invalid log-violations '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_VIOLATIONS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 1500:
                    return (False, f"violation-timer must be between 0 and 1500")
            except (ValueError, TypeError):
                return (False, f"violation-timer must be numeric, got: {value}")
