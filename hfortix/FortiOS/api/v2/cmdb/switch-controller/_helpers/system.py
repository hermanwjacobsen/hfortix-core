...
def validate_system_put(
    ...
    if 'parallel-process-override' in payload:
        ...
        if value and value not in VALID_BODY_PARALLEL_PROCESS_OVERRIDE:
            return (False, f"Invalid parallel-process-override '{value}'. Must be one of: {', '.join(VALID_BODY_PARALLEL_PROCESS_OVERRIDE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 24:
                    return (False, f"parallel-process must be between 1 and 24")
            except (ValueError, TypeError):
                return (False, f"parallel-process must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 30 or int_val > 1800:
                    return (False, f"data-sync-interval must be between 30 and 1800")
            except (ValueError, TypeError):
                return (False, f"data-sync-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 255:
                    return (False, f"iot-weight-threshold must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"iot-weight-threshold must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 10080:
                    return (False, f"iot-scan-interval must be between 2 and 10080")
            except (ValueError, TypeError):
                return (False, f"iot-scan-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 180:
                    return (False, f"nac-periodic-interval must be between 5 and 180")
            except (ValueError, TypeError):
                return (False, f"nac-periodic-interval must be numeric, got: {value}")
    ...
    if 'dynamic-periodic-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 5 or int_val > 180:
                    return (False, f"dynamic-periodic-interval must be between 5 and 180")
            except (ValueError, TypeError):
                return (False, f"dynamic-periodic-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 8 or int_val > 600:
                    return (False, f"caputp-echo-interval must be between 8 and 600")
            except (ValueError, TypeError):
                return (False, f"caputp-echo-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 64:
                    return (False, f"caputp-max-retransmit must be between 0 and 64")
            except (ValueError, TypeError):
                return (False, f"caputp-max-retransmit must be numeric, got: {value}")
