...
def validate_igmp_snooping_put(
    ...
    if 'flood-unknown-multicast' in payload:
        ...
        if value and value not in VALID_BODY_FLOOD_UNKNOWN_MULTICAST:
            return (False, f"Invalid flood-unknown-multicast '{value}'. Must be one of: {', '.join(VALID_BODY_FLOOD_UNKNOWN_MULTICAST)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 1200:
                    return (False, f"query-interval must be between 10 and 1200")
            except (ValueError, TypeError):
