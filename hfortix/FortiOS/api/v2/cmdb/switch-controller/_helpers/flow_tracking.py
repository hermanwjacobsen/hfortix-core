...
def validate_flow_tracking_put(
    ...
    if 'sample-mode' in payload:
        ...
        if value and value not in VALID_BODY_SAMPLE_MODE:
            return (False, f"Invalid sample-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SAMPLE_MODE)}")
    ...
    if 'format' in payload:
        ...
        if value and value not in VALID_BODY_FORMAT:
            return (False, f"Invalid format '{value}'. Must be one of: {', '.join(VALID_BODY_FORMAT)}")
    
    # Validate level if present
    if 'level' in payload:
        value = payload.get('level')
        if value and value not in VALID_BODY_LEVEL:
            return (False, f"Invalid level '{value}'. Must be one of: {', '.join(VALID_BODY_LEVEL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 512 or int_val > 9216:
                    return (False, f"max-export-pkt-size must be between 512 and 9216")
            except (ValueError, TypeError):
                return (False, f"max-export-pkt-size must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 60:
                    return (False, f"template-export-period must be between 1 and 60")
            except (ValueError, TypeError):
                return (False, f"template-export-period must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-general must be between 60 and 604800")
            except (ValueError, TypeError):
                return (False, f"timeout-general must be numeric, got: {value}")
    ...
    if 'timeout-icmp' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-icmp must be between 60 and 604800")
            except (ValueError, TypeError):
    ...
    if 'timeout-max' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-max must be between 60 and 604800")
            except (ValueError, TypeError):
    ...
    if 'timeout-tcp' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-tcp must be between 60 and 604800")
            except (ValueError, TypeError):
    ...
    if 'timeout-tcp-fin' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-tcp-fin must be between 60 and 604800")
            except (ValueError, TypeError):
                return (False, f"timeout-tcp-fin must be numeric, got: {value}")
    ...
    if 'timeout-tcp-rst' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-tcp-rst must be between 60 and 604800")
            except (ValueError, TypeError):
                return (False, f"timeout-tcp-rst must be numeric, got: {value}")
    ...
    if 'timeout-udp' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 604800:
                    return (False, f"timeout-udp must be between 60 and 604800")
            except (ValueError, TypeError):
