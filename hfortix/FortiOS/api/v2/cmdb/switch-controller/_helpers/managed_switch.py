...
def validate_managed_switch_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'fsw-wan1-admin' in payload:
        ...
        if value and value not in VALID_BODY_FSW_WAN1_ADMIN:
            return (False, f"Invalid fsw-wan1-admin '{value}'. Must be one of: {', '.join(VALID_BODY_FSW_WAN1_ADMIN)}")
    ...
    if 'poe-pre-standard-detection' in payload:
        ...
        if value and value not in VALID_BODY_POE_PRE_STANDARD_DETECTION:
            return (False, f"Invalid poe-pre-standard-detection '{value}'. Must be one of: {', '.join(VALID_BODY_POE_PRE_STANDARD_DETECTION)}")
    ...
    if 'dhcp-server-access-list' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_SERVER_ACCESS_LIST:
            return (False, f"Invalid dhcp-server-access-list '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_SERVER_ACCESS_LIST)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 255:
                    return (False, f"poe-detection-type must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"poe-detection-type must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"max-poe-budget must be between 0 and 65535")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 1:
                    return (False, f"directly-connected must be between 0 and 1")
            except (ValueError, TypeError):
                return (False, f"directly-connected must be numeric, got: {value}")
    ...
    if 'max-allowed-trunk-members' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 255:
                    return (False, f"max-allowed-trunk-members must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"max-allowed-trunk-members must be numeric, got: {value}")
    ...
    if 'pre-provisioned' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 255:
                    return (False, f"pre-provisioned must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"pre-provisioned must be numeric, got: {value}")
    ...
    if 'tunnel-discovered' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 1:
                    return (False, f"tunnel-discovered must be between 0 and 1")
            except (ValueError, TypeError):
                return (False, f"tunnel-discovered must be numeric, got: {value}")
    ...
    if 'switch-dhcp_opt43_key' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 63:
            return (False, f"switch-dhcp_opt43_key cannot exceed 63 characters")
    ...
    if 'mclag-igmp-snooping-aware' in payload:
        ...
        if value and value not in VALID_BODY_MCLAG_IGMP_SNOOPING_AWARE:
            return (False, f"Invalid mclag-igmp-snooping-aware '{value}'. Must be one of: {', '.join(VALID_BODY_MCLAG_IGMP_SNOOPING_AWARE)}")
    ...
    if 'dynamically-discovered' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 1:
                    return (False, f"dynamically-discovered must be between 0 and 1")
            except (ValueError, TypeError):
                return (False, f"dynamically-discovered must be numeric, got: {value}")
    ...
    if 'ptp-status' in payload:
        ...
        if value and value not in VALID_BODY_PTP_STATUS:
            return (False, f"Invalid ptp-status '{value}'. Must be one of: {', '.join(VALID_BODY_PTP_STATUS)}")
    ...
    if 'radius-nas-ip-override' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_NAS_IP_OVERRIDE:
            return (False, f"Invalid radius-nas-ip-override '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_NAS_IP_OVERRIDE)}")
    ...
    if 'route-offload' in payload:
        ...
        if value and value not in VALID_BODY_ROUTE_OFFLOAD:
            return (False, f"Invalid route-offload '{value}'. Must be one of: {', '.join(VALID_BODY_ROUTE_OFFLOAD)}")
    ...
    if 'route-offload-mclag' in payload:
        ...
        if value and value not in VALID_BODY_ROUTE_OFFLOAD_MCLAG:
            return (False, f"Invalid route-offload-mclag '{value}'. Must be one of: {', '.join(VALID_BODY_ROUTE_OFFLOAD_MCLAG)}")
    ...
    if 'staged-image-version' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 127:
            return (False, f"staged-image-version cannot exceed 127 characters")
    ...
    if 'delayed-restart-trigger' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 255:
                    return (False, f"delayed-restart-trigger must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"delayed-restart-trigger must be numeric, got: {value}")
    ...
    if 'firmware-provision' in payload:
        ...
        if value and value not in VALID_BODY_FIRMWARE_PROVISION:
            return (False, f"Invalid firmware-provision '{value}'. Must be one of: {', '.join(VALID_BODY_FIRMWARE_PROVISION)}")
    ...
    if 'firmware-provision-version' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"firmware-provision-version cannot exceed 35 characters")
    ...
    if 'firmware-provision-latest' in payload:
        ...
        if value and value not in VALID_BODY_FIRMWARE_PROVISION_LATEST:
            return (False, f"Invalid firmware-provision-latest '{value}'. Must be one of: {', '.join(VALID_BODY_FIRMWARE_PROVISION_LATEST)}")
    ...
    if 'override-snmp-sysinfo' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE_SNMP_SYSINFO:
            return (False, f"Invalid override-snmp-sysinfo '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_SNMP_SYSINFO)}")
    ...
    if 'override-snmp-trap-threshold' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE_SNMP_TRAP_THRESHOLD:
            return (False, f"Invalid override-snmp-trap-threshold '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_SNMP_TRAP_THRESHOLD)}")
    ...
    if 'override-snmp-community' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE_SNMP_COMMUNITY:
            return (False, f"Invalid override-snmp-community '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_SNMP_COMMUNITY)}")
    ...
    if 'override-snmp-user' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE_SNMP_USER:
            return (False, f"Invalid override-snmp-user '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_SNMP_USER)}")
    ...
    if 'qos-drop-policy' in payload:
        ...
        if value and value not in VALID_BODY_QOS_DROP_POLICY:
            return (False, f"Invalid qos-drop-policy '{value}'. Must be one of: {', '.join(VALID_BODY_QOS_DROP_POLICY)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100:
                    return (False, f"qos-red-probability must be between 0 and 100")
            except (ValueError, TypeError):
                return (False, f"qos-red-probability must be numeric, got: {value}")
