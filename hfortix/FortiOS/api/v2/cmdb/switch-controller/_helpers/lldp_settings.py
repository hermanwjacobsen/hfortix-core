...
def validate_lldp_settings_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 255:
                    return (False, f"fast-start-interval must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"fast-start-interval must be numeric, got: {value}")
    ...
    if 'management-interface' in payload:
        ...
        if value and value not in VALID_BODY_MANAGEMENT_INTERFACE:
            return (False, f"Invalid management-interface '{value}'. Must be one of: {', '.join(VALID_BODY_MANAGEMENT_INTERFACE)}")
    ...
    if 'device-detection' in payload:
        ...
        if value and value not in VALID_BODY_DEVICE_DETECTION:
            return (False, f"Invalid device-detection '{value}'. Must be one of: {', '.join(VALID_BODY_DEVICE_DETECTION)}")
