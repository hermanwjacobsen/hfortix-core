...
def validate_traffic_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'policer-status' in payload:
        ...
        if value and value not in VALID_BODY_POLICER_STATUS:
            return (False, f"Invalid policer-status '{value}'. Must be one of: {', '.join(VALID_BODY_POLICER_STATUS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 524287000:
                    return (False, f"guaranteed-bandwidth must be between 0 and 524287000")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"guaranteed-burst must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"guaranteed-burst must be numeric, got: {value}")
    ...
    if 'maximum-burst' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"maximum-burst must be between 0 and 4294967295")
            except (ValueError, TypeError):
...
def validate_traffic_policy_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
