...
VALID_BODY_MED_TLVS = ['inventory-management', 'network-policy', 'power-management', 'location-identification']
VALID_BODY_802.1_TLVS = ['port-vlan-id']
VALID_BODY_802.3_TLVS = ['max-frame-size', 'power-negotiation']
...
def validate_lldp_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'med-tlvs' in payload:
        ...
        if value and value not in VALID_BODY_MED_TLVS:
            return (False, f"Invalid med-tlvs '{value}'. Must be one of: {', '.join(VALID_BODY_MED_TLVS)}")
    
    # Validate 802.1-tlvs if present
    if '802.1-tlvs' in payload:
        value = payload.get('802.1-tlvs')
        if value and value not in VALID_BODY_802.1_TLVS:
            return (False, f"Invalid 802.1-tlvs '{value}'. Must be one of: {', '.join(VALID_BODY_802.1_TLVS)}")
    
    # Validate 802.3-tlvs if present
    if '802.3-tlvs' in payload:
        value = payload.get('802.3-tlvs')
        if value and value not in VALID_BODY_802.3_TLVS:
            return (False, f"Invalid 802.3-tlvs '{value}'. Must be one of: {', '.join(VALID_BODY_802.3_TLVS)}")
    ...
    if 'auto-isl' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_ISL:
            return (False, f"Invalid auto-isl '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_ISL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"auto-isl-hello-timer must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"auto-isl-hello-timer must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 90:
                    return (False, f"auto-isl-receive-timeout must be between 0 and 90")
            except (ValueError, TypeError):
                return (False, f"auto-isl-receive-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 9:
                    return (False, f"auto-isl-port-group must be between 0 and 9")
            except (ValueError, TypeError):
                return (False, f"auto-isl-port-group must be numeric, got: {value}")
    ...
    if 'auto-mclag-icl' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_MCLAG_ICL:
            return (False, f"Invalid auto-mclag-icl '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_MCLAG_ICL)}")
    ...
    if 'auto-isl-auth' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_ISL_AUTH:
            return (False, f"Invalid auto-isl-auth '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_ISL_AUTH)}")
    ...
    if 'auto-isl-auth-identity' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 63:
            return (False, f"auto-isl-auth-identity cannot exceed 63 characters")
        ...
        if value is not None:
            try:
                ...
                if int_val < 180 or int_val > 3600:
                    return (False, f"auto-isl-auth-reauth must be between 180 and 3600")
            except (ValueError, TypeError):
                return (False, f"auto-isl-auth-reauth must be numeric, got: {value}")
    ...
    if 'auto-isl-auth-encrypt' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_ISL_AUTH_ENCRYPT:
            return (False, f"Invalid auto-isl-auth-encrypt '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_ISL_AUTH_ENCRYPT)}")
    ...
    if 'auto-isl-auth-macsec-profile' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 63:
            return (False, f"auto-isl-auth-macsec-profile cannot exceed 63 characters")
...
def validate_lldp_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
