...
VALID_BODY_CATEGORY = ['device', 'firewall-user', 'ems-tag', 'fortivoice-tag', 'vulnerability']
...
def validate_nac_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'category' in payload:
        ...
        if value and value not in VALID_BODY_CATEGORY:
            return (False, f"Invalid category '{value}'. Must be one of: {', '.join(VALID_BODY_CATEGORY)}")
    ...
    if 'match-type' in payload:
        ...
        if value and value not in VALID_BODY_MATCH_TYPE:
            return (False, f"Invalid match-type '{value}'. Must be one of: {', '.join(VALID_BODY_MATCH_TYPE)}")
    ...
    if 'match-remove' in payload:
        ...
        if value and value not in VALID_BODY_MATCH_REMOVE:
            return (False, f"Invalid match-remove '{value}'. Must be one of: {', '.join(VALID_BODY_MATCH_REMOVE)}")
...
def validate_nac_policy_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
