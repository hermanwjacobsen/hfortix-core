...
VALID_BODY_SCIM_USER_ATTR_TYPE = ['user-name', 'display-name', 'external-id', 'email']
...
VALID_BODY_USER_CLAIM_TYPE = ['email', 'given-name', 'name', 'upn', 'common-name', 'email-adfs-1x', 'group', 'upn-adfs-1x', 'role', 'sur-name', 'ppid', 'name-identifier', 'authentication-method', 'deny-only-group-sid', 'deny-only-primary-sid', 'deny-only-primary-group-sid', 'group-sid', 'primary-group-sid', 'primary-sid', 'windows-account-name']
VALID_BODY_GROUP_CLAIM_TYPE = ['email', 'given-name', 'name', 'upn', 'common-name', 'email-adfs-1x', 'group', 'upn-adfs-1x', 'role', 'sur-name', 'ppid', 'name-identifier', 'authentication-method', 'deny-only-group-sid', 'deny-only-primary-sid', 'deny-only-primary-group-sid', 'group-sid', 'primary-group-sid', 'primary-sid', 'windows-account-name']
...
def validate_saml_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'scim-user-attr-type' in payload:
        ...
        if value and value not in VALID_BODY_SCIM_USER_ATTR_TYPE:
            return (False, f"Invalid scim-user-attr-type '{value}'. Must be one of: {', '.join(VALID_BODY_SCIM_USER_ATTR_TYPE)}")
    ...
    if 'scim-group-attr-type' in payload:
        ...
        if value and value not in VALID_BODY_SCIM_GROUP_ATTR_TYPE:
            return (False, f"Invalid scim-group-attr-type '{value}'. Must be one of: {', '.join(VALID_BODY_SCIM_GROUP_ATTR_TYPE)}")
    ...
    if 'digest-method' in payload:
        ...
        if value and value not in VALID_BODY_DIGEST_METHOD:
            return (False, f"Invalid digest-method '{value}'. Must be one of: {', '.join(VALID_BODY_DIGEST_METHOD)}")
    ...
    if 'limit-relaystate' in payload:
        ...
        if value and value not in VALID_BODY_LIMIT_RELAYSTATE:
            return (False, f"Invalid limit-relaystate '{value}'. Must be one of: {', '.join(VALID_BODY_LIMIT_RELAYSTATE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 300:
                    return (False, f"clock-tolerance must be between 0 and 300")
            except (ValueError, TypeError):
                return (False, f"clock-tolerance must be numeric, got: {value}")
    ...
    if 'adfs-claim' in payload:
        ...
        if value and value not in VALID_BODY_ADFS_CLAIM:
            return (False, f"Invalid adfs-claim '{value}'. Must be one of: {', '.join(VALID_BODY_ADFS_CLAIM)}")
    ...
    if 'user-claim-type' in payload:
        ...
        if value and value not in VALID_BODY_USER_CLAIM_TYPE:
            return (False, f"Invalid user-claim-type '{value}'. Must be one of: {', '.join(VALID_BODY_USER_CLAIM_TYPE)}")
    ...
    if 'group-claim-type' in payload:
        ...
        if value and value not in VALID_BODY_GROUP_CLAIM_TYPE:
            return (False, f"Invalid group-claim-type '{value}'. Must be one of: {', '.join(VALID_BODY_GROUP_CLAIM_TYPE)}")
