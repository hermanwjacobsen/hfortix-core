...
VALID_BODY_SWITCH_CONTROLLER_SERVICE_TYPE = ['login', 'framed', 'callback-login', 'callback-framed', 'outbound', 'administrative', 'nas-prompt', 'authenticate-only', 'callback-nas-prompt', 'call-check', 'callback-administrative']
VALID_BODY_TRANSPORT_PROTOCOL = ['udp', 'tcp', 'tls']
VALID_BODY_TLS_MIN_PROTO_VERSION = ['default', 'SSLv3', 'TLSv1', 'TLSv1-1', 'TLSv1-2', 'TLSv1-3']
VALID_BODY_SERVER_IDENTITY_CHECK = ['enable', 'disable']
VALID_BODY_ACCOUNT_KEY_PROCESSING = ['same', 'strip']
VALID_BODY_ACCOUNT_KEY_CERT_FIELD = ['othername', 'rfc822name', 'dnsname', 'cn']
...
VALID_BODY_RSSO_ENDPOINT_ATTRIBUTE = ['User-Name', 'NAS-IP-Address', 'Framed-IP-Address', 'Framed-IP-Netmask', 'Filter-Id', 'Login-IP-Host', 'Reply-Message', 'Callback-Number', 'Callback-Id', 'Framed-Route', 'Framed-IPX-Network', 'Class', 'Called-Station-Id', 'Calling-Station-Id', 'NAS-Identifier', 'Proxy-State', 'Login-LAT-Service', 'Login-LAT-Node', 'Login-LAT-Group', 'Framed-AppleTalk-Zone', 'Acct-Session-Id', 'Acct-Multi-Session-Id']
VALID_BODY_RSSO_ENDPOINT_BLOCK_ATTRIBUTE = ['User-Name', 'NAS-IP-Address', 'Framed-IP-Address', 'Framed-IP-Netmask', 'Filter-Id', 'Login-IP-Host', 'Reply-Message', 'Callback-Number', 'Callback-Id', 'Framed-Route', 'Framed-IPX-Network', 'Class', 'Called-Station-Id', 'Calling-Station-Id', 'NAS-Identifier', 'Proxy-State', 'Login-LAT-Service', 'Login-LAT-Node', 'Login-LAT-Group', 'Framed-AppleTalk-Zone', 'Acct-Session-Id', 'Acct-Multi-Session-Id']
VALID_BODY_SSO_ATTRIBUTE = ['User-Name', 'NAS-IP-Address', 'Framed-IP-Address', 'Framed-IP-Netmask', 'Filter-Id', 'Login-IP-Host', 'Reply-Message', 'Callback-Number', 'Callback-Id', 'Framed-Route', 'Framed-IPX-Network', 'Class', 'Called-Station-Id', 'Calling-Station-Id', 'NAS-Identifier', 'Proxy-State', 'Login-LAT-Service', 'Login-LAT-Node', 'Login-LAT-Group', 'Framed-AppleTalk-Zone', 'Acct-Session-Id', 'Acct-Multi-Session-Id']
VALID_BODY_SSO_ATTRIBUTE_VALUE_OVERRIDE = ['enable', 'disable']
VALID_BODY_RSSO_LOG_FLAGS = ['protocol-error', 'profile-missing', 'accounting-stop-missed', 'accounting-event', 'endpoint-block', 'radiusd-other', 'none']
...
def validate_radius_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'all-usergroup' in payload:
        ...
        if value and value not in VALID_BODY_ALL_USERGROUP:
            return (False, f"Invalid all-usergroup '{value}'. Must be one of: {', '.join(VALID_BODY_ALL_USERGROUP)}")
    ...
    if 'use-management-vdom' in payload:
        ...
        if value and value not in VALID_BODY_USE_MANAGEMENT_VDOM:
            return (False, f"Invalid use-management-vdom '{value}'. Must be one of: {', '.join(VALID_BODY_USE_MANAGEMENT_VDOM)}")
    ...
    if 'switch-controller-nas-ip-dynamic' in payload:
        ...
        if value and value not in VALID_BODY_SWITCH_CONTROLLER_NAS_IP_DYNAMIC:
            return (False, f"Invalid switch-controller-nas-ip-dynamic '{value}'. Must be one of: {', '.join(VALID_BODY_SWITCH_CONTROLLER_NAS_IP_DYNAMIC)}")
    ...
    if 'nas-id-type' in payload:
        ...
        if value and value not in VALID_BODY_NAS_ID_TYPE:
            return (False, f"Invalid nas-id-type '{value}'. Must be one of: {', '.join(VALID_BODY_NAS_ID_TYPE)}")
    ...
    if 'call-station-id-type' in payload:
        ...
        if value and value not in VALID_BODY_CALL_STATION_ID_TYPE:
            return (False, f"Invalid call-station-id-type '{value}'. Must be one of: {', '.join(VALID_BODY_CALL_STATION_ID_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 86400:
                    return (False, f"acct-interim-interval must be between 60 and 86400")
            except (ValueError, TypeError):
                return (False, f"acct-interim-interval must be numeric, got: {value}")
    ...
    if 'radius-coa' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_COA:
            return (False, f"Invalid radius-coa '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_COA)}")
    ...
    if 'h3c-compatibility' in payload:
        ...
        if value and value not in VALID_BODY_H3C_COMPATIBILITY:
            return (False, f"Invalid h3c-compatibility '{value}'. Must be one of: {', '.join(VALID_BODY_H3C_COMPATIBILITY)}")
    ...
    if 'username-case-sensitive' in payload:
        ...
        if value and value not in VALID_BODY_USERNAME_CASE_SENSITIVE:
            return (False, f"Invalid username-case-sensitive '{value}'. Must be one of: {', '.join(VALID_BODY_USERNAME_CASE_SENSITIVE)}")
    ...
    if 'group-override-attr-type' in payload:
        ...
        if value and value not in VALID_BODY_GROUP_OVERRIDE_ATTR_TYPE:
            return (False, f"Invalid group-override-attr-type '{value}'. Must be one of: {', '.join(VALID_BODY_GROUP_OVERRIDE_ATTR_TYPE)}")
    ...
    if 'password-renewal' in payload:
        ...
        if value and value not in VALID_BODY_PASSWORD_RENEWAL:
            return (False, f"Invalid password-renewal '{value}'. Must be one of: {', '.join(VALID_BODY_PASSWORD_RENEWAL)}")
    ...
    if 'require-message-authenticator' in payload:
        ...
        if value and value not in VALID_BODY_REQUIRE_MESSAGE_AUTHENTICATOR:
            return (False, f"Invalid require-message-authenticator '{value}'. Must be one of: {', '.join(VALID_BODY_REQUIRE_MESSAGE_AUTHENTICATOR)}")
    ...
    if 'password-encoding' in payload:
        ...
        if value and value not in VALID_BODY_PASSWORD_ENCODING:
            return (False, f"Invalid password-encoding '{value}'. Must be one of: {', '.join(VALID_BODY_PASSWORD_ENCODING)}")
    ...
    if 'acct-all-servers' in payload:
        ...
        if value and value not in VALID_BODY_ACCT_ALL_SERVERS:
            return (False, f"Invalid acct-all-servers '{value}'. Must be one of: {', '.join(VALID_BODY_ACCT_ALL_SERVERS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 600:
                    return (False, f"switch-controller-acct-fast-framedip-detect must be between 2 and 600")
            except (ValueError, TypeError):
                return (False, f"switch-controller-acct-fast-framedip-detect must be numeric, got: {value}")
    ...
    if 'switch-controller-service-type' in payload:
        ...
        if value and value not in VALID_BODY_SWITCH_CONTROLLER_SERVICE_TYPE:
            return (False, f"Invalid switch-controller-service-type '{value}'. Must be one of: {', '.join(VALID_BODY_SWITCH_CONTROLLER_SERVICE_TYPE)}")
    ...
    if 'transport-protocol' in payload:
        ...
        if value and value not in VALID_BODY_TRANSPORT_PROTOCOL:
            return (False, f"Invalid transport-protocol '{value}'. Must be one of: {', '.join(VALID_BODY_TRANSPORT_PROTOCOL)}")
    ...
    if 'tls-min-proto-version' in payload:
        ...
        if value and value not in VALID_BODY_TLS_MIN_PROTO_VERSION:
            return (False, f"Invalid tls-min-proto-version '{value}'. Must be one of: {', '.join(VALID_BODY_TLS_MIN_PROTO_VERSION)}")
    ...
    if 'account-key-processing' in payload:
        ...
        if value and value not in VALID_BODY_ACCOUNT_KEY_PROCESSING:
            return (False, f"Invalid account-key-processing '{value}'. Must be one of: {', '.join(VALID_BODY_ACCOUNT_KEY_PROCESSING)}")
    ...
    if 'account-key-cert-field' in payload:
        ...
        if value and value not in VALID_BODY_ACCOUNT_KEY_CERT_FIELD:
            return (False, f"Invalid account-key-cert-field '{value}'. Must be one of: {', '.join(VALID_BODY_ACCOUNT_KEY_CERT_FIELD)}")
    
    # Validate rsso if present
    if 'rsso' in payload:
        value = payload.get('rsso')
        if value and value not in VALID_BODY_RSSO:
            return (False, f"Invalid rsso '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO)}")
    ...
    if 'rsso-radius-server-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"rsso-radius-server-port must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"rsso-radius-server-port must be numeric, got: {value}")
    ...
    if 'rsso-radius-response' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_RADIUS_RESPONSE:
            return (False, f"Invalid rsso-radius-response '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_RADIUS_RESPONSE)}")
    ...
    if 'rsso-validate-request-secret' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_VALIDATE_REQUEST_SECRET:
            return (False, f"Invalid rsso-validate-request-secret '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_VALIDATE_REQUEST_SECRET)}")
    ...
    if 'rsso-endpoint-attribute' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_ENDPOINT_ATTRIBUTE:
            return (False, f"Invalid rsso-endpoint-attribute '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_ENDPOINT_ATTRIBUTE)}")
    ...
    if 'rsso-endpoint-block-attribute' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_ENDPOINT_BLOCK_ATTRIBUTE:
            return (False, f"Invalid rsso-endpoint-block-attribute '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_ENDPOINT_BLOCK_ATTRIBUTE)}")
    ...
    if 'sso-attribute' in payload:
        ...
        if value and value not in VALID_BODY_SSO_ATTRIBUTE:
            return (False, f"Invalid sso-attribute '{value}'. Must be one of: {', '.join(VALID_BODY_SSO_ATTRIBUTE)}")
    ...
    if 'sso-attribute-value-override' in payload:
        ...
        if value and value not in VALID_BODY_SSO_ATTRIBUTE_VALUE_OVERRIDE:
            return (False, f"Invalid sso-attribute-value-override '{value}'. Must be one of: {', '.join(VALID_BODY_SSO_ATTRIBUTE_VALUE_OVERRIDE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"rsso-context-timeout must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"rsso-context-timeout must be numeric, got: {value}")
    ...
    if 'rsso-log-period' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"rsso-log-period must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"rsso-log-period must be numeric, got: {value}")
    ...
    if 'rsso-log-flags' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_LOG_FLAGS:
            return (False, f"Invalid rsso-log-flags '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_LOG_FLAGS)}")
    ...
    if 'rsso-flush-ip-session' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_FLUSH_IP_SESSION:
            return (False, f"Invalid rsso-flush-ip-session '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_FLUSH_IP_SESSION)}")
    ...
    if 'rsso-ep-one-ip-only' in payload:
        ...
        if value and value not in VALID_BODY_RSSO_EP_ONE_IP_ONLY:
            return (False, f"Invalid rsso-ep-one-ip-only '{value}'. Must be one of: {', '.join(VALID_BODY_RSSO_EP_ONE_IP_ONLY)}")
    ...
    if 'delimiter' in payload:
        ...
        if value and value not in VALID_BODY_DELIMITER:
            return (False, f"Invalid delimiter '{value}'. Must be one of: {', '.join(VALID_BODY_DELIMITER)}")
