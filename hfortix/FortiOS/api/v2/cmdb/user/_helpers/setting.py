...
VALID_BODY_AUTH_SSL_MIN_PROTO_VERSION = ['default', 'SSLv3', 'TLSv1', 'TLSv1-1', 'TLSv1-2', 'TLSv1-3']
VALID_BODY_AUTH_SSL_MAX_PROTO_VERSION = ['sslv3', 'tlsv1', 'tlsv1-1', 'tlsv1-2', 'tlsv1-3']
...
def validate_setting_put(
    ...
    if 'auth-secure-http' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SECURE_HTTP:
            return (False, f"Invalid auth-secure-http '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SECURE_HTTP)}")
    ...
    if 'auth-http-basic' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_HTTP_BASIC:
            return (False, f"Invalid auth-http-basic '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_HTTP_BASIC)}")
    ...
    if 'auth-ssl-allow-renegotiation' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SSL_ALLOW_RENEGOTIATION:
            return (False, f"Invalid auth-ssl-allow-renegotiation '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SSL_ALLOW_RENEGOTIATION)}")
    ...
    if 'auth-src-mac' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SRC_MAC:
            return (False, f"Invalid auth-src-mac '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SRC_MAC)}")
    ...
    if 'auth-on-demand' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_ON_DEMAND:
            return (False, f"Invalid auth-on-demand '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_ON_DEMAND)}")
    ...
    if 'auth-timeout-type' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_TIMEOUT_TYPE:
            return (False, f"Invalid auth-timeout-type '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_TIMEOUT_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"auth-portal-timeout must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"auth-portal-timeout must be numeric, got: {value}")
    ...
    if 'radius-ses-timeout-act' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_SES_TIMEOUT_ACT:
            return (False, f"Invalid radius-ses-timeout-act '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_SES_TIMEOUT_ACT)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 3600:
                    return (False, f"auth-blackout-time must be between 0 and 3600")
            except (ValueError, TypeError):
                return (False, f"auth-blackout-time must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 100:
                    return (False, f"auth-invalid-max must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"auth-invalid-max must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 10:
                    return (False, f"auth-lockout-threshold must be between 1 and 10")
            except (ValueError, TypeError):
                return (False, f"auth-lockout-threshold must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"auth-lockout-duration must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"auth-lockout-duration must be numeric, got: {value}")
    ...
    if 'per-policy-disclaimer' in payload:
        ...
        if value and value not in VALID_BODY_PER_POLICY_DISCLAIMER:
            return (False, f"Invalid per-policy-disclaimer '{value}'. Must be one of: {', '.join(VALID_BODY_PER_POLICY_DISCLAIMER)}")
    ...
    if 'auth-ssl-min-proto-version' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SSL_MIN_PROTO_VERSION:
            return (False, f"Invalid auth-ssl-min-proto-version '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SSL_MIN_PROTO_VERSION)}")
    ...
    if 'auth-ssl-max-proto-version' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SSL_MAX_PROTO_VERSION:
            return (False, f"Invalid auth-ssl-max-proto-version '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SSL_MAX_PROTO_VERSION)}")
    ...
    if 'auth-ssl-sigalgs' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_SSL_SIGALGS:
            return (False, f"Invalid auth-ssl-sigalgs '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_SSL_SIGALGS)}")
    ...
    if 'default-user-password-policy' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"default-user-password-policy cannot exceed 35 characters")
    
    # Validate cors if present
    if 'cors' in payload:
        value = payload.get('cors')
        if value and value not in VALID_BODY_CORS:
            return (False, f"Invalid cors '{value}'. Must be one of: {', '.join(VALID_BODY_CORS)}")
