...
VALID_BODY_CONNECT_PROTOCOL = ['rpc-over-tcp', 'rpc-over-http', 'rpc-over-https']
...
def validate_exchange_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'connect-protocol' in payload:
        ...
        if value and value not in VALID_BODY_CONNECT_PROTOCOL:
            return (False, f"Invalid connect-protocol '{value}'. Must be one of: {', '.join(VALID_BODY_CONNECT_PROTOCOL)}")
    ...
    if 'validate-server-certificate' in payload:
        ...
        if value and value not in VALID_BODY_VALIDATE_SERVER_CERTIFICATE:
            return (False, f"Invalid validate-server-certificate '{value}'. Must be one of: {', '.join(VALID_BODY_VALIDATE_SERVER_CERTIFICATE)}")
    ...
    if 'auth-level' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_LEVEL:
            return (False, f"Invalid auth-level '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_LEVEL)}")
    ...
    if 'http-auth-type' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_AUTH_TYPE:
            return (False, f"Invalid http-auth-type '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_AUTH_TYPE)}")
    ...
    if 'auto-discover-kdc' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_DISCOVER_KDC:
            return (False, f"Invalid auto-discover-kdc '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_DISCOVER_KDC)}")
...
def validate_exchange_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
