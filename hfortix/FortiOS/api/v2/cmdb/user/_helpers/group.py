...
def validate_group_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'group-type' in payload:
        ...
        if value and value not in VALID_BODY_GROUP_TYPE:
            return (False, f"Invalid group-type '{value}'. Must be one of: {', '.join(VALID_BODY_GROUP_TYPE)}")
    ...
    if 'auth-concurrent-override' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_CONCURRENT_OVERRIDE:
            return (False, f"Invalid auth-concurrent-override '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_CONCURRENT_OVERRIDE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100:
                    return (False, f"auth-concurrent-value must be between 0 and 100")
            except (ValueError, TypeError):
                return (False, f"auth-concurrent-value must be numeric, got: {value}")
    ...
    if 'user-id' in payload:
        ...
        if value and value not in VALID_BODY_USER_ID:
            return (False, f"Invalid user-id '{value}'. Must be one of: {', '.join(VALID_BODY_USER_ID)}")
    
    # Validate password if present
    if 'password' in payload:
        value = payload.get('password')
        if value and value not in VALID_BODY_PASSWORD:
            return (False, f"Invalid password '{value}'. Must be one of: {', '.join(VALID_BODY_PASSWORD)}")
    ...
    if 'user-name' in payload:
        ...
        if value and value not in VALID_BODY_USER_NAME:
            return (False, f"Invalid user-name '{value}'. Must be one of: {', '.join(VALID_BODY_USER_NAME)}")
    
    # Validate sponsor if present
    if 'sponsor' in payload:
        value = payload.get('sponsor')
        if value and value not in VALID_BODY_SPONSOR:
            return (False, f"Invalid sponsor '{value}'. Must be one of: {', '.join(VALID_BODY_SPONSOR)}")
    
    # Validate company if present
    if 'company' in payload:
        value = payload.get('company')
        if value and value not in VALID_BODY_COMPANY:
            return (False, f"Invalid company '{value}'. Must be one of: {', '.join(VALID_BODY_COMPANY)}")
    
    # Validate email if present
    if 'email' in payload:
        value = payload.get('email')
        if value and value not in VALID_BODY_EMAIL:
            return (False, f"Invalid email '{value}'. Must be one of: {', '.join(VALID_BODY_EMAIL)}")
    ...
    if 'mobile-phone' in payload:
        ...
        if value and value not in VALID_BODY_MOBILE_PHONE:
            return (False, f"Invalid mobile-phone '{value}'. Must be one of: {', '.join(VALID_BODY_MOBILE_PHONE)}")
    ...
    if 'expire-type' in payload:
        ...
        if value and value not in VALID_BODY_EXPIRE_TYPE:
            return (False, f"Invalid expire-type '{value}'. Must be one of: {', '.join(VALID_BODY_EXPIRE_TYPE)}")
    ...
    if 'multiple-guest-add' in payload:
        ...
        if value and value not in VALID_BODY_MULTIPLE_GUEST_ADD:
            return (False, f"Invalid multiple-guest-add '{value}'. Must be one of: {', '.join(VALID_BODY_MULTIPLE_GUEST_ADD)}")
