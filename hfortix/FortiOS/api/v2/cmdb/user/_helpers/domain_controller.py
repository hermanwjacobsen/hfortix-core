...
def validate_domain_controller_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'ad-mode' in payload:
        ...
        if value and value not in VALID_BODY_AD_MODE:
            return (False, f"Invalid ad-mode '{value}'. Must be one of: {', '.join(VALID_BODY_AD_MODE)}")
    ...
    if 'replication-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"replication-port must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"replication-port must be numeric, got: {value}")
    ...
    if 'change-detection' in payload:
        ...
        if value and value not in VALID_BODY_CHANGE_DETECTION:
            return (False, f"Invalid change-detection '{value}'. Must be one of: {', '.join(VALID_BODY_CHANGE_DETECTION)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 10080:
                    return (False, f"change-detection-period must be between 5 and 10080")
            except (ValueError, TypeError):
                return (False, f"change-detection-period must be numeric, got: {value}")
    ...
    if 'dns-srv-lookup' in payload:
        ...
        if value and value not in VALID_BODY_DNS_SRV_LOOKUP:
            return (False, f"Invalid dns-srv-lookup '{value}'. Must be one of: {', '.join(VALID_BODY_DNS_SRV_LOOKUP)}")
...
def validate_domain_controller_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
