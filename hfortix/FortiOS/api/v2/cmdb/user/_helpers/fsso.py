...
def validate_fsso_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'group-poll-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 2880:
                    return (False, f"group-poll-interval must be between 1 and 2880")
            except (ValueError, TypeError):
                return (False, f"group-poll-interval must be numeric, got: {value}")
    ...
    if 'ldap-poll' in payload:
        ...
        if value and value not in VALID_BODY_LDAP_POLL:
            return (False, f"Invalid ldap-poll '{value}'. Must be one of: {', '.join(VALID_BODY_LDAP_POLL)}")
    ...
    if 'ldap-poll-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 2880:
                    return (False, f"ldap-poll-interval must be between 1 and 2880")
            except (ValueError, TypeError):
                return (False, f"ldap-poll-interval must be numeric, got: {value}")
    ...
    if 'ssl' in payload:
        ...
        if value and value not in VALID_BODY_SSL:
            return (False, f"Invalid ssl '{value}'. Must be one of: {', '.join(VALID_BODY_SSL)}")
    ...
    if 'ssl-server-host-ip-check' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_HOST_IP_CHECK:
            return (False, f"Invalid ssl-server-host-ip-check '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_HOST_IP_CHECK)}")
