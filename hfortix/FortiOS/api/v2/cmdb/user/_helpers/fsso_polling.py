...
def validate_fsso_polling_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"polling-frequency must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"polling-frequency must be numeric, got: {value}")
    
    # Validate smbv1 if present
    if 'smbv1' in payload:
        value = payload.get('smbv1')
        if value and value not in VALID_BODY_SMBV1:
            return (False, f"Invalid smbv1 '{value}'. Must be one of: {', '.join(VALID_BODY_SMBV1)}")
    ...
    if 'smb-ntlmv1-auth' in payload:
        ...
        if value and value not in VALID_BODY_SMB_NTLMV1_AUTH:
            return (False, f"Invalid smb-ntlmv1-auth '{value}'. Must be one of: {', '.join(VALID_BODY_SMB_NTLMV1_AUTH)}")
...
def validate_fsso_polling_delete(id: str | None = None) -> tuple[bool, str | None]:
    """
