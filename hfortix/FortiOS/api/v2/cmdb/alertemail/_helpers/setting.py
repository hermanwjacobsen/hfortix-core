...
VALID_BODY_SEVERITY = ['emergency', 'alert', 'critical', 'error', 'warning', 'notification', 'information', 'debug']
...
def validate_setting_put(
    ...
    if 'filter-mode' in payload:
        ...
        if value and value not in VALID_BODY_FILTER_MODE:
            return (False, f"Invalid filter-mode '{value}'. Must be one of: {', '.join(VALID_BODY_FILTER_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 99999:
                    return (False, f"email-interval must be between 1 and 99999")
    ...
    if 'IPS-logs' in payload:
        ...
        if value and value not in VALID_BODY_IPS_LOGS:
            return (False, f"Invalid IPS-logs '{value}'. Must be one of: {', '.join(VALID_BODY_IPS_LOGS)}")
    ...
    if 'firewall-authentication-failure-logs' in payload:
        ...
        if value and value not in VALID_BODY_FIREWALL_AUTHENTICATION_FAILURE_LOGS:
            return (False, f"Invalid firewall-authentication-failure-logs '{value}'. Must be one of: {', '.join(VALID_BODY_FIREWALL_AUTHENTICATION_FAILURE_LOGS)}")
    
    # Validate HA-logs if present
    if 'HA-logs' in payload:
        value = payload.get('HA-logs')
        if value and value not in VALID_BODY_HA_LOGS:
            return (False, f"Invalid HA-logs '{value}'. Must be one of: {', '.join(VALID_BODY_HA_LOGS)}")
    ...
    if 'IPsec-errors-logs' in payload:
        ...
        if value and value not in VALID_BODY_IPSEC_ERRORS_LOGS:
            return (False, f"Invalid IPsec-errors-logs '{value}'. Must be one of: {', '.join(VALID_BODY_IPSEC_ERRORS_LOGS)}")
    ...
    if 'FDS-update-logs' in payload:
        ...
        if value and value not in VALID_BODY_FDS_UPDATE_LOGS:
            return (False, f"Invalid FDS-update-logs '{value}'. Must be one of: {', '.join(VALID_BODY_FDS_UPDATE_LOGS)}")
    ...
    if 'PPP-errors-logs' in payload:
        ...
        if value and value not in VALID_BODY_PPP_ERRORS_LOGS:
            return (False, f"Invalid PPP-errors-logs '{value}'. Must be one of: {', '.join(VALID_BODY_PPP_ERRORS_LOGS)}")
    ...
    if 'antivirus-logs' in payload:
        ...
        if value and value not in VALID_BODY_ANTIVIRUS_LOGS:
            return (False, f"Invalid antivirus-logs '{value}'. Must be one of: {', '.join(VALID_BODY_ANTIVIRUS_LOGS)}")
    ...
    if 'webfilter-logs' in payload:
        ...
        if value and value not in VALID_BODY_WEBFILTER_LOGS:
            return (False, f"Invalid webfilter-logs '{value}'. Must be one of: {', '.join(VALID_BODY_WEBFILTER_LOGS)}")
    ...
    if 'configuration-changes-logs' in payload:
        ...
        if value and value not in VALID_BODY_CONFIGURATION_CHANGES_LOGS:
            return (False, f"Invalid configuration-changes-logs '{value}'. Must be one of: {', '.join(VALID_BODY_CONFIGURATION_CHANGES_LOGS)}")
    ...
    if 'violation-traffic-logs' in payload:
        ...
        if value and value not in VALID_BODY_VIOLATION_TRAFFIC_LOGS:
            return (False, f"Invalid violation-traffic-logs '{value}'. Must be one of: {', '.join(VALID_BODY_VIOLATION_TRAFFIC_LOGS)}")
    ...
    if 'admin-login-logs' in payload:
        ...
        if value and value not in VALID_BODY_ADMIN_LOGIN_LOGS:
            return (False, f"Invalid admin-login-logs '{value}'. Must be one of: {', '.join(VALID_BODY_ADMIN_LOGIN_LOGS)}")
    ...
    if 'FDS-license-expiring-warning' in payload:
        ...
        if value and value not in VALID_BODY_FDS_LICENSE_EXPIRING_WARNING:
            return (False, f"Invalid FDS-license-expiring-warning '{value}'. Must be one of: {', '.join(VALID_BODY_FDS_LICENSE_EXPIRING_WARNING)}")
    ...
    if 'log-disk-usage-warning' in payload:
        ...
        if value and value not in VALID_BODY_LOG_DISK_USAGE_WARNING:
            return (False, f"Invalid log-disk-usage-warning '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_DISK_USAGE_WARNING)}")
    ...
    if 'fortiguard-log-quota-warning' in payload:
        ...
        if value and value not in VALID_BODY_FORTIGUARD_LOG_QUOTA_WARNING:
            return (False, f"Invalid fortiguard-log-quota-warning '{value}'. Must be one of: {', '.join(VALID_BODY_FORTIGUARD_LOG_QUOTA_WARNING)}")
    ...
    if 'amc-interface-bypass-mode' in payload:
        ...
        if value and value not in VALID_BODY_AMC_INTERFACE_BYPASS_MODE:
            return (False, f"Invalid amc-interface-bypass-mode '{value}'. Must be one of: {', '.join(VALID_BODY_AMC_INTERFACE_BYPASS_MODE)}")
    ...
    if 'FIPS-CC-errors' in payload:
        ...
        if value and value not in VALID_BODY_FIPS_CC_ERRORS:
            return (False, f"Invalid FIPS-CC-errors '{value}'. Must be one of: {', '.join(VALID_BODY_FIPS_CC_ERRORS)}")
    ...
    if 'FSSO-disconnect-logs' in payload:
        ...
        if value and value not in VALID_BODY_FSSO_DISCONNECT_LOGS:
            return (False, f"Invalid FSSO-disconnect-logs '{value}'. Must be one of: {', '.join(VALID_BODY_FSSO_DISCONNECT_LOGS)}")
    ...
    if 'ssh-logs' in payload:
        ...
        if value and value not in VALID_BODY_SSH_LOGS:
            return (False, f"Invalid ssh-logs '{value}'. Must be one of: {', '.join(VALID_BODY_SSH_LOGS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 99:
                    return (False, f"local-disk-usage must be between 1 and 99")
            except (ValueError, TypeError):
                return (False, f"local-disk-usage must be numeric, got: {value}")
    ...
    if 'emergency-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"emergency-interval must be between 1 and 99999")
            except (ValueError, TypeError):
                return (False, f"emergency-interval must be numeric, got: {value}")
    ...
    if 'alert-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"alert-interval must be between 1 and 99999")
            except (ValueError, TypeError):
    ...
    if 'critical-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"critical-interval must be between 1 and 99999")
            except (ValueError, TypeError):
                return (False, f"critical-interval must be numeric, got: {value}")
    ...
    if 'error-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"error-interval must be between 1 and 99999")
            except (ValueError, TypeError):
    ...
    if 'warning-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"warning-interval must be between 1 and 99999")
            except (ValueError, TypeError):
                return (False, f"warning-interval must be numeric, got: {value}")
    ...
    if 'notification-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"notification-interval must be between 1 and 99999")
            except (ValueError, TypeError):
                return (False, f"notification-interval must be numeric, got: {value}")
    ...
    if 'information-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"information-interval must be between 1 and 99999")
            except (ValueError, TypeError):
                return (False, f"information-interval must be numeric, got: {value}")
    ...
    if 'debug-interval' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 99999:
                    return (False, f"debug-interval must be between 1 and 99999")
            except (ValueError, TypeError):
