...
def validate_global__put(
    ...
    if 'fail-open' in payload:
        ...
        if value and value not in VALID_BODY_FAIL_OPEN:
            return (False, f"Invalid fail-open '{value}'. Must be one of: {', '.join(VALID_BODY_FAIL_OPEN)}")
    ...
    if 'traffic-submit' in payload:
        ...
        if value and value not in VALID_BODY_TRAFFIC_SUBMIT:
            return (False, f"Invalid traffic-submit '{value}'. Must be one of: {', '.join(VALID_BODY_TRAFFIC_SUBMIT)}")
    ...
    if 'anomaly-mode' in payload:
        ...
        if value and value not in VALID_BODY_ANOMALY_MODE:
            return (False, f"Invalid anomaly-mode '{value}'. Must be one of: {', '.join(VALID_BODY_ANOMALY_MODE)}")
    ...
    if 'session-limit-mode' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_LIMIT_MODE:
            return (False, f"Invalid session-limit-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_LIMIT_MODE)}")
    ...
    if 'sync-session-ttl' in payload:
        ...
        if value and value not in VALID_BODY_SYNC_SESSION_TTL:
            return (False, f"Invalid sync-session-ttl '{value}'. Must be one of: {', '.join(VALID_BODY_SYNC_SESSION_TTL)}")
    ...
    if 'np-accel-mode' in payload:
        ...
        if value and value not in VALID_BODY_NP_ACCEL_MODE:
            return (False, f"Invalid np-accel-mode '{value}'. Must be one of: {', '.join(VALID_BODY_NP_ACCEL_MODE)}")
    ...
    if 'ips-reserve-cpu' in payload:
        ...
        if value and value not in VALID_BODY_IPS_RESERVE_CPU:
            return (False, f"Invalid ips-reserve-cpu '{value}'. Must be one of: {', '.join(VALID_BODY_IPS_RESERVE_CPU)}")
    ...
    if 'cp-accel-mode' in payload:
        ...
        if value and value not in VALID_BODY_CP_ACCEL_MODE:
            return (False, f"Invalid cp-accel-mode '{value}'. Must be one of: {', '.join(VALID_BODY_CP_ACCEL_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2147483647:
                    return (False, f"deep-app-insp-timeout must be between 0 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"deep-app-insp-timeout must be numeric, got: {value}")
    ...
    if 'deep-app-insp-db-limit' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2147483647:
                    return (False, f"deep-app-insp-db-limit must be between 0 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"deep-app-insp-db-limit must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 128 or int_val > 4096:
                    return (False, f"packet-log-queue-depth must be between 128 and 4096")
            except (ValueError, TypeError):
                return (False, f"packet-log-queue-depth must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"ngfw-max-scan-range must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"ngfw-max-scan-range must be numeric, got: {value}")
    ...
    if 'machine-learning-detection' in payload:
        ...
        if value and value not in VALID_BODY_MACHINE_LEARNING_DETECTION:
            return (False, f"Invalid machine-learning-detection '{value}'. Must be one of: {', '.join(VALID_BODY_MACHINE_LEARNING_DETECTION)}")
