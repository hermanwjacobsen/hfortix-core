...
def validate_settings_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 255:
                    return (False, f"packet-log-history must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"packet-log-history must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 255:
                    return (False, f"packet-log-post-attack must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"packet-log-post-attack must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 64 or int_val > 8192:
                    return (False, f"packet-log-memory must be between 64 and 8192")
            except (ValueError, TypeError):
                return (False, f"packet-log-memory must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"ips-packet-quota must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"ips-packet-quota must be numeric, got: {value}")
    ...
    if 'proxy-inline-ips' in payload:
        ...
        if value and value not in VALID_BODY_PROXY_INLINE_IPS:
            return (False, f"Invalid proxy-inline-ips '{value}'. Must be one of: {', '.join(VALID_BODY_PROXY_INLINE_IPS)}")
    ...
    if 'ha-session-pickup' in payload:
        ...
        if value and value not in VALID_BODY_HA_SESSION_PICKUP:
            return (False, f"Invalid ha-session-pickup '{value}'. Must be one of: {', '.join(VALID_BODY_HA_SESSION_PICKUP)}")
