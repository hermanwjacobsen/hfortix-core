...
VALID_BODY_FULL_ARCHIVE_PROTO = ['smtp', 'pop3', 'imap', 'http-get', 'http-post', 'ftp', 'nntp', 'mapi', 'ssh', 'cifs']
VALID_BODY_SUMMARY_PROTO = ['smtp', 'pop3', 'imap', 'http-get', 'http-post', 'ftp', 'nntp', 'mapi', 'ssh', 'cifs']
...
def validate_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'dlp-log' in payload:
        ...
        if value and value not in VALID_BODY_DLP_LOG:
            return (False, f"Invalid dlp-log '{value}'. Must be one of: {', '.join(VALID_BODY_DLP_LOG)}")
    ...
    if 'nac-quar-log' in payload:
        ...
        if value and value not in VALID_BODY_NAC_QUAR_LOG:
            return (False, f"Invalid nac-quar-log '{value}'. Must be one of: {', '.join(VALID_BODY_NAC_QUAR_LOG)}")
    ...
    if 'full-archive-proto' in payload:
        ...
        if value and value not in VALID_BODY_FULL_ARCHIVE_PROTO:
            return (False, f"Invalid full-archive-proto '{value}'. Must be one of: {', '.join(VALID_BODY_FULL_ARCHIVE_PROTO)}")
    ...
    if 'summary-proto' in payload:
        ...
        if value and value not in VALID_BODY_SUMMARY_PROTO:
            return (False, f"Invalid summary-proto '{value}'. Must be one of: {', '.join(VALID_BODY_SUMMARY_PROTO)}")
    ...
    if 'fortidata-error-action' in payload:
        ...
        if value and value not in VALID_BODY_FORTIDATA_ERROR_ACTION:
            return (False, f"Invalid fortidata-error-action '{value}'. Must be one of: {', '.join(VALID_BODY_FORTIDATA_ERROR_ACTION)}")
