...
VALID_BODY_UPLOAD_OPTION = ['store-and-upload', 'realtime', '1-minute', '5-minute']
...
def validate_fortianalyzer3_override_setting_put(
    ...
    if 'fallback-to-primary' in payload:
        ...
        if value and value not in VALID_BODY_FALLBACK_TO_PRIMARY:
            return (False, f"Invalid fallback-to-primary '{value}'. Must be one of: {', '.join(VALID_BODY_FALLBACK_TO_PRIMARY)}")
    ...
    if 'certificate-verification' in payload:
        ...
        if value and value not in VALID_BODY_CERTIFICATE_VERIFICATION:
            return (False, f"Invalid certificate-verification '{value}'. Must be one of: {', '.join(VALID_BODY_CERTIFICATE_VERIFICATION)}")
    ...
    if 'access-config' in payload:
        ...
        if value and value not in VALID_BODY_ACCESS_CONFIG:
            return (False, f"Invalid access-config '{value}'. Must be one of: {', '.join(VALID_BODY_ACCESS_CONFIG)}")
    ...
    if 'hmac-algorithm' in payload:
        ...
        if value and value not in VALID_BODY_HMAC_ALGORITHM:
            return (False, f"Invalid hmac-algorithm '{value}'. Must be one of: {', '.join(VALID_BODY_HMAC_ALGORITHM)}")
    ...
    if 'enc-algorithm' in payload:
        ...
        if value and value not in VALID_BODY_ENC_ALGORITHM:
            return (False, f"Invalid enc-algorithm '{value}'. Must be one of: {', '.join(VALID_BODY_ENC_ALGORITHM)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 120:
                    return (False, f"monitor-keepalive-period must be between 1 and 120")
            except (ValueError, TypeError):
                return (False, f"monitor-keepalive-period must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 86400:
                    return (False, f"monitor-failure-retry-period must be between 1 and 86400")
            except (ValueError, TypeError):
                return (False, f"monitor-failure-retry-period must be numeric, got: {value}")
    ...
    if 'upload-option' in payload:
        ...
        if value and value not in VALID_BODY_UPLOAD_OPTION:
            return (False, f"Invalid upload-option '{value}'. Must be one of: {', '.join(VALID_BODY_UPLOAD_OPTION)}")
    ...
    if 'upload-interval' in payload:
        ...
        if value and value not in VALID_BODY_UPLOAD_INTERVAL:
            return (False, f"Invalid upload-interval '{value}'. Must be one of: {', '.join(VALID_BODY_UPLOAD_INTERVAL)}")
    ...
    if 'reliable' in payload:
        ...
        if value and value not in VALID_BODY_RELIABLE:
            return (False, f"Invalid reliable '{value}'. Must be one of: {', '.join(VALID_BODY_RELIABLE)}")
    
    # Validate priority if present
    if 'priority' in payload:
        value = payload.get('priority')
        if value and value not in VALID_BODY_PRIORITY:
            return (False, f"Invalid priority '{value}'. Must be one of: {', '.join(VALID_BODY_PRIORITY)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100000:
                    return (False, f"max-log-rate must be between 0 and 100000")
            except (ValueError, TypeError):
