...
def validate_disk_filter_put(
    ...
    if 'forward-traffic' in payload:
        ...
        if value and value not in VALID_BODY_FORWARD_TRAFFIC:
            return (False, f"Invalid forward-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_FORWARD_TRAFFIC)}")
    ...
    if 'local-traffic' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_TRAFFIC:
            return (False, f"Invalid local-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_TRAFFIC)}")
    ...
    if 'multicast-traffic' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_TRAFFIC:
            return (False, f"Invalid multicast-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_TRAFFIC)}")
    ...
    if 'sniffer-traffic' in payload:
        ...
        if value and value not in VALID_BODY_SNIFFER_TRAFFIC:
            return (False, f"Invalid sniffer-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_SNIFFER_TRAFFIC)}")
    ...
    if 'ztna-traffic' in payload:
        ...
        if value and value not in VALID_BODY_ZTNA_TRAFFIC:
            return (False, f"Invalid ztna-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_ZTNA_TRAFFIC)}")
    ...
    if 'http-transaction' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_TRANSACTION:
            return (False, f"Invalid http-transaction '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_TRANSACTION)}")
    
    # Validate anomaly if present
    if 'anomaly' in payload:
        value = payload.get('anomaly')
        if value and value not in VALID_BODY_ANOMALY:
            return (False, f"Invalid anomaly '{value}'. Must be one of: {', '.join(VALID_BODY_ANOMALY)}")
    
    # Validate voip if present
    if 'voip' in payload:
        value = payload.get('voip')
        if value and value not in VALID_BODY_VOIP:
            return (False, f"Invalid voip '{value}'. Must be one of: {', '.join(VALID_BODY_VOIP)}")
    ...
    if 'dlp-archive' in payload:
        ...
        if value and value not in VALID_BODY_DLP_ARCHIVE:
            return (False, f"Invalid dlp-archive '{value}'. Must be one of: {', '.join(VALID_BODY_DLP_ARCHIVE)}")
    ...
    if 'forti-switch' in payload:
        ...
        if value and value not in VALID_BODY_FORTI_SWITCH:
            return (False, f"Invalid forti-switch '{value}'. Must be one of: {', '.join(VALID_BODY_FORTI_SWITCH)}")
    
    # Validate debug if present
    if 'debug' in payload:
        value = payload.get('debug')
        if value and value not in VALID_BODY_DEBUG:
            return (False, f"Invalid debug '{value}'. Must be one of: {', '.join(VALID_BODY_DEBUG)}")
