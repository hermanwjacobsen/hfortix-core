...
def validate_gui_display_put(
    ...
    if 'resolve-hosts' in payload:
        ...
        if value and value not in VALID_BODY_RESOLVE_HOSTS:
            return (False, f"Invalid resolve-hosts '{value}'. Must be one of: {', '.join(VALID_BODY_RESOLVE_HOSTS)}")
    ...
    if 'resolve-apps' in payload:
        ...
        if value and value not in VALID_BODY_RESOLVE_APPS:
            return (False, f"Invalid resolve-apps '{value}'. Must be one of: {', '.join(VALID_BODY_RESOLVE_APPS)}")
    ...
    if 'fortiview-unscanned-apps' in payload:
        ...
        if value and value not in VALID_BODY_FORTIVIEW_UNSCANNED_APPS:
            return (False, f"Invalid fortiview-unscanned-apps '{value}'. Must be one of: {', '.join(VALID_BODY_FORTIVIEW_UNSCANNED_APPS)}")
