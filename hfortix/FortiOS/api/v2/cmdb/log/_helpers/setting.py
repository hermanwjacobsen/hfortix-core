...
def validate_setting_put(
    ...
    if 'resolve-ip' in payload:
        ...
        if value and value not in VALID_BODY_RESOLVE_IP:
            return (False, f"Invalid resolve-ip '{value}'. Must be one of: {', '.join(VALID_BODY_RESOLVE_IP)}")
    ...
    if 'resolve-port' in payload:
        ...
        if value and value not in VALID_BODY_RESOLVE_PORT:
            return (False, f"Invalid resolve-port '{value}'. Must be one of: {', '.join(VALID_BODY_RESOLVE_PORT)}")
    ...
    if 'log-user-in-upper' in payload:
        ...
        if value and value not in VALID_BODY_LOG_USER_IN_UPPER:
            return (False, f"Invalid log-user-in-upper '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_USER_IN_UPPER)}")
    ...
    if 'fwpolicy-implicit-log' in payload:
        ...
        if value and value not in VALID_BODY_FWPOLICY_IMPLICIT_LOG:
            return (False, f"Invalid fwpolicy-implicit-log '{value}'. Must be one of: {', '.join(VALID_BODY_FWPOLICY_IMPLICIT_LOG)}")
    ...
    if 'fwpolicy6-implicit-log' in payload:
        ...
        if value and value not in VALID_BODY_FWPOLICY6_IMPLICIT_LOG:
            return (False, f"Invalid fwpolicy6-implicit-log '{value}'. Must be one of: {', '.join(VALID_BODY_FWPOLICY6_IMPLICIT_LOG)}")
    ...
    if 'local-in-allow' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_IN_ALLOW:
            return (False, f"Invalid local-in-allow '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_IN_ALLOW)}")
    ...
    if 'local-in-deny-unicast' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_IN_DENY_UNICAST:
            return (False, f"Invalid local-in-deny-unicast '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_IN_DENY_UNICAST)}")
    ...
    if 'local-in-deny-broadcast' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_IN_DENY_BROADCAST:
            return (False, f"Invalid local-in-deny-broadcast '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_IN_DENY_BROADCAST)}")
    ...
    if 'local-in-policy-log' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_IN_POLICY_LOG:
            return (False, f"Invalid local-in-policy-log '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_IN_POLICY_LOG)}")
    ...
    if 'local-out' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_OUT:
            return (False, f"Invalid local-out '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_OUT)}")
    ...
    if 'local-out-ioc-detection' in payload:
        ...
        if value and value not in VALID_BODY_LOCAL_OUT_IOC_DETECTION:
            return (False, f"Invalid local-out-ioc-detection '{value}'. Must be one of: {', '.join(VALID_BODY_LOCAL_OUT_IOC_DETECTION)}")
    ...
    if 'daemon-log' in payload:
        ...
        if value and value not in VALID_BODY_DAEMON_LOG:
            return (False, f"Invalid daemon-log '{value}'. Must be one of: {', '.join(VALID_BODY_DAEMON_LOG)}")
    ...
    if 'neighbor-event' in payload:
        ...
        if value and value not in VALID_BODY_NEIGHBOR_EVENT:
            return (False, f"Invalid neighbor-event '{value}'. Must be one of: {', '.join(VALID_BODY_NEIGHBOR_EVENT)}")
    ...
    if 'brief-traffic-format' in payload:
        ...
        if value and value not in VALID_BODY_BRIEF_TRAFFIC_FORMAT:
            return (False, f"Invalid brief-traffic-format '{value}'. Must be one of: {', '.join(VALID_BODY_BRIEF_TRAFFIC_FORMAT)}")
    ...
    if 'user-anonymize' in payload:
        ...
        if value and value not in VALID_BODY_USER_ANONYMIZE:
            return (False, f"Invalid user-anonymize '{value}'. Must be one of: {', '.join(VALID_BODY_USER_ANONYMIZE)}")
    ...
    if 'expolicy-implicit-log' in payload:
        ...
        if value and value not in VALID_BODY_EXPOLICY_IMPLICIT_LOG:
            return (False, f"Invalid expolicy-implicit-log '{value}'. Must be one of: {', '.join(VALID_BODY_EXPOLICY_IMPLICIT_LOG)}")
    ...
    if 'log-policy-comment' in payload:
        ...
        if value and value not in VALID_BODY_LOG_POLICY_COMMENT:
            return (False, f"Invalid log-policy-comment '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_POLICY_COMMENT)}")
    ...
    if 'faz-override' in payload:
        ...
        if value and value not in VALID_BODY_FAZ_OVERRIDE:
            return (False, f"Invalid faz-override '{value}'. Must be one of: {', '.join(VALID_BODY_FAZ_OVERRIDE)}")
    ...
    if 'syslog-override' in payload:
        ...
        if value and value not in VALID_BODY_SYSLOG_OVERRIDE:
            return (False, f"Invalid syslog-override '{value}'. Must be one of: {', '.join(VALID_BODY_SYSLOG_OVERRIDE)}")
    ...
    if 'rest-api-set' in payload:
        ...
        if value and value not in VALID_BODY_REST_API_SET:
            return (False, f"Invalid rest-api-set '{value}'. Must be one of: {', '.join(VALID_BODY_REST_API_SET)}")
    ...
    if 'rest-api-get' in payload:
        ...
        if value and value not in VALID_BODY_REST_API_GET:
            return (False, f"Invalid rest-api-get '{value}'. Must be one of: {', '.join(VALID_BODY_REST_API_GET)}")
    ...
    if 'rest-api-performance' in payload:
        ...
        if value and value not in VALID_BODY_REST_API_PERFORMANCE:
            return (False, f"Invalid rest-api-performance '{value}'. Must be one of: {', '.join(VALID_BODY_REST_API_PERFORMANCE)}")
    ...
    if 'long-live-session-stat' in payload:
        ...
        if value and value not in VALID_BODY_LONG_LIVE_SESSION_STAT:
            return (False, f"Invalid long-live-session-stat '{value}'. Must be one of: {', '.join(VALID_BODY_LONG_LIVE_SESSION_STAT)}")
    ...
    if 'extended-utm-log' in payload:
        ...
        if value and value not in VALID_BODY_EXTENDED_UTM_LOG:
            return (False, f"Invalid extended-utm-log '{value}'. Must be one of: {', '.join(VALID_BODY_EXTENDED_UTM_LOG)}")
    ...
    if 'zone-name' in payload:
        ...
        if value and value not in VALID_BODY_ZONE_NAME:
            return (False, f"Invalid zone-name '{value}'. Must be one of: {', '.join(VALID_BODY_ZONE_NAME)}")
    ...
    if 'web-svc-perf' in payload:
        ...
        if value and value not in VALID_BODY_WEB_SVC_PERF:
            return (False, f"Invalid web-svc-perf '{value}'. Must be one of: {', '.join(VALID_BODY_WEB_SVC_PERF)}")
