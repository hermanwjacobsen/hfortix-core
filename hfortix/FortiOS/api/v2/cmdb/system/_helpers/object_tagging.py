...
def validate_object_tagging_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'address' in payload:
        ...
        if value and value not in VALID_BODY_ADDRESS:
            return (False, f"Invalid address '{value}'. Must be one of: {', '.join(VALID_BODY_ADDRESS)}")
    
    # Validate device if present
    if 'device' in payload:
        value = payload.get('device')
        if value and value not in VALID_BODY_DEVICE:
            return (False, f"Invalid device '{value}'. Must be one of: {', '.join(VALID_BODY_DEVICE)}")
    
    # Validate interface if present
    if 'interface' in payload:
        value = payload.get('interface')
        if value and value not in VALID_BODY_INTERFACE:
            return (False, f"Invalid interface '{value}'. Must be one of: {', '.join(VALID_BODY_INTERFACE)}")
    ...
    if 'multiple' in payload:
        ...
        if value and value not in VALID_BODY_MULTIPLE:
            return (False, f"Invalid multiple '{value}'. Must be one of: {', '.join(VALID_BODY_MULTIPLE)}")
