...
VALID_BODY_NAME = ['ftp', 'tftp', 'ras', 'h323', 'tns', 'mms', 'sip', 'pptp', 'rtsp', 'dns-udp', 'dns-tcp', 'pmap', 'rsh', 'dcerpc', 'mgcp']
...
def validate_session_helper_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'name' in payload:
        ...
        if value and value not in VALID_BODY_NAME:
            return (False, f"Invalid name '{value}'. Must be one of: {', '.join(VALID_BODY_NAME)}")
...
def validate_session_helper_delete(id: str | None = None) -> tuple[bool, str | None]:
    """
