...
def validate_dhcp6_server_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'rapid-commit' in payload:
        ...
        if value and value not in VALID_BODY_RAPID_COMMIT:
            return (False, f"Invalid rapid-commit '{value}'. Must be one of: {', '.join(VALID_BODY_RAPID_COMMIT)}")
    ...
    if 'dns-search-list' in payload:
        ...
        if value and value not in VALID_BODY_DNS_SEARCH_LIST:
            return (False, f"Invalid dns-search-list '{value}'. Must be one of: {', '.join(VALID_BODY_DNS_SEARCH_LIST)}")
    ...
    if 'delegated-prefix-route' in payload:
        ...
        if value and value not in VALID_BODY_DELEGATED_PREFIX_ROUTE:
            return (False, f"Invalid delegated-prefix-route '{value}'. Must be one of: {', '.join(VALID_BODY_DELEGATED_PREFIX_ROUTE)}")
    ...
    if 'delegated-prefix-iaid' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"delegated-prefix-iaid must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"delegated-prefix-iaid must be numeric, got: {value}")
    ...
    if 'prefix-mode' in payload:
        ...
        if value and value not in VALID_BODY_PREFIX_MODE:
            return (False, f"Invalid prefix-mode '{value}'. Must be one of: {', '.join(VALID_BODY_PREFIX_MODE)}")
...
def validate_dhcp6_server_delete(id: str | None = None) -> tuple[bool, str | None]:
    """
