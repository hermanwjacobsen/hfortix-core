...
VALID_BODY_DDNS_SERVER = ['dyndns.org', 'dyns.net', 'tzo.com', 'vavic.com', 'dipdns.net', 'now.net.cn', 'dhs.org', 'easydns.com', 'genericDDNS', 'FortiGuardDDNS', 'noip.com']
...
def validate_ddns_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'ddns-server' in payload:
        ...
        if value and value not in VALID_BODY_DDNS_SERVER:
            return (False, f"Invalid ddns-server '{value}'. Must be one of: {', '.join(VALID_BODY_DDNS_SERVER)}")
    ...
    if 'use-public-ip' in payload:
        ...
        if value and value not in VALID_BODY_USE_PUBLIC_IP:
            return (False, f"Invalid use-public-ip '{value}'. Must be one of: {', '.join(VALID_BODY_USE_PUBLIC_IP)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 2592000:
                    return (False, f"update-interval must be between 60 and 2592000")
            except (ValueError, TypeError):
    ...
    if 'clear-text' in payload:
        ...
        if value and value not in VALID_BODY_CLEAR_TEXT:
            return (False, f"Invalid clear-text '{value}'. Must be one of: {', '.join(VALID_BODY_CLEAR_TEXT)}")
