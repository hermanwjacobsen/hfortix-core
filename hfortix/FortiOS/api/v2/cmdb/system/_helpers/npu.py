...
def validate_npu_put(
    ...
    if 'dedicated-management-cpu' in payload:
        ...
        if value and value not in VALID_BODY_DEDICATED_MANAGEMENT_CPU:
            return (False, f"Invalid dedicated-management-cpu '{value}'. Must be one of: {', '.join(VALID_BODY_DEDICATED_MANAGEMENT_CPU)}")
    ...
    if 'dedicated-management-affinity' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 79:
            return (False, f"dedicated-management-affinity cannot exceed 79 characters")
    ...
    if 'capwap-offload' in payload:
        ...
        if value and value not in VALID_BODY_CAPWAP_OFFLOAD:
            return (False, f"Invalid capwap-offload '{value}'. Must be one of: {', '.join(VALID_BODY_CAPWAP_OFFLOAD)}")
    ...
    if 'ipsec-mtu-override' in payload:
        ...
        if value and value not in VALID_BODY_IPSEC_MTU_OVERRIDE:
            return (False, f"Invalid ipsec-mtu-override '{value}'. Must be one of: {', '.join(VALID_BODY_IPSEC_MTU_OVERRIDE)}")
    ...
    if 'ipsec-ordering' in payload:
        ...
        if value and value not in VALID_BODY_IPSEC_ORDERING:
            return (False, f"Invalid ipsec-ordering '{value}'. Must be one of: {', '.join(VALID_BODY_IPSEC_ORDERING)}")
