...
def validate_vdom_link_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'vcluster' in payload:
        ...
        if value and value not in VALID_BODY_VCLUSTER:
            return (False, f"Invalid vcluster '{value}'. Must be one of: {', '.join(VALID_BODY_VCLUSTER)}")
...
def validate_vdom_link_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
