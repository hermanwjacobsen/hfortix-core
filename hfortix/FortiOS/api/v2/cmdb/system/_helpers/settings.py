...
VALID_BODY_FIREWALL_SESSION_DIRTY = ['check-all', 'check-new', 'check-policy-option']
...
VALID_BODY_V4_ECMP_MODE = ['source-ip-based', 'weight-based', 'usage-based', 'source-dest-ip-based']
...
def validate_settings_put(
    ...
    if 'vdom-type' in payload:
        ...
        if value and value not in VALID_BODY_VDOM_TYPE:
            return (False, f"Invalid vdom-type '{value}'. Must be one of: {', '.join(VALID_BODY_VDOM_TYPE)}")
    ...
    if 'lan-extension-controller-addr' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"lan-extension-controller-addr cannot exceed 255 characters")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1024 or int_val > 65535:
                    return (False, f"lan-extension-controller-port must be between 1024 and 65535")
            except (ValueError, TypeError):
                return (False, f"lan-extension-controller-port must be numeric, got: {value}")
    
    # Validate opmode if present
    if 'opmode' in payload:
        value = payload.get('opmode')
        if value and value not in VALID_BODY_OPMODE:
            return (False, f"Invalid opmode '{value}'. Must be one of: {', '.join(VALID_BODY_OPMODE)}")
    ...
    if 'ngfw-mode' in payload:
        ...
        if value and value not in VALID_BODY_NGFW_MODE:
            return (False, f"Invalid ngfw-mode '{value}'. Must be one of: {', '.join(VALID_BODY_NGFW_MODE)}")
    ...
    if 'http-external-dest' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_EXTERNAL_DEST:
            return (False, f"Invalid http-external-dest '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_EXTERNAL_DEST)}")
    ...
    if 'firewall-session-dirty' in payload:
        ...
        if value and value not in VALID_BODY_FIREWALL_SESSION_DIRTY:
            return (False, f"Invalid firewall-session-dirty '{value}'. Must be one of: {', '.join(VALID_BODY_FIREWALL_SESSION_DIRTY)}")
    ...
    if 'bfd-dont-enforce-src-port' in payload:
        ...
        if value and value not in VALID_BODY_BFD_DONT_ENFORCE_SRC_PORT:
            return (False, f"Invalid bfd-dont-enforce-src-port '{value}'. Must be one of: {', '.join(VALID_BODY_BFD_DONT_ENFORCE_SRC_PORT)}")
    ...
    if 'utf8-spam-tagging' in payload:
        ...
        if value and value not in VALID_BODY_UTF8_SPAM_TAGGING:
            return (False, f"Invalid utf8-spam-tagging '{value}'. Must be one of: {', '.join(VALID_BODY_UTF8_SPAM_TAGGING)}")
    ...
    if 'wccp-cache-engine' in payload:
        ...
        if value and value not in VALID_BODY_WCCP_CACHE_ENGINE:
            return (False, f"Invalid wccp-cache-engine '{value}'. Must be one of: {', '.join(VALID_BODY_WCCP_CACHE_ENGINE)}")
    ...
    if 'vpn-stats-log' in payload:
        ...
        if value and value not in VALID_BODY_VPN_STATS_LOG:
            return (False, f"Invalid vpn-stats-log '{value}'. Must be one of: {', '.join(VALID_BODY_VPN_STATS_LOG)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"vpn-stats-period must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"vpn-stats-period must be numeric, got: {value}")
    ...
    if 'v4-ecmp-mode' in payload:
        ...
        if value and value not in VALID_BODY_V4_ECMP_MODE:
            return (False, f"Invalid v4-ecmp-mode '{value}'. Must be one of: {', '.join(VALID_BODY_V4_ECMP_MODE)}")
    ...
    if 'fw-session-hairpin' in payload:
        ...
        if value and value not in VALID_BODY_FW_SESSION_HAIRPIN:
            return (False, f"Invalid fw-session-hairpin '{value}'. Must be one of: {', '.join(VALID_BODY_FW_SESSION_HAIRPIN)}")
    ...
    if 'prp-trailer-action' in payload:
        ...
        if value and value not in VALID_BODY_PRP_TRAILER_ACTION:
            return (False, f"Invalid prp-trailer-action '{value}'. Must be one of: {', '.join(VALID_BODY_PRP_TRAILER_ACTION)}")
    ...
    if 'snat-hairpin-traffic' in payload:
        ...
        if value and value not in VALID_BODY_SNAT_HAIRPIN_TRAFFIC:
            return (False, f"Invalid snat-hairpin-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_SNAT_HAIRPIN_TRAFFIC)}")
    ...
    if 'dhcp-proxy' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_PROXY:
            return (False, f"Invalid dhcp-proxy '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_PROXY)}")
    ...
    if 'dhcp-proxy-interface-select-method' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_PROXY_INTERFACE_SELECT_METHOD:
            return (False, f"Invalid dhcp-proxy-interface-select-method '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_PROXY_INTERFACE_SELECT_METHOD)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 511:
                    return (False, f"dhcp-proxy-vrf-select must be between 0 and 511")
            except (ValueError, TypeError):
                return (False, f"dhcp-proxy-vrf-select must be numeric, got: {value}")
    ...
    if 'central-nat' in payload:
        ...
        if value and value not in VALID_BODY_CENTRAL_NAT:
            return (False, f"Invalid central-nat '{value}'. Must be one of: {', '.join(VALID_BODY_CENTRAL_NAT)}")
    ...
    if 'link-down-access' in payload:
        ...
        if value and value not in VALID_BODY_LINK_DOWN_ACCESS:
            return (False, f"Invalid link-down-access '{value}'. Must be one of: {', '.join(VALID_BODY_LINK_DOWN_ACCESS)}")
    ...
    if 'nat46-generate-ipv6-fragment-header' in payload:
        ...
        if value and value not in VALID_BODY_NAT46_GENERATE_IPV6_FRAGMENT_HEADER:
            return (False, f"Invalid nat46-generate-ipv6-fragment-header '{value}'. Must be one of: {', '.join(VALID_BODY_NAT46_GENERATE_IPV6_FRAGMENT_HEADER)}")
    ...
    if 'nat46-force-ipv4-packet-forwarding' in payload:
        ...
        if value and value not in VALID_BODY_NAT46_FORCE_IPV4_PACKET_FORWARDING:
            return (False, f"Invalid nat46-force-ipv4-packet-forwarding '{value}'. Must be one of: {', '.join(VALID_BODY_NAT46_FORCE_IPV4_PACKET_FORWARDING)}")
    ...
    if 'nat64-force-ipv6-packet-forwarding' in payload:
        ...
        if value and value not in VALID_BODY_NAT64_FORCE_IPV6_PACKET_FORWARDING:
            return (False, f"Invalid nat64-force-ipv6-packet-forwarding '{value}'. Must be one of: {', '.join(VALID_BODY_NAT64_FORCE_IPV6_PACKET_FORWARDING)}")
    ...
    if 'detect-unknown-esp' in payload:
        ...
        if value and value not in VALID_BODY_DETECT_UNKNOWN_ESP:
            return (False, f"Invalid detect-unknown-esp '{value}'. Must be one of: {', '.join(VALID_BODY_DETECT_UNKNOWN_ESP)}")
    ...
    if 'intree-ses-best-route' in payload:
        ...
        if value and value not in VALID_BODY_INTREE_SES_BEST_ROUTE:
            return (False, f"Invalid intree-ses-best-route '{value}'. Must be one of: {', '.join(VALID_BODY_INTREE_SES_BEST_ROUTE)}")
    ...
    if 'auxiliary-session' in payload:
        ...
        if value and value not in VALID_BODY_AUXILIARY_SESSION:
            return (False, f"Invalid auxiliary-session '{value}'. Must be one of: {', '.join(VALID_BODY_AUXILIARY_SESSION)}")
    ...
    if 'asymroute' in payload:
        ...
        if value and value not in VALID_BODY_ASYMROUTE:
            return (False, f"Invalid asymroute '{value}'. Must be one of: {', '.join(VALID_BODY_ASYMROUTE)}")
    ...
    if 'asymroute-icmp' in payload:
        ...
        if value and value not in VALID_BODY_ASYMROUTE_ICMP:
            return (False, f"Invalid asymroute-icmp '{value}'. Must be one of: {', '.join(VALID_BODY_ASYMROUTE_ICMP)}")
    ...
    if 'tcp-session-without-syn' in payload:
        ...
        if value and value not in VALID_BODY_TCP_SESSION_WITHOUT_SYN:
            return (False, f"Invalid tcp-session-without-syn '{value}'. Must be one of: {', '.join(VALID_BODY_TCP_SESSION_WITHOUT_SYN)}")
    ...
    if 'ses-denied-traffic' in payload:
        ...
        if value and value not in VALID_BODY_SES_DENIED_TRAFFIC:
            return (False, f"Invalid ses-denied-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_SES_DENIED_TRAFFIC)}")
    ...
    if 'ses-denied-multicast-traffic' in payload:
        ...
        if value and value not in VALID_BODY_SES_DENIED_MULTICAST_TRAFFIC:
            return (False, f"Invalid ses-denied-multicast-traffic '{value}'. Must be one of: {', '.join(VALID_BODY_SES_DENIED_MULTICAST_TRAFFIC)}")
    ...
    if 'strict-src-check' in payload:
        ...
        if value and value not in VALID_BODY_STRICT_SRC_CHECK:
            return (False, f"Invalid strict-src-check '{value}'. Must be one of: {', '.join(VALID_BODY_STRICT_SRC_CHECK)}")
    ...
    if 'allow-linkdown-path' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_LINKDOWN_PATH:
            return (False, f"Invalid allow-linkdown-path '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_LINKDOWN_PATH)}")
    ...
    if 'asymroute6' in payload:
        ...
        if value and value not in VALID_BODY_ASYMROUTE6:
            return (False, f"Invalid asymroute6 '{value}'. Must be one of: {', '.join(VALID_BODY_ASYMROUTE6)}")
    ...
    if 'asymroute6-icmp' in payload:
        ...
        if value and value not in VALID_BODY_ASYMROUTE6_ICMP:
            return (False, f"Invalid asymroute6-icmp '{value}'. Must be one of: {', '.join(VALID_BODY_ASYMROUTE6_ICMP)}")
    ...
    if 'sctp-session-without-init' in payload:
        ...
        if value and value not in VALID_BODY_SCTP_SESSION_WITHOUT_INIT:
            return (False, f"Invalid sctp-session-without-init '{value}'. Must be one of: {', '.join(VALID_BODY_SCTP_SESSION_WITHOUT_INIT)}")
    ...
    if 'sip-expectation' in payload:
        ...
        if value and value not in VALID_BODY_SIP_EXPECTATION:
            return (False, f"Invalid sip-expectation '{value}'. Must be one of: {', '.join(VALID_BODY_SIP_EXPECTATION)}")
    ...
    if 'sip-nat-trace' in payload:
        ...
        if value and value not in VALID_BODY_SIP_NAT_TRACE:
            return (False, f"Invalid sip-nat-trace '{value}'. Must be one of: {', '.join(VALID_BODY_SIP_NAT_TRACE)}")
    ...
    if 'h323-direct-model' in payload:
        ...
        if value and value not in VALID_BODY_H323_DIRECT_MODEL:
            return (False, f"Invalid h323-direct-model '{value}'. Must be one of: {', '.join(VALID_BODY_H323_DIRECT_MODEL)}")
    ...
    if 'multicast-forward' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_FORWARD:
            return (False, f"Invalid multicast-forward '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_FORWARD)}")
    ...
    if 'multicast-ttl-notchange' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_TTL_NOTCHANGE:
            return (False, f"Invalid multicast-ttl-notchange '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_TTL_NOTCHANGE)}")
    ...
    if 'multicast-skip-policy' in payload:
        ...
        if value and value not in VALID_BODY_MULTICAST_SKIP_POLICY:
            return (False, f"Invalid multicast-skip-policy '{value}'. Must be one of: {', '.join(VALID_BODY_MULTICAST_SKIP_POLICY)}")
    ...
    if 'allow-subnet-overlap' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_SUBNET_OVERLAP:
            return (False, f"Invalid allow-subnet-overlap '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_SUBNET_OVERLAP)}")
    ...
    if 'deny-tcp-with-icmp' in payload:
        ...
        if value and value not in VALID_BODY_DENY_TCP_WITH_ICMP:
            return (False, f"Invalid deny-tcp-with-icmp '{value}'. Must be one of: {', '.join(VALID_BODY_DENY_TCP_WITH_ICMP)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 365:
                    return (False, f"discovered-device-timeout must be between 1 and 365")
            except (ValueError, TypeError):
                return (False, f"discovered-device-timeout must be numeric, got: {value}")
    ...
    if 'email-portal-check-dns' in payload:
        ...
        if value and value not in VALID_BODY_EMAIL_PORTAL_CHECK_DNS:
            return (False, f"Invalid email-portal-check-dns '{value}'. Must be one of: {', '.join(VALID_BODY_EMAIL_PORTAL_CHECK_DNS)}")
    ...
    if 'default-voip-alg-mode' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_VOIP_ALG_MODE:
            return (False, f"Invalid default-voip-alg-mode '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_VOIP_ALG_MODE)}")
    ...
    if 'gui-icap' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ICAP:
            return (False, f"Invalid gui-icap '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ICAP)}")
    ...
    if 'gui-implicit-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_IMPLICIT_POLICY:
            return (False, f"Invalid gui-implicit-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_IMPLICIT_POLICY)}")
    ...
    if 'gui-dns-database' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DNS_DATABASE:
            return (False, f"Invalid gui-dns-database '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DNS_DATABASE)}")
    ...
    if 'gui-load-balance' in payload:
        ...
        if value and value not in VALID_BODY_GUI_LOAD_BALANCE:
            return (False, f"Invalid gui-load-balance '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_LOAD_BALANCE)}")
    ...
    if 'gui-multicast-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_MULTICAST_POLICY:
            return (False, f"Invalid gui-multicast-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_MULTICAST_POLICY)}")
    ...
    if 'gui-dos-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DOS_POLICY:
            return (False, f"Invalid gui-dos-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DOS_POLICY)}")
    ...
    if 'gui-object-colors' in payload:
        ...
        if value and value not in VALID_BODY_GUI_OBJECT_COLORS:
            return (False, f"Invalid gui-object-colors '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_OBJECT_COLORS)}")
    ...
    if 'gui-route-tag-address-creation' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ROUTE_TAG_ADDRESS_CREATION:
            return (False, f"Invalid gui-route-tag-address-creation '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ROUTE_TAG_ADDRESS_CREATION)}")
    ...
    if 'gui-voip-profile' in payload:
        ...
        if value and value not in VALID_BODY_GUI_VOIP_PROFILE:
            return (False, f"Invalid gui-voip-profile '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_VOIP_PROFILE)}")
    ...
    if 'gui-ap-profile' in payload:
        ...
        if value and value not in VALID_BODY_GUI_AP_PROFILE:
            return (False, f"Invalid gui-ap-profile '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_AP_PROFILE)}")
    ...
    if 'gui-security-profile-group' in payload:
        ...
        if value and value not in VALID_BODY_GUI_SECURITY_PROFILE_GROUP:
            return (False, f"Invalid gui-security-profile-group '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_SECURITY_PROFILE_GROUP)}")
    ...
    if 'gui-local-in-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_LOCAL_IN_POLICY:
            return (False, f"Invalid gui-local-in-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_LOCAL_IN_POLICY)}")
    ...
    if 'gui-explicit-proxy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_EXPLICIT_PROXY:
            return (False, f"Invalid gui-explicit-proxy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_EXPLICIT_PROXY)}")
    ...
    if 'gui-dynamic-routing' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DYNAMIC_ROUTING:
            return (False, f"Invalid gui-dynamic-routing '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DYNAMIC_ROUTING)}")
    ...
    if 'gui-policy-based-ipsec' in payload:
        ...
        if value and value not in VALID_BODY_GUI_POLICY_BASED_IPSEC:
            return (False, f"Invalid gui-policy-based-ipsec '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_POLICY_BASED_IPSEC)}")
    ...
    if 'gui-threat-weight' in payload:
        ...
        if value and value not in VALID_BODY_GUI_THREAT_WEIGHT:
            return (False, f"Invalid gui-threat-weight '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_THREAT_WEIGHT)}")
    ...
    if 'gui-spamfilter' in payload:
        ...
        if value and value not in VALID_BODY_GUI_SPAMFILTER:
            return (False, f"Invalid gui-spamfilter '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_SPAMFILTER)}")
    ...
    if 'gui-file-filter' in payload:
        ...
        if value and value not in VALID_BODY_GUI_FILE_FILTER:
            return (False, f"Invalid gui-file-filter '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_FILE_FILTER)}")
    ...
    if 'gui-application-control' in payload:
        ...
        if value and value not in VALID_BODY_GUI_APPLICATION_CONTROL:
            return (False, f"Invalid gui-application-control '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_APPLICATION_CONTROL)}")
    
    # Validate gui-ips if present
    if 'gui-ips' in payload:
        value = payload.get('gui-ips')
        if value and value not in VALID_BODY_GUI_IPS:
            return (False, f"Invalid gui-ips '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_IPS)}")
    ...
    if 'gui-dhcp-advanced' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DHCP_ADVANCED:
            return (False, f"Invalid gui-dhcp-advanced '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DHCP_ADVANCED)}")
    
    # Validate gui-vpn if present
    if 'gui-vpn' in payload:
        value = payload.get('gui-vpn')
        if value and value not in VALID_BODY_GUI_VPN:
            return (False, f"Invalid gui-vpn '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_VPN)}")
    ...
    if 'gui-wireless-controller' in payload:
        ...
        if value and value not in VALID_BODY_GUI_WIRELESS_CONTROLLER:
            return (False, f"Invalid gui-wireless-controller '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_WIRELESS_CONTROLLER)}")
    ...
    if 'gui-advanced-wireless-features' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ADVANCED_WIRELESS_FEATURES:
            return (False, f"Invalid gui-advanced-wireless-features '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ADVANCED_WIRELESS_FEATURES)}")
    ...
    if 'gui-switch-controller' in payload:
        ...
        if value and value not in VALID_BODY_GUI_SWITCH_CONTROLLER:
            return (False, f"Invalid gui-switch-controller '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_SWITCH_CONTROLLER)}")
    ...
    if 'gui-fortiap-split-tunneling' in payload:
        ...
        if value and value not in VALID_BODY_GUI_FORTIAP_SPLIT_TUNNELING:
            return (False, f"Invalid gui-fortiap-split-tunneling '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_FORTIAP_SPLIT_TUNNELING)}")
    ...
    if 'gui-webfilter-advanced' in payload:
        ...
        if value and value not in VALID_BODY_GUI_WEBFILTER_ADVANCED:
            return (False, f"Invalid gui-webfilter-advanced '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_WEBFILTER_ADVANCED)}")
    ...
    if 'gui-traffic-shaping' in payload:
        ...
        if value and value not in VALID_BODY_GUI_TRAFFIC_SHAPING:
            return (False, f"Invalid gui-traffic-shaping '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_TRAFFIC_SHAPING)}")
    ...
    if 'gui-wan-load-balancing' in payload:
        ...
        if value and value not in VALID_BODY_GUI_WAN_LOAD_BALANCING:
            return (False, f"Invalid gui-wan-load-balancing '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_WAN_LOAD_BALANCING)}")
    ...
    if 'gui-antivirus' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ANTIVIRUS:
            return (False, f"Invalid gui-antivirus '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ANTIVIRUS)}")
    ...
    if 'gui-webfilter' in payload:
        ...
        if value and value not in VALID_BODY_GUI_WEBFILTER:
            return (False, f"Invalid gui-webfilter '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_WEBFILTER)}")
    ...
    if 'gui-videofilter' in payload:
        ...
        if value and value not in VALID_BODY_GUI_VIDEOFILTER:
            return (False, f"Invalid gui-videofilter '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_VIDEOFILTER)}")
    ...
    if 'gui-dnsfilter' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DNSFILTER:
            return (False, f"Invalid gui-dnsfilter '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DNSFILTER)}")
    ...
    if 'gui-waf-profile' in payload:
        ...
        if value and value not in VALID_BODY_GUI_WAF_PROFILE:
            return (False, f"Invalid gui-waf-profile '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_WAF_PROFILE)}")
    ...
    if 'gui-dlp-profile' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DLP_PROFILE:
            return (False, f"Invalid gui-dlp-profile '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DLP_PROFILE)}")
    ...
    if 'gui-dlp-advanced' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DLP_ADVANCED:
            return (False, f"Invalid gui-dlp-advanced '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DLP_ADVANCED)}")
    ...
    if 'gui-virtual-patch-profile' in payload:
        ...
        if value and value not in VALID_BODY_GUI_VIRTUAL_PATCH_PROFILE:
            return (False, f"Invalid gui-virtual-patch-profile '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_VIRTUAL_PATCH_PROFILE)}")
    ...
    if 'gui-casb' in payload:
        ...
        if value and value not in VALID_BODY_GUI_CASB:
            return (False, f"Invalid gui-casb '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_CASB)}")
    ...
    if 'gui-fortiextender-controller' in payload:
        ...
        if value and value not in VALID_BODY_GUI_FORTIEXTENDER_CONTROLLER:
            return (False, f"Invalid gui-fortiextender-controller '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_FORTIEXTENDER_CONTROLLER)}")
    ...
    if 'gui-advanced-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ADVANCED_POLICY:
            return (False, f"Invalid gui-advanced-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ADVANCED_POLICY)}")
    ...
    if 'gui-allow-unnamed-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ALLOW_UNNAMED_POLICY:
            return (False, f"Invalid gui-allow-unnamed-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ALLOW_UNNAMED_POLICY)}")
    ...
    if 'gui-email-collection' in payload:
        ...
        if value and value not in VALID_BODY_GUI_EMAIL_COLLECTION:
            return (False, f"Invalid gui-email-collection '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_EMAIL_COLLECTION)}")
    ...
    if 'gui-multiple-interface-policy' in payload:
        ...
        if value and value not in VALID_BODY_GUI_MULTIPLE_INTERFACE_POLICY:
            return (False, f"Invalid gui-multiple-interface-policy '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_MULTIPLE_INTERFACE_POLICY)}")
    ...
    if 'gui-policy-disclaimer' in payload:
        ...
        if value and value not in VALID_BODY_GUI_POLICY_DISCLAIMER:
            return (False, f"Invalid gui-policy-disclaimer '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_POLICY_DISCLAIMER)}")
    ...
    if 'gui-ztna' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ZTNA:
            return (False, f"Invalid gui-ztna '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ZTNA)}")
    
    # Validate gui-ot if present
    if 'gui-ot' in payload:
        value = payload.get('gui-ot')
        if value and value not in VALID_BODY_GUI_OT:
            return (False, f"Invalid gui-ot '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_OT)}")
    ...
    if 'gui-dynamic-device-os-id' in payload:
        ...
        if value and value not in VALID_BODY_GUI_DYNAMIC_DEVICE_OS_ID:
            return (False, f"Invalid gui-dynamic-device-os-id '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_DYNAMIC_DEVICE_OS_ID)}")
    ...
    if 'ike-session-resume' in payload:
        ...
        if value and value not in VALID_BODY_IKE_SESSION_RESUME:
            return (False, f"Invalid ike-session-resume '{value}'. Must be one of: {', '.join(VALID_BODY_IKE_SESSION_RESUME)}")
    ...
    if 'ike-quick-crash-detect' in payload:
        ...
        if value and value not in VALID_BODY_IKE_QUICK_CRASH_DETECT:
            return (False, f"Invalid ike-quick-crash-detect '{value}'. Must be one of: {', '.join(VALID_BODY_IKE_QUICK_CRASH_DETECT)}")
    ...
    if 'ike-dn-format' in payload:
        ...
        if value and value not in VALID_BODY_IKE_DN_FORMAT:
            return (False, f"Invalid ike-dn-format '{value}'. Must be one of: {', '.join(VALID_BODY_IKE_DN_FORMAT)}")
    ...
    if 'ike-policy-route' in payload:
        ...
        if value and value not in VALID_BODY_IKE_POLICY_ROUTE:
            return (False, f"Invalid ike-policy-route '{value}'. Must be one of: {', '.join(VALID_BODY_IKE_POLICY_ROUTE)}")
    ...
    if 'ike-detailed-event-logs' in payload:
        ...
        if value and value not in VALID_BODY_IKE_DETAILED_EVENT_LOGS:
            return (False, f"Invalid ike-detailed-event-logs '{value}'. Must be one of: {', '.join(VALID_BODY_IKE_DETAILED_EVENT_LOGS)}")
    ...
    if 'block-land-attack' in payload:
        ...
        if value and value not in VALID_BODY_BLOCK_LAND_ATTACK:
            return (False, f"Invalid block-land-attack '{value}'. Must be one of: {', '.join(VALID_BODY_BLOCK_LAND_ATTACK)}")
    ...
    if 'default-app-port-as-service' in payload:
        ...
        if value and value not in VALID_BODY_DEFAULT_APP_PORT_AS_SERVICE:
            return (False, f"Invalid default-app-port-as-service '{value}'. Must be one of: {', '.join(VALID_BODY_DEFAULT_APP_PORT_AS_SERVICE)}")
    ...
    if 'fqdn-session-check' in payload:
        ...
        if value and value not in VALID_BODY_FQDN_SESSION_CHECK:
            return (False, f"Invalid fqdn-session-check '{value}'. Must be one of: {', '.join(VALID_BODY_FQDN_SESSION_CHECK)}")
    ...
    if 'ext-resource-session-check' in payload:
        ...
        if value and value not in VALID_BODY_EXT_RESOURCE_SESSION_CHECK:
            return (False, f"Invalid ext-resource-session-check '{value}'. Must be one of: {', '.join(VALID_BODY_EXT_RESOURCE_SESSION_CHECK)}")
    ...
    if 'dyn-addr-session-check' in payload:
        ...
        if value and value not in VALID_BODY_DYN_ADDR_SESSION_CHECK:
            return (False, f"Invalid dyn-addr-session-check '{value}'. Must be one of: {', '.join(VALID_BODY_DYN_ADDR_SESSION_CHECK)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 365:
                    return (False, f"default-policy-expiry-days must be between 0 and 365")
            except (ValueError, TypeError):
                return (False, f"default-policy-expiry-days must be numeric, got: {value}")
    ...
    if 'gui-enforce-change-summary' in payload:
        ...
        if value and value not in VALID_BODY_GUI_ENFORCE_CHANGE_SUMMARY:
            return (False, f"Invalid gui-enforce-change-summary '{value}'. Must be one of: {', '.join(VALID_BODY_GUI_ENFORCE_CHANGE_SUMMARY)}")
    ...
    if 'internet-service-database-cache' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE_DATABASE_CACHE:
            return (False, f"Invalid internet-service-database-cache '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE_DATABASE_CACHE)}")
    ...
    if 'internet-service-app-ctrl-size' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"internet-service-app-ctrl-size must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"internet-service-app-ctrl-size must be numeric, got: {value}")
