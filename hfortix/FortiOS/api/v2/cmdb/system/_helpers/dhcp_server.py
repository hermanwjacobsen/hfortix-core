...
def validate_dhcp_server_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 300 or int_val > 8640000:
                    return (False, f"lease-time must be between 300 and 8640000")
            except (ValueError, TypeError):
    ...
    if 'mac-acl-default-action' in payload:
        ...
        if value and value not in VALID_BODY_MAC_ACL_DEFAULT_ACTION:
            return (False, f"Invalid mac-acl-default-action '{value}'. Must be one of: {', '.join(VALID_BODY_MAC_ACL_DEFAULT_ACTION)}")
    ...
    if 'forticlient-on-net-status' in payload:
        ...
        if value and value not in VALID_BODY_FORTICLIENT_ON_NET_STATUS:
            return (False, f"Invalid forticlient-on-net-status '{value}'. Must be one of: {', '.join(VALID_BODY_FORTICLIENT_ON_NET_STATUS)}")
    ...
    if 'dns-service' in payload:
        ...
        if value and value not in VALID_BODY_DNS_SERVICE:
            return (False, f"Invalid dns-service '{value}'. Must be one of: {', '.join(VALID_BODY_DNS_SERVICE)}")
    ...
    if 'wifi-ac-service' in payload:
        ...
        if value and value not in VALID_BODY_WIFI_AC_SERVICE:
            return (False, f"Invalid wifi-ac-service '{value}'. Must be one of: {', '.join(VALID_BODY_WIFI_AC_SERVICE)}")
    ...
    if 'ntp-service' in payload:
        ...
        if value and value not in VALID_BODY_NTP_SERVICE:
            return (False, f"Invalid ntp-service '{value}'. Must be one of: {', '.join(VALID_BODY_NTP_SERVICE)}")
    ...
    if 'timezone-option' in payload:
        ...
        if value and value not in VALID_BODY_TIMEZONE_OPTION:
            return (False, f"Invalid timezone-option '{value}'. Must be one of: {', '.join(VALID_BODY_TIMEZONE_OPTION)}")
    ...
    if 'ip-mode' in payload:
        ...
        if value and value not in VALID_BODY_IP_MODE:
            return (False, f"Invalid ip-mode '{value}'. Must be one of: {', '.join(VALID_BODY_IP_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 8640000:
                    return (False, f"conflicted-ip-timeout must be between 60 and 8640000")
            except (ValueError, TypeError):
                return (False, f"conflicted-ip-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 8640000:
                    return (False, f"ipsec-lease-hold must be between 0 and 8640000")
            except (ValueError, TypeError):
                return (False, f"ipsec-lease-hold must be numeric, got: {value}")
    ...
    if 'auto-configuration' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_CONFIGURATION:
            return (False, f"Invalid auto-configuration '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_CONFIGURATION)}")
    ...
    if 'dhcp-settings-from-fortiipam' in payload:
        ...
        if value and value not in VALID_BODY_DHCP_SETTINGS_FROM_FORTIIPAM:
            return (False, f"Invalid dhcp-settings-from-fortiipam '{value}'. Must be one of: {', '.join(VALID_BODY_DHCP_SETTINGS_FROM_FORTIIPAM)}")
    ...
    if 'auto-managed-status' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_MANAGED_STATUS:
            return (False, f"Invalid auto-managed-status '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_MANAGED_STATUS)}")
    ...
    if 'ddns-update' in payload:
        ...
        if value and value not in VALID_BODY_DDNS_UPDATE:
            return (False, f"Invalid ddns-update '{value}'. Must be one of: {', '.join(VALID_BODY_DDNS_UPDATE)}")
    ...
    if 'ddns-update-override' in payload:
        ...
        if value and value not in VALID_BODY_DDNS_UPDATE_OVERRIDE:
            return (False, f"Invalid ddns-update-override '{value}'. Must be one of: {', '.join(VALID_BODY_DDNS_UPDATE_OVERRIDE)}")
    ...
    if 'ddns-auth' in payload:
        ...
        if value and value not in VALID_BODY_DDNS_AUTH:
            return (False, f"Invalid ddns-auth '{value}'. Must be one of: {', '.join(VALID_BODY_DDNS_AUTH)}")
    ...
    if 'vci-match' in payload:
        ...
        if value and value not in VALID_BODY_VCI_MATCH:
            return (False, f"Invalid vci-match '{value}'. Must be one of: {', '.join(VALID_BODY_VCI_MATCH)}")
    ...
    if 'shared-subnet' in payload:
        ...
        if value and value not in VALID_BODY_SHARED_SUBNET:
            return (False, f"Invalid shared-subnet '{value}'. Must be one of: {', '.join(VALID_BODY_SHARED_SUBNET)}")
...
def validate_dhcp_server_delete(id: str | None = None) -> tuple[bool, str | None]:
    """
