...
def validate_standalone_cluster_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 255:
                    return (False, f"standalone-group-id must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"standalone-group-id must be numeric, got: {value}")
    ...
    if 'group-member-id' in payload:
        ...
        if value is not None:
            try:
                ...
                return (False, f"group-member-id must be numeric, got: {value}")
    ...
    if 'layer2-connection' in payload:
        ...
        if value and value not in VALID_BODY_LAYER2_CONNECTION:
            return (False, f"Invalid layer2-connection '{value}'. Must be one of: {', '.join(VALID_BODY_LAYER2_CONNECTION)}")
    ...
    if 'asymmetric-traffic-control' in payload:
        ...
        if value and value not in VALID_BODY_ASYMMETRIC_TRAFFIC_CONTROL:
            return (False, f"Invalid asymmetric-traffic-control '{value}'. Must be one of: {', '.join(VALID_BODY_ASYMMETRIC_TRAFFIC_CONTROL)}")
    ...
    if 'helper-traffic-bounce' in payload:
        ...
        if value and value not in VALID_BODY_HELPER_TRAFFIC_BOUNCE:
            return (False, f"Invalid helper-traffic-bounce '{value}'. Must be one of: {', '.join(VALID_BODY_HELPER_TRAFFIC_BOUNCE)}")
    ...
    if 'utm-traffic-bounce' in payload:
        ...
        if value and value not in VALID_BODY_UTM_TRAFFIC_BOUNCE:
            return (False, f"Invalid utm-traffic-bounce '{value}'. Must be one of: {', '.join(VALID_BODY_UTM_TRAFFIC_BOUNCE)}")
