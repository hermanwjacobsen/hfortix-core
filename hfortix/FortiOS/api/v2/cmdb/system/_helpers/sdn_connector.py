...
VALID_BODY_TYPE = ['aci', 'alicloud', 'aws', 'azure', 'gcp', 'nsx', 'nuage', 'oci', 'openstack', 'kubernetes', 'vmware', 'sepm', 'aci-direct', 'ibm', 'nutanix', 'sap']
...
VALID_BODY_IBM_REGION = ['dallas', 'washington-dc', 'london', 'frankfurt', 'sydney', 'tokyo', 'osaka', 'toronto', 'sao-paulo', 'madrid']
...
def validate_sdn_connector_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'use-metadata-iam' in payload:
        ...
        if value and value not in VALID_BODY_USE_METADATA_IAM:
            return (False, f"Invalid use-metadata-iam '{value}'. Must be one of: {', '.join(VALID_BODY_USE_METADATA_IAM)}")
    ...
    if 'microsoft-365' in payload:
        ...
        if value and value not in VALID_BODY_MICROSOFT_365:
            return (False, f"Invalid microsoft-365 '{value}'. Must be one of: {', '.join(VALID_BODY_MICROSOFT_365)}")
    ...
    if 'ha-status' in payload:
        ...
        if value and value not in VALID_BODY_HA_STATUS:
            return (False, f"Invalid ha-status '{value}'. Must be one of: {', '.join(VALID_BODY_HA_STATUS)}")
    ...
    if 'verify-certificate' in payload:
        ...
        if value and value not in VALID_BODY_VERIFY_CERTIFICATE:
            return (False, f"Invalid verify-certificate '{value}'. Must be one of: {', '.join(VALID_BODY_VERIFY_CERTIFICATE)}")
    ...
    if 'message-server-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"message-server-port must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"message-server-port must be numeric, got: {value}")
    ...
    if 'alt-resource-ip' in payload:
        ...
        if value and value not in VALID_BODY_ALT_RESOURCE_IP:
            return (False, f"Invalid alt-resource-ip '{value}'. Must be one of: {', '.join(VALID_BODY_ALT_RESOURCE_IP)}")
    ...
    if 'azure-region' in payload:
        ...
        if value and value not in VALID_BODY_AZURE_REGION:
            return (False, f"Invalid azure-region '{value}'. Must be one of: {', '.join(VALID_BODY_AZURE_REGION)}")
    ...
    if 'oci-region-type' in payload:
        ...
        if value and value not in VALID_BODY_OCI_REGION_TYPE:
            return (False, f"Invalid oci-region-type '{value}'. Must be one of: {', '.join(VALID_BODY_OCI_REGION_TYPE)}")
    ...
    if 'ibm-region' in payload:
        ...
        if value and value not in VALID_BODY_IBM_REGION:
            return (False, f"Invalid ibm-region '{value}'. Must be one of: {', '.join(VALID_BODY_IBM_REGION)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 3600:
                    return (False, f"update-interval must be between 0 and 3600")
            except (ValueError, TypeError):
                return (False, f"update-interval must be numeric, got: {value}")
...
def validate_sdn_connector_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
