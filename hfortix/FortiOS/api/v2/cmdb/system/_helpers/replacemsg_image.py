...
def validate_replacemsg_image_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'image-type' in payload:
        ...
        if value and value not in VALID_BODY_IMAGE_TYPE:
            return (False, f"Invalid image-type '{value}'. Must be one of: {', '.join(VALID_BODY_IMAGE_TYPE)}")
...
def validate_replacemsg_image_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
