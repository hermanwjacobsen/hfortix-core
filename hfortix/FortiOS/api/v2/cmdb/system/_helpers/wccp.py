...
def validate_wccp_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'ports-defined' in payload:
        ...
        if value and value not in VALID_BODY_PORTS_DEFINED:
            return (False, f"Invalid ports-defined '{value}'. Must be one of: {', '.join(VALID_BODY_PORTS_DEFINED)}")
    ...
    if 'forward-method' in payload:
        ...
        if value and value not in VALID_BODY_FORWARD_METHOD:
            return (False, f"Invalid forward-method '{value}'. Must be one of: {', '.join(VALID_BODY_FORWARD_METHOD)}")
    ...
    if 'cache-engine-method' in payload:
        ...
        if value and value not in VALID_BODY_CACHE_ENGINE_METHOD:
            return (False, f"Invalid cache-engine-method '{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_ENGINE_METHOD)}")
    ...
    if 'service-type' in payload:
        ...
        if value and value not in VALID_BODY_SERVICE_TYPE:
            return (False, f"Invalid service-type '{value}'. Must be one of: {', '.join(VALID_BODY_SERVICE_TYPE)}")
    ...
    if 'primary-hash' in payload:
        ...
        if value and value not in VALID_BODY_PRIMARY_HASH:
            return (False, f"Invalid primary-hash '{value}'. Must be one of: {', '.join(VALID_BODY_PRIMARY_HASH)}")
    ...
    if 'assignment-weight' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 255:
                    return (False, f"assignment-weight must be between 0 and 255")
            except (ValueError, TypeError):
                return (False, f"assignment-weight must be numeric, got: {value}")
    ...
    if 'assignment-bucket-format' in payload:
        ...
        if value and value not in VALID_BODY_ASSIGNMENT_BUCKET_FORMAT:
            return (False, f"Invalid assignment-bucket-format '{value}'. Must be one of: {', '.join(VALID_BODY_ASSIGNMENT_BUCKET_FORMAT)}")
    ...
    if 'return-method' in payload:
        ...
        if value and value not in VALID_BODY_RETURN_METHOD:
            return (False, f"Invalid return-method '{value}'. Must be one of: {', '.join(VALID_BODY_RETURN_METHOD)}")
    ...
    if 'assignment-method' in payload:
        ...
        if value and value not in VALID_BODY_ASSIGNMENT_METHOD:
            return (False, f"Invalid assignment-method '{value}'. Must be one of: {', '.join(VALID_BODY_ASSIGNMENT_METHOD)}")
