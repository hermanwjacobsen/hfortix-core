...
def validate_accprofile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'scope' in payload:
        ...
        if value and value not in VALID_BODY_SCOPE:
            return (False, f"Invalid scope '{value}'. Must be one of: {', '.join(VALID_BODY_SCOPE)}")
    ...
    if 'secfabgrp' in payload:
        ...
        if value and value not in VALID_BODY_SECFABGRP:
            return (False, f"Invalid secfabgrp '{value}'. Must be one of: {', '.join(VALID_BODY_SECFABGRP)}")
    ...
    if 'admintimeout-override' in payload:
        ...
        if value and value not in VALID_BODY_ADMINTIMEOUT_OVERRIDE:
            return (False, f"Invalid admintimeout-override '{value}'. Must be one of: {', '.join(VALID_BODY_ADMINTIMEOUT_OVERRIDE)}")
    ...
    if 'cli-diagnose' in payload:
        ...
        if value and value not in VALID_BODY_CLI_DIAGNOSE:
            return (False, f"Invalid cli-diagnose '{value}'. Must be one of: {', '.join(VALID_BODY_CLI_DIAGNOSE)}")
    
    # Validate cli-get if present
    if 'cli-get' in payload:
        value = payload.get('cli-get')
        if value and value not in VALID_BODY_CLI_GET:
            return (False, f"Invalid cli-get '{value}'. Must be one of: {', '.join(VALID_BODY_CLI_GET)}")
    ...
    if 'cli-show' in payload:
        ...
        if value and value not in VALID_BODY_CLI_SHOW:
            return (False, f"Invalid cli-show '{value}'. Must be one of: {', '.join(VALID_BODY_CLI_SHOW)}")
    ...
    if 'cli-exec' in payload:
        ...
        if value and value not in VALID_BODY_CLI_EXEC:
            return (False, f"Invalid cli-exec '{value}'. Must be one of: {', '.join(VALID_BODY_CLI_EXEC)}")
    ...
    if 'cli-config' in payload:
        ...
        if value and value not in VALID_BODY_CLI_CONFIG:
            return (False, f"Invalid cli-config '{value}'. Must be one of: {', '.join(VALID_BODY_CLI_CONFIG)}")
    ...
    if 'system-execute-ssh' in payload:
        ...
        if value and value not in VALID_BODY_SYSTEM_EXECUTE_SSH:
            return (False, f"Invalid system-execute-ssh '{value}'. Must be one of: {', '.join(VALID_BODY_SYSTEM_EXECUTE_SSH)}")
    ...
    if 'system-execute-telnet' in payload:
        ...
        if value and value not in VALID_BODY_SYSTEM_EXECUTE_TELNET:
            return (False, f"Invalid system-execute-telnet '{value}'. Must be one of: {', '.join(VALID_BODY_SYSTEM_EXECUTE_TELNET)}")
...
def validate_accprofile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
