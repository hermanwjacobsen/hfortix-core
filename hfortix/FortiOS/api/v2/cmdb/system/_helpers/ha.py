...
VALID_BODY_UPGRADE_MODE = ['simultaneous', 'uninterruptible', 'local-only', 'secondary-only']
...
VALID_BODY_SCHEDULE = ['none', 'leastconnection', 'round-robin', 'weight-round-robin', 'random', 'ip', 'ipport']
...
VALID_BODY_IPSEC_PHASE2_PROPOSAL = ['aes128-sha1', 'aes128-sha256', 'aes128-sha384', 'aes128-sha512', 'aes192-sha1', 'aes192-sha256', 'aes192-sha384', 'aes192-sha512', 'aes256-sha1', 'aes256-sha256', 'aes256-sha384', 'aes256-sha512', 'aes128gcm', 'aes256gcm', 'chacha20poly1305']
...
def validate_ha_put(
    ...
    if 'sync-packet-balance' in payload:
        ...
        if value and value not in VALID_BODY_SYNC_PACKET_BALANCE:
            return (False, f"Invalid sync-packet-balance '{value}'. Must be one of: {', '.join(VALID_BODY_SYNC_PACKET_BALANCE)}")
    ...
    if 'load-balance-all' in payload:
        ...
        if value and value not in VALID_BODY_LOAD_BALANCE_ALL:
            return (False, f"Invalid load-balance-all '{value}'. Must be one of: {', '.join(VALID_BODY_LOAD_BALANCE_ALL)}")
    ...
    if 'sync-config' in payload:
        ...
        if value and value not in VALID_BODY_SYNC_CONFIG:
            return (False, f"Invalid sync-config '{value}'. Must be one of: {', '.join(VALID_BODY_SYNC_CONFIG)}")
    ...
    if 'encryption' in payload:
        ...
        if value and value not in VALID_BODY_ENCRYPTION:
            return (False, f"Invalid encryption '{value}'. Must be one of: {', '.join(VALID_BODY_ENCRYPTION)}")
    
    # Validate authentication if present
    if 'authentication' in payload:
        value = payload.get('authentication')
        if value and value not in VALID_BODY_AUTHENTICATION:
            return (False, f"Invalid authentication '{value}'. Must be one of: {', '.join(VALID_BODY_AUTHENTICATION)}")
    ...
    if 'hb-interval-in-milliseconds' in payload:
        ...
        if value and value not in VALID_BODY_HB_INTERVAL_IN_MILLISECONDS:
            return (False, f"Invalid hb-interval-in-milliseconds '{value}'. Must be one of: {', '.join(VALID_BODY_HB_INTERVAL_IN_MILLISECONDS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 60:
                    return (False, f"hb-lost-threshold must be between 1 and 60")
            except (ValueError, TypeError):
                return (False, f"hb-lost-threshold must be numeric, got: {value}")
    ...
    if 'gratuitous-arps' in payload:
        ...
        if value and value not in VALID_BODY_GRATUITOUS_ARPS:
            return (False, f"Invalid gratuitous-arps '{value}'. Must be one of: {', '.join(VALID_BODY_GRATUITOUS_ARPS)}")
    ...
    if 'session-pickup' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_PICKUP:
            return (False, f"Invalid session-pickup '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_PICKUP)}")
    ...
    if 'session-pickup-connectionless' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_PICKUP_CONNECTIONLESS:
            return (False, f"Invalid session-pickup-connectionless '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_PICKUP_CONNECTIONLESS)}")
    ...
    if 'session-pickup-expectation' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_PICKUP_EXPECTATION:
            return (False, f"Invalid session-pickup-expectation '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_PICKUP_EXPECTATION)}")
    ...
    if 'session-pickup-nat' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_PICKUP_NAT:
            return (False, f"Invalid session-pickup-nat '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_PICKUP_NAT)}")
    ...
    if 'session-pickup-delay' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_PICKUP_DELAY:
            return (False, f"Invalid session-pickup-delay '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_PICKUP_DELAY)}")
    ...
    if 'link-failed-signal' in payload:
        ...
        if value and value not in VALID_BODY_LINK_FAILED_SIGNAL:
            return (False, f"Invalid link-failed-signal '{value}'. Must be one of: {', '.join(VALID_BODY_LINK_FAILED_SIGNAL)}")
    ...
    if 'upgrade-mode' in payload:
        ...
        if value and value not in VALID_BODY_UPGRADE_MODE:
            return (False, f"Invalid upgrade-mode '{value}'. Must be one of: {', '.join(VALID_BODY_UPGRADE_MODE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 15 or int_val > 300:
                    return (False, f"uninterruptible-primary-wait must be between 15 and 300")
            except (ValueError, TypeError):
                return (False, f"uninterruptible-primary-wait must be numeric, got: {value}")
    ...
    if 'standalone-mgmt-vdom' in payload:
        ...
        if value and value not in VALID_BODY_STANDALONE_MGMT_VDOM:
            return (False, f"Invalid standalone-mgmt-vdom '{value}'. Must be one of: {', '.join(VALID_BODY_STANDALONE_MGMT_VDOM)}")
    ...
    if 'ha-mgmt-status' in payload:
        ...
        if value and value not in VALID_BODY_HA_MGMT_STATUS:
            return (False, f"Invalid ha-mgmt-status '{value}'. Must be one of: {', '.join(VALID_BODY_HA_MGMT_STATUS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"ha-uptime-diff-margin must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"ha-uptime-diff-margin must be numeric, got: {value}")
    ...
    if 'standalone-config-sync' in payload:
        ...
        if value and value not in VALID_BODY_STANDALONE_CONFIG_SYNC:
            return (False, f"Invalid standalone-config-sync '{value}'. Must be one of: {', '.join(VALID_BODY_STANDALONE_CONFIG_SYNC)}")
    ...
    if 'logical-sn' in payload:
        ...
        if value and value not in VALID_BODY_LOGICAL_SN:
            return (False, f"Invalid logical-sn '{value}'. Must be one of: {', '.join(VALID_BODY_LOGICAL_SN)}")
    
    # Validate schedule if present
    if 'schedule' in payload:
        value = payload.get('schedule')
        if value and value not in VALID_BODY_SCHEDULE:
            return (False, f"Invalid schedule '{value}'. Must be one of: {', '.join(VALID_BODY_SCHEDULE)}")
    ...
    if 'override' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE:
            return (False, f"Invalid override '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE)}")
    ...
    if 'override-wait-time' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 3600:
                    return (False, f"override-wait-time must be between 0 and 3600")
            except (ValueError, TypeError):
                return (False, f"override-wait-time must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 50:
                    return (False, f"pingserver-failover-threshold must be between 0 and 50")
            except (ValueError, TypeError):
                return (False, f"pingserver-failover-threshold must be numeric, got: {value}")
    ...
    if 'pingserver-secondary-force-reset' in payload:
        ...
        if value and value not in VALID_BODY_PINGSERVER_SECONDARY_FORCE_RESET:
            return (False, f"Invalid pingserver-secondary-force-reset '{value}'. Must be one of: {', '.join(VALID_BODY_PINGSERVER_SECONDARY_FORCE_RESET)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 6 or int_val > 2147483647:
                    return (False, f"pingserver-flip-timeout must be between 6 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"pingserver-flip-timeout must be numeric, got: {value}")
    ...
    if 'vcluster-status' in payload:
        ...
        if value and value not in VALID_BODY_VCLUSTER_STATUS:
            return (False, f"Invalid vcluster-status '{value}'. Must be one of: {', '.join(VALID_BODY_VCLUSTER_STATUS)}")
    ...
    if 'ha-direct' in payload:
        ...
        if value and value not in VALID_BODY_HA_DIRECT:
            return (False, f"Invalid ha-direct '{value}'. Must be one of: {', '.join(VALID_BODY_HA_DIRECT)}")
    ...
    if 'ssd-failover' in payload:
        ...
        if value and value not in VALID_BODY_SSD_FAILOVER:
            return (False, f"Invalid ssd-failover '{value}'. Must be one of: {', '.join(VALID_BODY_SSD_FAILOVER)}")
    ...
    if 'memory-compatible-mode' in payload:
        ...
        if value and value not in VALID_BODY_MEMORY_COMPATIBLE_MODE:
            return (False, f"Invalid memory-compatible-mode '{value}'. Must be one of: {', '.join(VALID_BODY_MEMORY_COMPATIBLE_MODE)}")
    ...
    if 'memory-based-failover' in payload:
        ...
        if value and value not in VALID_BODY_MEMORY_BASED_FAILOVER:
            return (False, f"Invalid memory-based-failover '{value}'. Must be one of: {', '.join(VALID_BODY_MEMORY_BASED_FAILOVER)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 95:
                    return (False, f"memory-failover-threshold must be between 0 and 95")
            except (ValueError, TypeError):
                return (False, f"memory-failover-threshold must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 300:
                    return (False, f"memory-failover-monitor-period must be between 1 and 300")
            except (ValueError, TypeError):
                return (False, f"memory-failover-monitor-period must be numeric, got: {value}")
    ...
    if 'memory-failover-sample-rate' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 60:
                    return (False, f"memory-failover-sample-rate must be between 1 and 60")
            except (ValueError, TypeError):
                return (False, f"memory-failover-sample-rate must be numeric, got: {value}")
    ...
    if 'memory-failover-flip-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 6 or int_val > 2147483647:
                    return (False, f"memory-failover-flip-timeout must be between 6 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"memory-failover-flip-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 300:
                    return (False, f"failover-hold-time must be between 0 and 300")
            except (ValueError, TypeError):
                return (False, f"failover-hold-time must be numeric, got: {value}")
    ...
    if 'check-secondary-dev-health' in payload:
        ...
        if value and value not in VALID_BODY_CHECK_SECONDARY_DEV_HEALTH:
            return (False, f"Invalid check-secondary-dev-health '{value}'. Must be one of: {', '.join(VALID_BODY_CHECK_SECONDARY_DEV_HEALTH)}")
    ...
    if 'ipsec-phase2-proposal' in payload:
        ...
        if value and value not in VALID_BODY_IPSEC_PHASE2_PROPOSAL:
            return (False, f"Invalid ipsec-phase2-proposal '{value}'. Must be one of: {', '.join(VALID_BODY_IPSEC_PHASE2_PROPOSAL)}")
    ...
    if 'bounce-intf-upon-failover' in payload:
        ...
        if value and value not in VALID_BODY_BOUNCE_INTF_UPON_FAILOVER:
            return (False, f"Invalid bounce-intf-upon-failover '{value}'. Must be one of: {', '.join(VALID_BODY_BOUNCE_INTF_UPON_FAILOVER)}")
