...
def validate_dns_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"dns-cache-limit must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"dns-cache-limit must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 86400:
                    return (False, f"dns-cache-ttl must be between 60 and 86400")
            except (ValueError, TypeError):
    ...
    if 'cache-notfound-responses' in payload:
        ...
        if value and value not in VALID_BODY_CACHE_NOTFOUND_RESPONSES:
            return (False, f"Invalid cache-notfound-responses '{value}'. Must be one of: {', '.join(VALID_BODY_CACHE_NOTFOUND_RESPONSES)}")
    ...
    if 'server-select-method' in payload:
        ...
        if value and value not in VALID_BODY_SERVER_SELECT_METHOD:
            return (False, f"Invalid server-select-method '{value}'. Must be one of: {', '.join(VALID_BODY_SERVER_SELECT_METHOD)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 86400:
                    return (False, f"fqdn-cache-ttl must be between 0 and 86400")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 3600 or int_val > 86400:
                    return (False, f"fqdn-max-refresh must be between 3600 and 86400")
            except (ValueError, TypeError):
                return (False, f"fqdn-max-refresh must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 3600:
                    return (False, f"fqdn-min-refresh must be between 10 and 3600")
            except (ValueError, TypeError):
                return (False, f"fqdn-min-refresh must be numeric, got: {value}")
    ...
    if 'hostname-ttl' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 86400:
                    return (False, f"hostname-ttl must be between 60 and 86400")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 50000:
                    return (False, f"hostname-limit must be between 0 and 50000")
            except (ValueError, TypeError):
