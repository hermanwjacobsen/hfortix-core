...
VALID_BODY_LOAD_BALANCE_MODE = ['source-ip-based', 'weight-based', 'usage-based', 'source-dest-ip-based', 'measured-volume-based']
...
def validate_sdwan_put(
    ...
    if 'load-balance-mode' in payload:
        ...
        if value and value not in VALID_BODY_LOAD_BALANCE_MODE:
            return (False, f"Invalid load-balance-mode '{value}'. Must be one of: {', '.join(VALID_BODY_LOAD_BALANCE_MODE)}")
    ...
    if 'speedtest-bypass-routing' in payload:
        ...
        if value and value not in VALID_BODY_SPEEDTEST_BYPASS_ROUTING:
            return (False, f"Invalid speedtest-bypass-routing '{value}'. Must be one of: {', '.join(VALID_BODY_SPEEDTEST_BYPASS_ROUTING)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2 or int_val > 4:
                    return (False, f"duplication-max-num must be between 2 and 4")
            except (ValueError, TypeError):
                return (False, f"duplication-max-num must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 250 or int_val > 1000:
                    return (False, f"duplication-max-discrepancy must be between 250 and 1000")
            except (ValueError, TypeError):
                return (False, f"duplication-max-discrepancy must be numeric, got: {value}")
    ...
    if 'neighbor-hold-down' in payload:
        ...
        if value and value not in VALID_BODY_NEIGHBOR_HOLD_DOWN:
            return (False, f"Invalid neighbor-hold-down '{value}'. Must be one of: {', '.join(VALID_BODY_NEIGHBOR_HOLD_DOWN)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 10000000:
                    return (False, f"neighbor-hold-down-time must be between 0 and 10000000")
            except (ValueError, TypeError):
                return (False, f"neighbor-hold-down-time must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 3600:
                    return (False, f"app-perf-log-period must be between 0 and 3600")
            except (ValueError, TypeError):
                return (False, f"app-perf-log-period must be numeric, got: {value}")
    ...
    if 'neighbor-hold-boot-time' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 10000000:
                    return (False, f"neighbor-hold-boot-time must be between 0 and 10000000")
            except (ValueError, TypeError):
                return (False, f"neighbor-hold-boot-time must be numeric, got: {value}")
