...
VALID_BODY_EVENTS = ['cpu-high', 'mem-low', 'log-full', 'intf-ip', 'vpn-tun-up', 'vpn-tun-down', 'ha-switch', 'ha-hb-failure', 'ips-signature', 'ips-anomaly', 'av-virus', 'av-oversize', 'av-pattern', 'av-fragmented', 'fm-if-change', 'fm-conf-change', 'bgp-established', 'bgp-backward-transition', 'ha-member-up', 'ha-member-down', 'ent-conf-change', 'av-conserve', 'av-bypass', 'av-oversize-passed', 'av-oversize-blocked', 'ips-pkg-update', 'ips-fail-open', 'temperature-high', 'voltage-alert', 'power-supply', 'faz-disconnect', 'faz', 'wc-ap-up', 'wc-ap-down', 'fswctl-session-up', 'fswctl-session-down', 'load-balance-real-server-down', 'device-new', 'per-cpu-high', 'dhcp', 'pool-usage', 'ippool', 'interface', 'ospf-nbr-state-change', 'ospf-virtnbr-state-change', 'bfd']
...
def validate_snmp_community_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'query-v1-status' in payload:
        ...
        if value and value not in VALID_BODY_QUERY_V1_STATUS:
            return (False, f"Invalid query-v1-status '{value}'. Must be one of: {', '.join(VALID_BODY_QUERY_V1_STATUS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"query-v1-port must be between 1 and 65535")
            except (ValueError, TypeError):
    ...
    if 'query-v2c-status' in payload:
        ...
        if value and value not in VALID_BODY_QUERY_V2C_STATUS:
            return (False, f"Invalid query-v2c-status '{value}'. Must be one of: {', '.join(VALID_BODY_QUERY_V2C_STATUS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"query-v2c-port must be between 0 and 65535")
            except (ValueError, TypeError):
    ...
    if 'trap-v1-status' in payload:
        ...
        if value and value not in VALID_BODY_TRAP_V1_STATUS:
            return (False, f"Invalid trap-v1-status '{value}'. Must be one of: {', '.join(VALID_BODY_TRAP_V1_STATUS)}")
    ...
    if 'trap-v1-lport' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"trap-v1-lport must be between 1 and 65535")
            except (ValueError, TypeError):
    ...
    if 'trap-v1-rport' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"trap-v1-rport must be between 1 and 65535")
            except (ValueError, TypeError):
    ...
    if 'trap-v2c-status' in payload:
        ...
        if value and value not in VALID_BODY_TRAP_V2C_STATUS:
            return (False, f"Invalid trap-v2c-status '{value}'. Must be one of: {', '.join(VALID_BODY_TRAP_V2C_STATUS)}")
    ...
    if 'trap-v2c-lport' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"trap-v2c-lport must be between 1 and 65535")
            except (ValueError, TypeError):
    ...
    if 'trap-v2c-rport' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"trap-v2c-rport must be between 1 and 65535")
            except (ValueError, TypeError):
    ...
    if 'events' in payload:
        ...
        if value and value not in VALID_BODY_EVENTS:
            return (False, f"Invalid events '{value}'. Must be one of: {', '.join(VALID_BODY_EVENTS)}")
...
def validate_snmp_community_delete(id: str | None = None) -> tuple[bool, str | None]:
    """
