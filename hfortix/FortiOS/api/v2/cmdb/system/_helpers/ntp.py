...
def validate_ntp_put(
    ...
    if 'ntpsync' in payload:
        ...
        if value and value not in VALID_BODY_NTPSYNC:
            return (False, f"Invalid ntpsync '{value}'. Must be one of: {', '.join(VALID_BODY_NTPSYNC)}")
    ...
    if 'server-mode' in payload:
        ...
        if value and value not in VALID_BODY_SERVER_MODE:
            return (False, f"Invalid server-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SERVER_MODE)}")
    ...
    if 'key-type' in payload:
        ...
        if value and value not in VALID_BODY_KEY_TYPE:
            return (False, f"Invalid key-type '{value}'. Must be one of: {', '.join(VALID_BODY_KEY_TYPE)}")
