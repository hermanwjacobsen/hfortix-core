...
def validate_dns_database_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'view' in payload:
        ...
        if value and value not in VALID_BODY_VIEW:
            return (False, f"Invalid view '{value}'. Must be one of: {', '.join(VALID_BODY_VIEW)}")
    ...
    if 'authoritative' in payload:
        ...
        if value and value not in VALID_BODY_AUTHORITATIVE:
            return (False, f"Invalid authoritative '{value}'. Must be one of: {', '.join(VALID_BODY_AUTHORITATIVE)}")
...
def validate_dns_database_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
