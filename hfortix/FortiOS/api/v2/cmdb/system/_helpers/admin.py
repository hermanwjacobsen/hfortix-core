...
VALID_BODY_TWO_FACTOR = ['disable', 'fortitoken', 'fortitoken-cloud', 'email', 'sms']
...
def validate_admin_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'remote-auth' in payload:
        ...
        if value and value not in VALID_BODY_REMOTE_AUTH:
            return (False, f"Invalid remote-auth '{value}'. Must be one of: {', '.join(VALID_BODY_REMOTE_AUTH)}")
    ...
    if 'wildcard' in payload:
        ...
        if value and value not in VALID_BODY_WILDCARD:
            return (False, f"Invalid wildcard '{value}'. Must be one of: {', '.join(VALID_BODY_WILDCARD)}")
    ...
    if 'peer-auth' in payload:
        ...
        if value and value not in VALID_BODY_PEER_AUTH:
            return (False, f"Invalid peer-auth '{value}'. Must be one of: {', '.join(VALID_BODY_PEER_AUTH)}")
    ...
    if 'allow-remove-admin-session' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_REMOVE_ADMIN_SESSION:
            return (False, f"Invalid allow-remove-admin-session '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_REMOVE_ADMIN_SESSION)}")
    ...
    if 'accprofile-override' in payload:
        ...
        if value and value not in VALID_BODY_ACCPROFILE_OVERRIDE:
            return (False, f"Invalid accprofile-override '{value}'. Must be one of: {', '.join(VALID_BODY_ACCPROFILE_OVERRIDE)}")
    ...
    if 'vdom-override' in payload:
        ...
        if value and value not in VALID_BODY_VDOM_OVERRIDE:
            return (False, f"Invalid vdom-override '{value}'. Must be one of: {', '.join(VALID_BODY_VDOM_OVERRIDE)}")
    ...
    if 'force-password-change' in payload:
        ...
        if value and value not in VALID_BODY_FORCE_PASSWORD_CHANGE:
            return (False, f"Invalid force-password-change '{value}'. Must be one of: {', '.join(VALID_BODY_FORCE_PASSWORD_CHANGE)}")
    ...
    if 'sms-server' in payload:
        ...
        if value and value not in VALID_BODY_SMS_SERVER:
            return (False, f"Invalid sms-server '{value}'. Must be one of: {', '.join(VALID_BODY_SMS_SERVER)}")
    ...
    if 'guest-auth' in payload:
        ...
        if value and value not in VALID_BODY_GUEST_AUTH:
            return (False, f"Invalid guest-auth '{value}'. Must be one of: {', '.join(VALID_BODY_GUEST_AUTH)}")
