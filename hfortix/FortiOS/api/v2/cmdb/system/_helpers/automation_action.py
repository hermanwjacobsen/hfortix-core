...
VALID_BODY_ACTION_TYPE = ['email', 'fortiexplorer-notification', 'alert', 'disable-ssid', 'system-actions', 'quarantine', 'quarantine-forticlient', 'quarantine-nsx', 'quarantine-fortinac', 'ban-ip', 'aws-lambda', 'azure-function', 'google-cloud-function', 'alicloud-function', 'webhook', 'cli-script', 'diagnose-script', 'regular-expression', 'slack-notification', 'microsoft-teams-notification']
...
def validate_automation_action_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'action-type' in payload:
        ...
        if value and value not in VALID_BODY_ACTION_TYPE:
            return (False, f"Invalid action-type '{value}'. Must be one of: {', '.join(VALID_BODY_ACTION_TYPE)}")
    ...
    if 'system-action' in payload:
        ...
        if value and value not in VALID_BODY_SYSTEM_ACTION:
            return (False, f"Invalid system-action '{value}'. Must be one of: {', '.join(VALID_BODY_SYSTEM_ACTION)}")
    ...
    if 'forticare-email' in payload:
        ...
        if value and value not in VALID_BODY_FORTICARE_EMAIL:
            return (False, f"Invalid forticare-email '{value}'. Must be one of: {', '.join(VALID_BODY_FORTICARE_EMAIL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2592000:
                    return (False, f"minimum-interval must be between 0 and 2592000")
            except (ValueError, TypeError):
                return (False, f"minimum-interval must be numeric, got: {value}")
    ...
    if 'azure-function-authorization' in payload:
        ...
        if value and value not in VALID_BODY_AZURE_FUNCTION_AUTHORIZATION:
            return (False, f"Invalid azure-function-authorization '{value}'. Must be one of: {', '.join(VALID_BODY_AZURE_FUNCTION_AUTHORIZATION)}")
    ...
    if 'alicloud-function-authorization' in payload:
        ...
        if value and value not in VALID_BODY_ALICLOUD_FUNCTION_AUTHORIZATION:
            return (False, f"Invalid alicloud-function-authorization '{value}'. Must be one of: {', '.join(VALID_BODY_ALICLOUD_FUNCTION_AUTHORIZATION)}")
    
    # Validate alicloud-access-key-id if present
    if 'alicloud-access-key-id' in payload:
        value = payload.get('alicloud-access-key-id')
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"alicloud-access-key-id cannot exceed 35 characters")
    ...
    if 'message-type' in payload:
        ...
        if value and value not in VALID_BODY_MESSAGE_TYPE:
            return (False, f"Invalid message-type '{value}'. Must be one of: {', '.join(VALID_BODY_MESSAGE_TYPE)}")
    ...
    if 'replacement-message' in payload:
        ...
        if value and value not in VALID_BODY_REPLACEMENT_MESSAGE:
            return (False, f"Invalid replacement-message '{value}'. Must be one of: {', '.join(VALID_BODY_REPLACEMENT_MESSAGE)}")
    ...
    if 'method' in payload:
        ...
        if value and value not in VALID_BODY_METHOD:
            return (False, f"Invalid method '{value}'. Must be one of: {', '.join(VALID_BODY_METHOD)}")
    ...
    if 'verify-host-cert' in payload:
        ...
        if value and value not in VALID_BODY_VERIFY_HOST_CERT:
            return (False, f"Invalid verify-host-cert '{value}'. Must be one of: {', '.join(VALID_BODY_VERIFY_HOST_CERT)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 36000:
                    return (False, f"output-interval must be between 0 and 36000")
            except (ValueError, TypeError):
                return (False, f"output-interval must be numeric, got: {value}")
    ...
    if 'file-only' in payload:
        ...
        if value and value not in VALID_BODY_FILE_ONLY:
            return (False, f"Invalid file-only '{value}'. Must be one of: {', '.join(VALID_BODY_FILE_ONLY)}")
    ...
    if 'execute-security-fabric' in payload:
        ...
        if value and value not in VALID_BODY_EXECUTE_SECURITY_FABRIC:
            return (False, f"Invalid execute-security-fabric '{value}'. Must be one of: {', '.join(VALID_BODY_EXECUTE_SECURITY_FABRIC)}")
    ...
    if 'log-debug-print' in payload:
        ...
        if value and value not in VALID_BODY_LOG_DEBUG_PRINT:
            return (False, f"Invalid log-debug-print '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_DEBUG_PRINT)}")
...
def validate_automation_action_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
