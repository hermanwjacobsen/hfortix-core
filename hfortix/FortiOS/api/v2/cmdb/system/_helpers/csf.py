...
def validate_csf_put(
    ...
    if 'upstream-interface-select-method' in payload:
        ...
        if value and value not in VALID_BODY_UPSTREAM_INTERFACE_SELECT_METHOD:
            return (False, f"Invalid upstream-interface-select-method '{value}'. Must be one of: {', '.join(VALID_BODY_UPSTREAM_INTERFACE_SELECT_METHOD)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"upstream-port must be between 1 and 65535")
    ...
    if 'accept-auth-by-cert' in payload:
        ...
        if value and value not in VALID_BODY_ACCEPT_AUTH_BY_CERT:
            return (False, f"Invalid accept-auth-by-cert '{value}'. Must be one of: {', '.join(VALID_BODY_ACCEPT_AUTH_BY_CERT)}")
    ...
    if 'log-unification' in payload:
        ...
        if value and value not in VALID_BODY_LOG_UNIFICATION:
            return (False, f"Invalid log-unification '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_UNIFICATION)}")
    ...
    if 'authorization-request-type' in payload:
        ...
        if value and value not in VALID_BODY_AUTHORIZATION_REQUEST_TYPE:
            return (False, f"Invalid authorization-request-type '{value}'. Must be one of: {', '.join(VALID_BODY_AUTHORIZATION_REQUEST_TYPE)}")
    ...
    if 'downstream-access' in payload:
        ...
        if value and value not in VALID_BODY_DOWNSTREAM_ACCESS:
            return (False, f"Invalid downstream-access '{value}'. Must be one of: {', '.join(VALID_BODY_DOWNSTREAM_ACCESS)}")
    ...
    if 'legacy-authentication' in payload:
        ...
        if value and value not in VALID_BODY_LEGACY_AUTHENTICATION:
            return (False, f"Invalid legacy-authentication '{value}'. Must be one of: {', '.join(VALID_BODY_LEGACY_AUTHENTICATION)}")
    
    # Validate downstream-accprofile if present
    if 'downstream-accprofile' in payload:
        value = payload.get('downstream-accprofile')
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"downstream-accprofile cannot exceed 35 characters")
    ...
    if 'configuration-sync' in payload:
        ...
        if value and value not in VALID_BODY_CONFIGURATION_SYNC:
            return (False, f"Invalid configuration-sync '{value}'. Must be one of: {', '.join(VALID_BODY_CONFIGURATION_SYNC)}")
    ...
    if 'fabric-object-unification' in payload:
        ...
        if value and value not in VALID_BODY_FABRIC_OBJECT_UNIFICATION:
            return (False, f"Invalid fabric-object-unification '{value}'. Must be one of: {', '.join(VALID_BODY_FABRIC_OBJECT_UNIFICATION)}")
    ...
    if 'saml-configuration-sync' in payload:
        ...
        if value and value not in VALID_BODY_SAML_CONFIGURATION_SYNC:
            return (False, f"Invalid saml-configuration-sync '{value}'. Must be one of: {', '.join(VALID_BODY_SAML_CONFIGURATION_SYNC)}")
    ...
    if 'forticloud-account-enforcement' in payload:
        ...
        if value and value not in VALID_BODY_FORTICLOUD_ACCOUNT_ENFORCEMENT:
            return (False, f"Invalid forticloud-account-enforcement '{value}'. Must be one of: {', '.join(VALID_BODY_FORTICLOUD_ACCOUNT_ENFORCEMENT)}")
    ...
    if 'file-mgmt' in payload:
        ...
        if value and value not in VALID_BODY_FILE_MGMT:
            return (False, f"Invalid file-mgmt '{value}'. Must be one of: {', '.join(VALID_BODY_FILE_MGMT)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"file-quota must be between 0 and 4294967295")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 99:
                    return (False, f"file-quota-warning must be between 1 and 99")
            except (ValueError, TypeError):
                return (False, f"file-quota-warning must be numeric, got: {value}")
