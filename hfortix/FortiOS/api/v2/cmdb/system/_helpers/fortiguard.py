...
VALID_BODY_AUTO_FIRMWARE_UPGRADE_DAY = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']
...
def validate_fortiguard_put(
    ...
    if 'fortiguard-anycast' in payload:
        ...
        if value and value not in VALID_BODY_FORTIGUARD_ANYCAST:
            return (False, f"Invalid fortiguard-anycast '{value}'. Must be one of: {', '.join(VALID_BODY_FORTIGUARD_ANYCAST)}")
    ...
    if 'fortiguard-anycast-source' in payload:
        ...
        if value and value not in VALID_BODY_FORTIGUARD_ANYCAST_SOURCE:
            return (False, f"Invalid fortiguard-anycast-source '{value}'. Must be one of: {', '.join(VALID_BODY_FORTIGUARD_ANYCAST_SOURCE)}")
    ...
    if 'port' in payload:
        ...
        if value and value not in VALID_BODY_PORT:
            return (False, f"Invalid port '{value}'. Must be one of: {', '.join(VALID_BODY_PORT)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 266:
                    return (False, f"load-balance-servers must be between 1 and 266")
            except (ValueError, TypeError):
                return (False, f"load-balance-servers must be numeric, got: {value}")
    ...
    if 'auto-join-forticloud' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_JOIN_FORTICLOUD:
            return (False, f"Invalid auto-join-forticloud '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_JOIN_FORTICLOUD)}")
    ...
    if 'update-server-location' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_SERVER_LOCATION:
            return (False, f"Invalid update-server-location '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_SERVER_LOCATION)}")
    ...
    if 'update-ffdb' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_FFDB:
            return (False, f"Invalid update-ffdb '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_FFDB)}")
    ...
    if 'update-uwdb' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_UWDB:
            return (False, f"Invalid update-uwdb '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_UWDB)}")
    ...
    if 'update-dldb' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_DLDB:
            return (False, f"Invalid update-dldb '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_DLDB)}")
    ...
    if 'update-extdb' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_EXTDB:
            return (False, f"Invalid update-extdb '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_EXTDB)}")
    ...
    if 'update-build-proxy' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_BUILD_PROXY:
            return (False, f"Invalid update-build-proxy '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_BUILD_PROXY)}")
    ...
    if 'persistent-connection' in payload:
        ...
        if value and value not in VALID_BODY_PERSISTENT_CONNECTION:
            return (False, f"Invalid persistent-connection '{value}'. Must be one of: {', '.join(VALID_BODY_PERSISTENT_CONNECTION)}")
    ...
    if 'auto-firmware-upgrade' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_FIRMWARE_UPGRADE:
            return (False, f"Invalid auto-firmware-upgrade '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_FIRMWARE_UPGRADE)}")
    ...
    if 'auto-firmware-upgrade-day' in payload:
        ...
        if value and value not in VALID_BODY_AUTO_FIRMWARE_UPGRADE_DAY:
            return (False, f"Invalid auto-firmware-upgrade-day '{value}'. Must be one of: {', '.join(VALID_BODY_AUTO_FIRMWARE_UPGRADE_DAY)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 14:
                    return (False, f"auto-firmware-upgrade-delay must be between 0 and 14")
            except (ValueError, TypeError):
                return (False, f"auto-firmware-upgrade-delay must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 23:
                    return (False, f"auto-firmware-upgrade-start-hour must be between 0 and 23")
            except (ValueError, TypeError):
                return (False, f"auto-firmware-upgrade-start-hour must be numeric, got: {value}")
    ...
    if 'auto-firmware-upgrade-end-hour' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 23:
                    return (False, f"auto-firmware-upgrade-end-hour must be between 0 and 23")
            except (ValueError, TypeError):
                return (False, f"auto-firmware-upgrade-end-hour must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 100:
                    return (False, f"FDS-license-expiring-days must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"FDS-license-expiring-days must be numeric, got: {value}")
    ...
    if 'subscribe-update-notification' in payload:
        ...
        if value and value not in VALID_BODY_SUBSCRIBE_UPDATE_NOTIFICATION:
            return (False, f"Invalid subscribe-update-notification '{value}'. Must be one of: {', '.join(VALID_BODY_SUBSCRIBE_UPDATE_NOTIFICATION)}")
    ...
    if 'antispam-force-off' in payload:
        ...
        if value and value not in VALID_BODY_ANTISPAM_FORCE_OFF:
            return (False, f"Invalid antispam-force-off '{value}'. Must be one of: {', '.join(VALID_BODY_ANTISPAM_FORCE_OFF)}")
    ...
    if 'antispam-cache' in payload:
        ...
        if value and value not in VALID_BODY_ANTISPAM_CACHE:
            return (False, f"Invalid antispam-cache '{value}'. Must be one of: {', '.join(VALID_BODY_ANTISPAM_CACHE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 300 or int_val > 86400:
                    return (False, f"antispam-cache-ttl must be between 300 and 86400")
            except (ValueError, TypeError):
                return (False, f"antispam-cache-ttl must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 150:
                    return (False, f"antispam-cache-mpermille must be between 1 and 150")
            except (ValueError, TypeError):
                return (False, f"antispam-cache-mpermille must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"antispam-license must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"antispam-license must be numeric, got: {value}")
    ...
    if 'antispam-expiration' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"antispam-expiration must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"antispam-expiration must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"antispam-timeout must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"antispam-timeout must be numeric, got: {value}")
    ...
    if 'outbreak-prevention-force-off' in payload:
        ...
        if value and value not in VALID_BODY_OUTBREAK_PREVENTION_FORCE_OFF:
            return (False, f"Invalid outbreak-prevention-force-off '{value}'. Must be one of: {', '.join(VALID_BODY_OUTBREAK_PREVENTION_FORCE_OFF)}")
    ...
    if 'outbreak-prevention-cache' in payload:
        ...
        if value and value not in VALID_BODY_OUTBREAK_PREVENTION_CACHE:
            return (False, f"Invalid outbreak-prevention-cache '{value}'. Must be one of: {', '.join(VALID_BODY_OUTBREAK_PREVENTION_CACHE)}")
    ...
    if 'outbreak-prevention-cache-ttl' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 300 or int_val > 86400:
                    return (False, f"outbreak-prevention-cache-ttl must be between 300 and 86400")
            except (ValueError, TypeError):
                return (False, f"outbreak-prevention-cache-ttl must be numeric, got: {value}")
    ...
    if 'outbreak-prevention-cache-mpermille' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 150:
                    return (False, f"outbreak-prevention-cache-mpermille must be between 1 and 150")
            except (ValueError, TypeError):
                return (False, f"outbreak-prevention-cache-mpermille must be numeric, got: {value}")
    ...
    if 'outbreak-prevention-license' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"outbreak-prevention-license must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"outbreak-prevention-license must be numeric, got: {value}")
    ...
    if 'outbreak-prevention-expiration' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"outbreak-prevention-expiration must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"outbreak-prevention-expiration must be numeric, got: {value}")
    ...
    if 'outbreak-prevention-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 30:
                    return (False, f"outbreak-prevention-timeout must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"outbreak-prevention-timeout must be numeric, got: {value}")
    ...
    if 'webfilter-force-off' in payload:
        ...
        if value and value not in VALID_BODY_WEBFILTER_FORCE_OFF:
            return (False, f"Invalid webfilter-force-off '{value}'. Must be one of: {', '.join(VALID_BODY_WEBFILTER_FORCE_OFF)}")
    ...
    if 'webfilter-cache' in payload:
        ...
        if value and value not in VALID_BODY_WEBFILTER_CACHE:
            return (False, f"Invalid webfilter-cache '{value}'. Must be one of: {', '.join(VALID_BODY_WEBFILTER_CACHE)}")
    ...
    if 'webfilter-cache-ttl' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 300 or int_val > 86400:
                    return (False, f"webfilter-cache-ttl must be between 300 and 86400")
            except (ValueError, TypeError):
                return (False, f"webfilter-cache-ttl must be numeric, got: {value}")
    ...
    if 'webfilter-license' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"webfilter-license must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"webfilter-license must be numeric, got: {value}")
    ...
    if 'webfilter-expiration' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"webfilter-expiration must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"webfilter-expiration must be numeric, got: {value}")
    ...
    if 'webfilter-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 30:
                    return (False, f"webfilter-timeout must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"webfilter-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"sdns-server-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"sdns-server-port must be numeric, got: {value}")
    ...
    if 'anycast-sdns-server-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"anycast-sdns-server-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"anycast-sdns-server-port must be numeric, got: {value}")
    ...
    if 'sdns-options' in payload:
        ...
        if value and value not in VALID_BODY_SDNS_OPTIONS:
            return (False, f"Invalid sdns-options '{value}'. Must be one of: {', '.join(VALID_BODY_SDNS_OPTIONS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"proxy-server-port must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"proxy-server-port must be numeric, got: {value}")
    ...
    if 'ddns-server-port' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 65535:
                    return (False, f"ddns-server-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"ddns-server-port must be numeric, got: {value}")
