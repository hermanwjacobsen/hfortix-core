...
def validate_sdn_vpn_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'remote-type' in payload:
        ...
        if value and value not in VALID_BODY_REMOTE_TYPE:
            return (False, f"Invalid remote-type '{value}'. Must be one of: {', '.join(VALID_BODY_REMOTE_TYPE)}")
    ...
    if 'routing-type' in payload:
        ...
        if value and value not in VALID_BODY_ROUTING_TYPE:
            return (False, f"Invalid routing-type '{value}'. Must be one of: {', '.join(VALID_BODY_ROUTING_TYPE)}")
    ...
    if 'nat-traversal' in payload:
        ...
        if value and value not in VALID_BODY_NAT_TRAVERSAL:
            return (False, f"Invalid nat-traversal '{value}'. Must be one of: {', '.join(VALID_BODY_NAT_TRAVERSAL)}")
...
def validate_sdn_vpn_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
