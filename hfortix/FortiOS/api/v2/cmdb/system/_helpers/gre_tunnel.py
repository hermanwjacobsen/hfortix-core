...
def validate_gre_tunnel_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'sequence-number-transmission' in payload:
        ...
        if value and value not in VALID_BODY_SEQUENCE_NUMBER_TRANSMISSION:
            return (False, f"Invalid sequence-number-transmission '{value}'. Must be one of: {', '.join(VALID_BODY_SEQUENCE_NUMBER_TRANSMISSION)}")
    ...
    if 'sequence-number-reception' in payload:
        ...
        if value and value not in VALID_BODY_SEQUENCE_NUMBER_RECEPTION:
            return (False, f"Invalid sequence-number-reception '{value}'. Must be one of: {', '.join(VALID_BODY_SEQUENCE_NUMBER_RECEPTION)}")
    ...
    if 'checksum-transmission' in payload:
        ...
        if value and value not in VALID_BODY_CHECKSUM_TRANSMISSION:
            return (False, f"Invalid checksum-transmission '{value}'. Must be one of: {', '.join(VALID_BODY_CHECKSUM_TRANSMISSION)}")
    ...
    if 'checksum-reception' in payload:
        ...
        if value and value not in VALID_BODY_CHECKSUM_RECEPTION:
            return (False, f"Invalid checksum-reception '{value}'. Must be one of: {', '.join(VALID_BODY_CHECKSUM_RECEPTION)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"key-outbound must be between 0 and 4294967295")
    ...
    if 'key-inbound' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"key-inbound must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'dscp-copying' in payload:
        ...
        if value and value not in VALID_BODY_DSCP_COPYING:
            return (False, f"Invalid dscp-copying '{value}'. Must be one of: {', '.join(VALID_BODY_DSCP_COPYING)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 32767:
                    return (False, f"keepalive-interval must be between 0 and 32767")
            except (ValueError, TypeError):
                return (False, f"keepalive-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 255:
                    return (False, f"keepalive-failtimes must be between 1 and 255")
            except (ValueError, TypeError):
                return (False, f"keepalive-failtimes must be numeric, got: {value}")
...
def validate_gre_tunnel_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
