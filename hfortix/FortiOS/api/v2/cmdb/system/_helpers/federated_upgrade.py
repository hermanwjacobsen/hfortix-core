...
def validate_federated_upgrade_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"upgrade-id must be between 0 and 4294967295")
    ...
    if 'ignore-signing-errors' in payload:
        ...
        if value and value not in VALID_BODY_IGNORE_SIGNING_ERRORS:
            return (False, f"Invalid ignore-signing-errors '{value}'. Must be one of: {', '.join(VALID_BODY_IGNORE_SIGNING_ERRORS)}")
