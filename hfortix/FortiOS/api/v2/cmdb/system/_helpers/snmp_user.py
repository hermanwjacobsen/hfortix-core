...
def validate_snmp_user_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'trap-status' in payload:
        ...
        if value and value not in VALID_BODY_TRAP_STATUS:
            return (False, f"Invalid trap-status '{value}'. Must be one of: {', '.join(VALID_BODY_TRAP_STATUS)}")
    ...
    if 'queries' in payload:
        ...
        if value and value not in VALID_BODY_QUERIES:
            return (False, f"Invalid queries '{value}'. Must be one of: {', '.join(VALID_BODY_QUERIES)}")
    ...
    if 'security-level' in payload:
        ...
        if value and value not in VALID_BODY_SECURITY_LEVEL:
            return (False, f"Invalid security-level '{value}'. Must be one of: {', '.join(VALID_BODY_SECURITY_LEVEL)}")
    ...
    if 'auth-proto' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_PROTO:
            return (False, f"Invalid auth-proto '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_PROTO)}")
    ...
    if 'priv-proto' in payload:
        ...
        if value and value not in VALID_BODY_PRIV_PROTO:
            return (False, f"Invalid priv-proto '{value}'. Must be one of: {', '.join(VALID_BODY_PRIV_PROTO)}")
...
def validate_snmp_user_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
