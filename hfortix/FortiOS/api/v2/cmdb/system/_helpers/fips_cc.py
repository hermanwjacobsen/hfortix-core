...
def validate_fips_cc_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 1440:
                    return (False, f"self-test-period must be between 5 and 1440")
            except (ValueError, TypeError):
                return (False, f"self-test-period must be numeric, got: {value}")
    ...
    if 'key-generation-self-test' in payload:
        ...
        if value and value not in VALID_BODY_KEY_GENERATION_SELF_TEST:
            return (False, f"Invalid key-generation-self-test '{value}'. Must be one of: {', '.join(VALID_BODY_KEY_GENERATION_SELF_TEST)}")
