...
def validate_ha_monitor_put(
    ...
    if 'monitor-vlan' in payload:
        ...
        if value and value not in VALID_BODY_MONITOR_VLAN:
            return (False, f"Invalid monitor-vlan '{value}'. Must be one of: {', '.join(VALID_BODY_MONITOR_VLAN)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 30:
                    return (False, f"vlan-hb-interval must be between 1 and 30")
            except (ValueError, TypeError):
                return (False, f"vlan-hb-interval must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 60:
                    return (False, f"vlan-hb-lost-threshold must be between 1 and 60")
            except (ValueError, TypeError):
                return (False, f"vlan-hb-lost-threshold must be numeric, got: {value}")
