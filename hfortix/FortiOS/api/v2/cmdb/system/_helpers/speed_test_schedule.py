...
def validate_speed_test_schedule_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'dynamic-server' in payload:
        ...
        if value and value not in VALID_BODY_DYNAMIC_SERVER:
            return (False, f"Invalid dynamic-server '{value}'. Must be one of: {', '.join(VALID_BODY_DYNAMIC_SERVER)}")
    ...
    if 'update-shaper' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_SHAPER:
            return (False, f"Invalid update-shaper '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_SHAPER)}")
    ...
    if 'update-inbandwidth' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_INBANDWIDTH:
            return (False, f"Invalid update-inbandwidth '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_INBANDWIDTH)}")
    ...
    if 'update-outbandwidth' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_OUTBANDWIDTH:
            return (False, f"Invalid update-outbandwidth '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_OUTBANDWIDTH)}")
    ...
    if 'update-interface-shaping' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_INTERFACE_SHAPING:
            return (False, f"Invalid update-interface-shaping '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_INTERFACE_SHAPING)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 16776000:
                    return (False, f"update-inbandwidth-maximum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"update-inbandwidth-maximum must be numeric, got: {value}")
    ...
    if 'update-inbandwidth-minimum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"update-inbandwidth-minimum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"update-inbandwidth-minimum must be numeric, got: {value}")
    ...
    if 'update-outbandwidth-maximum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"update-outbandwidth-maximum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"update-outbandwidth-maximum must be numeric, got: {value}")
    ...
    if 'update-outbandwidth-minimum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"update-outbandwidth-minimum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"update-outbandwidth-minimum must be numeric, got: {value}")
    ...
    if 'expected-inbandwidth-minimum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"expected-inbandwidth-minimum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"expected-inbandwidth-minimum must be numeric, got: {value}")
    ...
    if 'expected-inbandwidth-maximum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"expected-inbandwidth-maximum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"expected-inbandwidth-maximum must be numeric, got: {value}")
    ...
    if 'expected-outbandwidth-minimum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"expected-outbandwidth-minimum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"expected-outbandwidth-minimum must be numeric, got: {value}")
    ...
    if 'expected-outbandwidth-maximum' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 16776000:
                    return (False, f"expected-outbandwidth-maximum must be between 0 and 16776000")
            except (ValueError, TypeError):
                return (False, f"expected-outbandwidth-maximum must be numeric, got: {value}")
