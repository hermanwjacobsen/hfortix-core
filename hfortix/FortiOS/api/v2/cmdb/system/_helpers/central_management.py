...
def validate_central_management_put(
    ...
    if 'fortigate-cloud-sso-default-profile' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"fortigate-cloud-sso-default-profile cannot exceed 35 characters")
    ...
    if 'schedule-config-restore' in payload:
        ...
        if value and value not in VALID_BODY_SCHEDULE_CONFIG_RESTORE:
            return (False, f"Invalid schedule-config-restore '{value}'. Must be one of: {', '.join(VALID_BODY_SCHEDULE_CONFIG_RESTORE)}")
    ...
    if 'schedule-script-restore' in payload:
        ...
        if value and value not in VALID_BODY_SCHEDULE_SCRIPT_RESTORE:
            return (False, f"Invalid schedule-script-restore '{value}'. Must be one of: {', '.join(VALID_BODY_SCHEDULE_SCRIPT_RESTORE)}")
    ...
    if 'allow-push-configuration' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_PUSH_CONFIGURATION:
            return (False, f"Invalid allow-push-configuration '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_PUSH_CONFIGURATION)}")
    ...
    if 'allow-push-firmware' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_PUSH_FIRMWARE:
            return (False, f"Invalid allow-push-firmware '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_PUSH_FIRMWARE)}")
    ...
    if 'allow-remote-firmware-upgrade' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_REMOTE_FIRMWARE_UPGRADE:
            return (False, f"Invalid allow-remote-firmware-upgrade '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_REMOTE_FIRMWARE_UPGRADE)}")
    ...
    if 'allow-monitor' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_MONITOR:
            return (False, f"Invalid allow-monitor '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_MONITOR)}")
    ...
    if 'fmg-update-port' in payload:
        ...
        if value and value not in VALID_BODY_FMG_UPDATE_PORT:
            return (False, f"Invalid fmg-update-port '{value}'. Must be one of: {', '.join(VALID_BODY_FMG_UPDATE_PORT)}")
    ...
    if 'fmg-update-http-header' in payload:
        ...
        if value and value not in VALID_BODY_FMG_UPDATE_HTTP_HEADER:
            return (False, f"Invalid fmg-update-http-header '{value}'. Must be one of: {', '.join(VALID_BODY_FMG_UPDATE_HTTP_HEADER)}")
    ...
    if 'include-default-servers' in payload:
        ...
        if value and value not in VALID_BODY_INCLUDE_DEFAULT_SERVERS:
            return (False, f"Invalid include-default-servers '{value}'. Must be one of: {', '.join(VALID_BODY_INCLUDE_DEFAULT_SERVERS)}")
