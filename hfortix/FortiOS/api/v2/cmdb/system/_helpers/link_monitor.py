...
VALID_BODY_PROTOCOL = ['ping', 'tcp-echo', 'udp-echo', 'http', 'https', 'twamp']
...
def validate_link_monitor_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'addr-mode' in payload:
        ...
        if value and value not in VALID_BODY_ADDR_MODE:
            return (False, f"Invalid addr-mode '{value}'. Must be one of: {', '.join(VALID_BODY_ADDR_MODE)}")
    ...
    if 'server-config' in payload:
        ...
        if value and value not in VALID_BODY_SERVER_CONFIG:
            return (False, f"Invalid server-config '{value}'. Must be one of: {', '.join(VALID_BODY_SERVER_CONFIG)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 20 or int_val > 5000:
                    return (False, f"probe-timeout must be between 20 and 5000")
            except (ValueError, TypeError):
    ...
    if 'update-cascade-interface' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_CASCADE_INTERFACE:
            return (False, f"Invalid update-cascade-interface '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_CASCADE_INTERFACE)}")
    ...
    if 'update-static-route' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_STATIC_ROUTE:
            return (False, f"Invalid update-static-route '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_STATIC_ROUTE)}")
    ...
    if 'update-policy-route' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_POLICY_ROUTE:
            return (False, f"Invalid update-policy-route '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_POLICY_ROUTE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"class-id must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'service-detection' in payload:
        ...
        if value and value not in VALID_BODY_SERVICE_DETECTION:
            return (False, f"Invalid service-detection '{value}'. Must be one of: {', '.join(VALID_BODY_SERVICE_DETECTION)}")
...
def validate_link_monitor_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
