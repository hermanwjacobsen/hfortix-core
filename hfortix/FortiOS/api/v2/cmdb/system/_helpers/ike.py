...
def validate_ike_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 50 or int_val > 20000:
                    return (False, f"embryonic-limit must be between 50 and 20000")
            except (ValueError, TypeError):
                return (False, f"embryonic-limit must be numeric, got: {value}")
    ...
    if 'dh-multiprocess' in payload:
        ...
        if value and value not in VALID_BODY_DH_MULTIPROCESS:
            return (False, f"Invalid dh-multiprocess '{value}'. Must be one of: {', '.join(VALID_BODY_DH_MULTIPROCESS)}")
    ...
    if 'dh-worker-count' in payload:
        ...
        if value is not None:
            try:
                ...
                return (False, f"dh-worker-count must be numeric, got: {value}")
    
    # Validate dh-mode if present
    if 'dh-mode' in payload:
        value = payload.get('dh-mode')
        if value and value not in VALID_BODY_DH_MODE:
            return (False, f"Invalid dh-mode '{value}'. Must be one of: {', '.join(VALID_BODY_DH_MODE)}")
    ...
    if 'dh-keypair-cache' in payload:
        ...
        if value and value not in VALID_BODY_DH_KEYPAIR_CACHE:
            return (False, f"Invalid dh-keypair-cache '{value}'. Must be one of: {', '.join(VALID_BODY_DH_KEYPAIR_CACHE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 50000:
                    return (False, f"dh-keypair-count must be between 0 and 50000")
            except (ValueError, TypeError):
                return (False, f"dh-keypair-count must be numeric, got: {value}")
    ...
    if 'dh-keypair-throttle' in payload:
        ...
        if value and value not in VALID_BODY_DH_KEYPAIR_THROTTLE:
            return (False, f"Invalid dh-keypair-throttle '{value}'. Must be one of: {', '.join(VALID_BODY_DH_KEYPAIR_THROTTLE)}")
