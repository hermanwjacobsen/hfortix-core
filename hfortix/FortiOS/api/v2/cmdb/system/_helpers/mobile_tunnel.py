...
def validate_mobile_tunnel_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"n-mhae-spi must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'n-mhae-key-type' in payload:
        ...
        if value and value not in VALID_BODY_N_MHAE_KEY_TYPE:
            return (False, f"Invalid n-mhae-key-type '{value}'. Must be one of: {', '.join(VALID_BODY_N_MHAE_KEY_TYPE)}")
    ...
    if 'hash-algorithm' in payload:
        ...
        if value and value not in VALID_BODY_HASH_ALGORITHM:
            return (False, f"Invalid hash-algorithm '{value}'. Must be one of: {', '.join(VALID_BODY_HASH_ALGORITHM)}")
...
def validate_mobile_tunnel_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
