...
VALID_BODY_EVENT_TYPE = ['ioc', 'event-log', 'reboot', 'low-memory', 'high-cpu', 'license-near-expiry', 'local-cert-near-expiry', 'ha-failover', 'config-change', 'security-rating-summary', 'virus-ips-db-updated', 'faz-event', 'incoming-webhook', 'fabric-event', 'ips-logs', 'anomaly-logs', 'virus-logs', 'ssh-logs', 'webfilter-violation', 'traffic-violation', 'stitch']
VALID_BODY_LICENSE_TYPE = ['forticare-support', 'fortiguard-webfilter', 'fortiguard-antispam', 'fortiguard-antivirus', 'fortiguard-ips', 'fortiguard-management', 'forticloud', 'any']
VALID_BODY_REPORT_TYPE = ['posture', 'coverage', 'optimization', 'any']
VALID_BODY_TRIGGER_FREQUENCY = ['hourly', 'daily', 'weekly', 'monthly', 'once']
VALID_BODY_TRIGGER_WEEKDAY = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']
...
def validate_automation_trigger_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'trigger-type' in payload:
        ...
        if value and value not in VALID_BODY_TRIGGER_TYPE:
            return (False, f"Invalid trigger-type '{value}'. Must be one of: {', '.join(VALID_BODY_TRIGGER_TYPE)}")
    ...
    if 'event-type' in payload:
        ...
        if value and value not in VALID_BODY_EVENT_TYPE:
            return (False, f"Invalid event-type '{value}'. Must be one of: {', '.join(VALID_BODY_EVENT_TYPE)}")
    ...
    if 'license-type' in payload:
        ...
        if value and value not in VALID_BODY_LICENSE_TYPE:
            return (False, f"Invalid license-type '{value}'. Must be one of: {', '.join(VALID_BODY_LICENSE_TYPE)}")
    ...
    if 'report-type' in payload:
        ...
        if value and value not in VALID_BODY_REPORT_TYPE:
            return (False, f"Invalid report-type '{value}'. Must be one of: {', '.join(VALID_BODY_REPORT_TYPE)}")
    ...
    if 'trigger-frequency' in payload:
        ...
        if value and value not in VALID_BODY_TRIGGER_FREQUENCY:
            return (False, f"Invalid trigger-frequency '{value}'. Must be one of: {', '.join(VALID_BODY_TRIGGER_FREQUENCY)}")
    ...
    if 'trigger-weekday' in payload:
        ...
        if value and value not in VALID_BODY_TRIGGER_WEEKDAY:
            return (False, f"Invalid trigger-weekday '{value}'. Must be one of: {', '.join(VALID_BODY_TRIGGER_WEEKDAY)}")
    ...
    if 'fabric-event-severity' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 255:
            return (False, f"fabric-event-severity cannot exceed 255 characters")
...
def validate_automation_trigger_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
