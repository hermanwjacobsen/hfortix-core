...
def validate_ipam_put(
    ...
    if 'automatic-conflict-resolution' in payload:
        ...
        if value and value not in VALID_BODY_AUTOMATIC_CONFLICT_RESOLUTION:
            return (False, f"Invalid automatic-conflict-resolution '{value}'. Must be one of: {', '.join(VALID_BODY_AUTOMATIC_CONFLICT_RESOLUTION)}")
    ...
    if 'require-subnet-size-match' in payload:
        ...
        if value and value not in VALID_BODY_REQUIRE_SUBNET_SIZE_MATCH:
            return (False, f"Invalid require-subnet-size-match '{value}'. Must be one of: {', '.join(VALID_BODY_REQUIRE_SUBNET_SIZE_MATCH)}")
    ...
    if 'manage-lan-addresses' in payload:
        ...
        if value and value not in VALID_BODY_MANAGE_LAN_ADDRESSES:
            return (False, f"Invalid manage-lan-addresses '{value}'. Must be one of: {', '.join(VALID_BODY_MANAGE_LAN_ADDRESSES)}")
    ...
    if 'manage-lan-extension-addresses' in payload:
        ...
        if value and value not in VALID_BODY_MANAGE_LAN_EXTENSION_ADDRESSES:
            return (False, f"Invalid manage-lan-extension-addresses '{value}'. Must be one of: {', '.join(VALID_BODY_MANAGE_LAN_EXTENSION_ADDRESSES)}")
    ...
    if 'manage-ssid-addresses' in payload:
        ...
        if value and value not in VALID_BODY_MANAGE_SSID_ADDRESSES:
            return (False, f"Invalid manage-ssid-addresses '{value}'. Must be one of: {', '.join(VALID_BODY_MANAGE_SSID_ADDRESSES)}")
