...
def validate_dns_server_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'doh' in payload:
        ...
        if value and value not in VALID_BODY_DOH:
            return (False, f"Invalid doh '{value}'. Must be one of: {', '.join(VALID_BODY_DOH)}")
    
    # Validate doh3 if present
    if 'doh3' in payload:
        value = payload.get('doh3')
        if value and value not in VALID_BODY_DOH3:
            return (False, f"Invalid doh3 '{value}'. Must be one of: {', '.join(VALID_BODY_DOH3)}")
    
    # Validate doq if present
    if 'doq' in payload:
        value = payload.get('doq')
        if value and value not in VALID_BODY_DOQ:
            return (False, f"Invalid doq '{value}'. Must be one of: {', '.join(VALID_BODY_DOQ)}")
...
def validate_dns_server_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
