...
def validate_network_visibility_put(
    ...
    if 'destination-visibility' in payload:
        ...
        if value and value not in VALID_BODY_DESTINATION_VISIBILITY:
            return (False, f"Invalid destination-visibility '{value}'. Must be one of: {', '.join(VALID_BODY_DESTINATION_VISIBILITY)}")
    ...
    if 'source-location' in payload:
        ...
        if value and value not in VALID_BODY_SOURCE_LOCATION:
            return (False, f"Invalid source-location '{value}'. Must be one of: {', '.join(VALID_BODY_SOURCE_LOCATION)}")
    ...
    if 'destination-hostname-visibility' in payload:
        ...
        if value and value not in VALID_BODY_DESTINATION_HOSTNAME_VISIBILITY:
            return (False, f"Invalid destination-hostname-visibility '{value}'. Must be one of: {', '.join(VALID_BODY_DESTINATION_HOSTNAME_VISIBILITY)}")
    ...
    if 'destination-location' in payload:
        ...
        if value and value not in VALID_BODY_DESTINATION_LOCATION:
            return (False, f"Invalid destination-location '{value}'. Must be one of: {', '.join(VALID_BODY_DESTINATION_LOCATION)}")
