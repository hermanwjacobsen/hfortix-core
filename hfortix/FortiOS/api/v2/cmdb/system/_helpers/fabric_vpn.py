...
def validate_fabric_vpn_put(
    ...
    if 'sync-mode' in payload:
        ...
        if value and value not in VALID_BODY_SYNC_MODE:
            return (False, f"Invalid sync-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SYNC_MODE)}")
    ...
    if 'policy-rule' in payload:
        ...
        if value and value not in VALID_BODY_POLICY_RULE:
            return (False, f"Invalid policy-rule '{value}'. Must be one of: {', '.join(VALID_BODY_POLICY_RULE)}")
    ...
    if 'vpn-role' in payload:
        ...
        if value and value not in VALID_BODY_VPN_ROLE:
            return (False, f"Invalid vpn-role '{value}'. Must be one of: {', '.join(VALID_BODY_VPN_ROLE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"loopback-advertised-subnet must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"loopback-advertised-subnet must be numeric, got: {value}")
