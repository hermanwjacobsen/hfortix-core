    ...
    if 'tcp-match-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 1800:
                    return (False, f"tcp-match-timeout must be between 0 and 1800")
            except (ValueError, TypeError):
                return (False, f"tcp-match-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 300:
                    return (False, f"tcp-halfopen-match-timeout must be between 0 and 300")
            except (ValueError, TypeError):
                return (False, f"tcp-halfopen-match-timeout must be numeric, got: {value}")
