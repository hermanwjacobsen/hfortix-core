...
def validate_automation_condition_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'condition-type' in payload:
        ...
        if value and value not in VALID_BODY_CONDITION_TYPE:
            return (False, f"Invalid condition-type '{value}'. Must be one of: {', '.join(VALID_BODY_CONDITION_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100:
                    return (False, f"cpu-usage-percent must be between 0 and 100")
            except (ValueError, TypeError):
                return (False, f"cpu-usage-percent must be numeric, got: {value}")
    ...
    if 'mem-usage-percent' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 100:
                    return (False, f"mem-usage-percent must be between 0 and 100")
            except (ValueError, TypeError):
                return (False, f"mem-usage-percent must be numeric, got: {value}")
    ...
    if 'vpn-tunnel-state' in payload:
        ...
        if value and value not in VALID_BODY_VPN_TUNNEL_STATE:
            return (False, f"Invalid vpn-tunnel-state '{value}'. Must be one of: {', '.join(VALID_BODY_VPN_TUNNEL_STATE)}")
...
def validate_automation_condition_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
