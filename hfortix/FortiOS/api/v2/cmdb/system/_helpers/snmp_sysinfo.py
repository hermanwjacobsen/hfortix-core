...
def validate_snmp_sysinfo_put(
    ...
    if 'engine-id-type' in payload:
        ...
        if value and value not in VALID_BODY_ENGINE_ID_TYPE:
            return (False, f"Invalid engine-id-type '{value}'. Must be one of: {', '.join(VALID_BODY_ENGINE_ID_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 100:
                    return (False, f"trap-high-cpu-threshold must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"trap-high-cpu-threshold must be numeric, got: {value}")
    ...
    if 'trap-low-memory-threshold' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 100:
                    return (False, f"trap-low-memory-threshold must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"trap-low-memory-threshold must be numeric, got: {value}")
    ...
    if 'trap-log-full-threshold' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 100:
                    return (False, f"trap-log-full-threshold must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"trap-log-full-threshold must be numeric, got: {value}")
    ...
    if 'trap-free-memory-threshold' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 100:
                    return (False, f"trap-free-memory-threshold must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"trap-free-memory-threshold must be numeric, got: {value}")
    ...
    if 'trap-freeable-memory-threshold' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 100:
                    return (False, f"trap-freeable-memory-threshold must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"trap-freeable-memory-threshold must be numeric, got: {value}")
    ...
    if 'append-index' in payload:
        ...
        if value and value not in VALID_BODY_APPEND_INDEX:
            return (False, f"Invalid append-index '{value}'. Must be one of: {', '.join(VALID_BODY_APPEND_INDEX)}")
    ...
    if 'non-mgmt-vdom-query' in payload:
        ...
        if value and value not in VALID_BODY_NON_MGMT_VDOM_QUERY:
            return (False, f"Invalid non-mgmt-vdom-query '{value}'. Must be one of: {', '.join(VALID_BODY_NON_MGMT_VDOM_QUERY)}")
