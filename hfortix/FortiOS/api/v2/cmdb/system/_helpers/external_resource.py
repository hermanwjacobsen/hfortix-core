...
VALID_BODY_TYPE = ['category', 'domain', 'malware', 'address', 'mac-address', 'data', 'generic-address']
...
def validate_external_resource_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'address-comment-field' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 511:
            return (False, f"address-comment-field cannot exceed 511 characters")
    ...
    if 'update-method' in payload:
        ...
        if value and value not in VALID_BODY_UPDATE_METHOD:
            return (False, f"Invalid update-method '{value}'. Must be one of: {', '.join(VALID_BODY_UPDATE_METHOD)}")
...
def validate_external_resource_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
