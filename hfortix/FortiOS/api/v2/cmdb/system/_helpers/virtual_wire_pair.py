...
def validate_virtual_wire_pair_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'wildcard-vlan' in payload:
        ...
        if value and value not in VALID_BODY_WILDCARD_VLAN:
            return (False, f"Invalid wildcard-vlan '{value}'. Must be one of: {', '.join(VALID_BODY_WILDCARD_VLAN)}")
...
def validate_virtual_wire_pair_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
