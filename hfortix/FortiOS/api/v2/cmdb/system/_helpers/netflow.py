...
def validate_netflow_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 3600:
                    return (False, f"active-flow-timeout must be between 60 and 3600")
            except (ValueError, TypeError):
                return (False, f"active-flow-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 600:
                    return (False, f"inactive-flow-timeout must be between 10 and 600")
            except (ValueError, TypeError):
                return (False, f"inactive-flow-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 86400:
                    return (False, f"template-tx-timeout must be between 60 and 86400")
            except (ValueError, TypeError):
                return (False, f"template-tx-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 6000:
                    return (False, f"template-tx-counter must be between 10 and 6000")
            except (ValueError, TypeError):
                return (False, f"template-tx-counter must be numeric, got: {value}")
    ...
    if 'session-cache-size' in payload:
        ...
        if value and value not in VALID_BODY_SESSION_CACHE_SIZE:
            return (False, f"Invalid session-cache-size '{value}'. Must be one of: {', '.join(VALID_BODY_SESSION_CACHE_SIZE)}")
