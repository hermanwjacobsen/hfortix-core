...
VALID_BODY_CATEGORY = ['activity-control', 'tenant-control', 'domain-control', 'safe-search-control', 'advanced-tenant-control', 'other']
...
def validate_user_activity_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'match-strategy' in payload:
        ...
        if value and value not in VALID_BODY_MATCH_STRATEGY:
            return (False, f"Invalid match-strategy '{value}'. Must be one of: {', '.join(VALID_BODY_MATCH_STRATEGY)}")
...
def validate_user_activity_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
