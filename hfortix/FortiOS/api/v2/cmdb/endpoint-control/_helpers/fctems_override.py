...
VALID_BODY_CAPABILITIES = ['fabric-auth', 'silent-approval', 'websocket', 'websocket-malware', 'push-ca-certs', 'common-tags-api', 'tenant-id', 'client-avatars', 'single-vdom-connector', 'fgt-sysinfo-api', 'ztna-server-info', 'used-tags']
...
def validate_fctems_override_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'dirty-reason' in payload:
        ...
        if value and value not in VALID_BODY_DIRTY_REASON:
            return (False, f"Invalid dirty-reason '{value}'. Must be one of: {', '.join(VALID_BODY_DIRTY_REASON)}")
    ...
    if 'fortinetone-cloud-authentication' in payload:
        ...
        if value and value not in VALID_BODY_FORTINETONE_CLOUD_AUTHENTICATION:
            return (False, f"Invalid fortinetone-cloud-authentication '{value}'. Must be one of: {', '.join(VALID_BODY_FORTINETONE_CLOUD_AUTHENTICATION)}")
    ...
    if 'pull-sysinfo' in payload:
        ...
        if value and value not in VALID_BODY_PULL_SYSINFO:
            return (False, f"Invalid pull-sysinfo '{value}'. Must be one of: {', '.join(VALID_BODY_PULL_SYSINFO)}")
    ...
    if 'pull-vulnerabilities' in payload:
        ...
        if value and value not in VALID_BODY_PULL_VULNERABILITIES:
            return (False, f"Invalid pull-vulnerabilities '{value}'. Must be one of: {', '.join(VALID_BODY_PULL_VULNERABILITIES)}")
    ...
    if 'pull-tags' in payload:
        ...
        if value and value not in VALID_BODY_PULL_TAGS:
            return (False, f"Invalid pull-tags '{value}'. Must be one of: {', '.join(VALID_BODY_PULL_TAGS)}")
    ...
    if 'pull-malware-hash' in payload:
        ...
        if value and value not in VALID_BODY_PULL_MALWARE_HASH:
            return (False, f"Invalid pull-malware-hash '{value}'. Must be one of: {', '.join(VALID_BODY_PULL_MALWARE_HASH)}")
    
    # Validate capabilities if present
    if 'capabilities' in payload:
        value = payload.get('capabilities')
        if value and value not in VALID_BODY_CAPABILITIES:
            return (False, f"Invalid capabilities '{value}'. Must be one of: {', '.join(VALID_BODY_CAPABILITIES)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 10 or int_val > 3600:
                    return (False, f"out-of-sync-threshold must be between 10 and 3600")
            except (ValueError, TypeError):
                return (False, f"out-of-sync-threshold must be numeric, got: {value}")
    ...
    if 'send-tags-to-all-vdoms' in payload:
        ...
        if value and value not in VALID_BODY_SEND_TAGS_TO_ALL_VDOMS:
            return (False, f"Invalid send-tags-to-all-vdoms '{value}'. Must be one of: {', '.join(VALID_BODY_SEND_TAGS_TO_ALL_VDOMS)}")
    ...
    if 'websocket-override' in payload:
        ...
        if value and value not in VALID_BODY_WEBSOCKET_OVERRIDE:
            return (False, f"Invalid websocket-override '{value}'. Must be one of: {', '.join(VALID_BODY_WEBSOCKET_OVERRIDE)}")
    ...
    if 'trust-ca-cn' in payload:
        ...
        if value and value not in VALID_BODY_TRUST_CA_CN:
            return (False, f"Invalid trust-ca-cn '{value}'. Must be one of: {', '.join(VALID_BODY_TRUST_CA_CN)}")
