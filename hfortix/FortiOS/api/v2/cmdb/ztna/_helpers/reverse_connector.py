...
def validate_reverse_connector_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 600:
                    return (False, f"health-check-interval must be between 0 and 600")
            except (ValueError, TypeError):
                return (False, f"health-check-interval must be numeric, got: {value}")
...
def validate_reverse_connector_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
