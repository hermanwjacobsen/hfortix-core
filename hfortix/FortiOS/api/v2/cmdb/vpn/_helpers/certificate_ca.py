...
def validate_certificate_ca_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'ssl-inspection-trusted' in payload:
        ...
        if value and value not in VALID_BODY_SSL_INSPECTION_TRUSTED:
            return (False, f"Invalid ssl-inspection-trusted '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_INSPECTION_TRUSTED)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"auto-update-days must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"auto-update-days must be numeric, got: {value}")
    ...
    if 'auto-update-days-warning' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"auto-update-days-warning must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"auto-update-days-warning must be numeric, got: {value}")
    ...
    if 'obsolete' in payload:
        ...
        if value and value not in VALID_BODY_OBSOLETE:
            return (False, f"Invalid obsolete '{value}'. Must be one of: {', '.join(VALID_BODY_OBSOLETE)}")
    ...
    if 'fabric-ca' in payload:
        ...
        if value and value not in VALID_BODY_FABRIC_CA:
            return (False, f"Invalid fabric-ca '{value}'. Must be one of: {', '.join(VALID_BODY_FABRIC_CA)}")
