...
VALID_BODY_ENC_ALG = ['null', 'des', '3des', 'aes128', 'aes192', 'aes256', 'aria128', 'aria192', 'aria256', 'seed']
...
def validate_ipsec_manualkey_interface_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'auth-alg' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_ALG:
            return (False, f"Invalid auth-alg '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_ALG)}")
    
    # Validate enc-alg if present
    if 'enc-alg' in payload:
        value = payload.get('enc-alg')
        if value and value not in VALID_BODY_ENC_ALG:
            return (False, f"Invalid enc-alg '{value}'. Must be one of: {', '.join(VALID_BODY_ENC_ALG)}")
...
def validate_ipsec_manualkey_interface_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
