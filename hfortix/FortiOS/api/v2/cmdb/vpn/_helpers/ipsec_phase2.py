...
def validate_ipsec_phase2_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'use-natip' in payload:
        ...
        if value and value not in VALID_BODY_USE_NATIP:
            return (False, f"Invalid use-natip '{value}'. Must be one of: {', '.join(VALID_BODY_USE_NATIP)}")
    ...
    if 'selector-match' in payload:
        ...
        if value and value not in VALID_BODY_SELECTOR_MATCH:
            return (False, f"Invalid selector-match '{value}'. Must be one of: {', '.join(VALID_BODY_SELECTOR_MATCH)}")
...
def validate_ipsec_phase2_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
