...
def validate_l2tp_put(
    ...
    if 'enforce-ipsec' in payload:
        ...
        if value and value not in VALID_BODY_ENFORCE_IPSEC:
            return (False, f"Invalid enforce-ipsec '{value}'. Must be one of: {', '.join(VALID_BODY_ENFORCE_IPSEC)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 3600:
                    return (False, f"hello-interval must be between 0 and 3600")
            except (ValueError, TypeError):
    ...
    if 'compress' in payload:
        ...
        if value and value not in VALID_BODY_COMPRESS:
            return (False, f"Invalid compress '{value}'. Must be one of: {', '.join(VALID_BODY_COMPRESS)}")
