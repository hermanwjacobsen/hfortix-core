...
VALID_BODY_GCH_CRYPTOKEY_ALGORITHM = ['rsa-sign-pkcs1-2048-sha256', 'rsa-sign-pkcs1-3072-sha256', 'rsa-sign-pkcs1-4096-sha256', 'rsa-sign-pkcs1-4096-sha512', 'rsa-sign-pss-2048-sha256', 'rsa-sign-pss-3072-sha256', 'rsa-sign-pss-4096-sha256', 'rsa-sign-pss-4096-sha512', 'ec-sign-p256-sha256', 'ec-sign-p384-sha384', 'ec-sign-secp256k1-sha256']
...
def validate_certificate_hsm_local_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'vendor' in payload:
        ...
        if value and value not in VALID_BODY_VENDOR:
            return (False, f"Invalid vendor '{value}'. Must be one of: {', '.join(VALID_BODY_VENDOR)}")
    ...
    if 'api-version' in payload:
        ...
        if value and value not in VALID_BODY_API_VERSION:
            return (False, f"Invalid api-version '{value}'. Must be one of: {', '.join(VALID_BODY_API_VERSION)}")
    ...
    if 'gch-cryptokey-version' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 31:
            return (False, f"gch-cryptokey-version cannot exceed 31 characters")
    
    # Validate gch-cloud-service-name if present
    if 'gch-cloud-service-name' in payload:
        value = payload.get('gch-cloud-service-name')
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"gch-cloud-service-name cannot exceed 35 characters")
    ...
    if 'gch-cryptokey-algorithm' in payload:
        ...
        if value and value not in VALID_BODY_GCH_CRYPTOKEY_ALGORITHM:
            return (False, f"Invalid gch-cryptokey-algorithm '{value}'. Must be one of: {', '.join(VALID_BODY_GCH_CRYPTOKEY_ALGORITHM)}")
...
def validate_certificate_hsm_local_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
