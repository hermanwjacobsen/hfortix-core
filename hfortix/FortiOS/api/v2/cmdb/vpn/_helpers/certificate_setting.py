...
def validate_certificate_setting_put(
    ...
    if 'ocsp-status' in payload:
        ...
        if value and value not in VALID_BODY_OCSP_STATUS:
            return (False, f"Invalid ocsp-status '{value}'. Must be one of: {', '.join(VALID_BODY_OCSP_STATUS)}")
    ...
    if 'ocsp-option' in payload:
        ...
        if value and value not in VALID_BODY_OCSP_OPTION:
            return (False, f"Invalid ocsp-option '{value}'. Must be one of: {', '.join(VALID_BODY_OCSP_OPTION)}")
    ...
    if 'check-ca-cert' in payload:
        ...
        if value and value not in VALID_BODY_CHECK_CA_CERT:
            return (False, f"Invalid check-ca-cert '{value}'. Must be one of: {', '.join(VALID_BODY_CHECK_CA_CERT)}")
    ...
    if 'check-ca-chain' in payload:
        ...
        if value and value not in VALID_BODY_CHECK_CA_CHAIN:
            return (False, f"Invalid check-ca-chain '{value}'. Must be one of: {', '.join(VALID_BODY_CHECK_CA_CHAIN)}")
    ...
    if 'subject-match' in payload:
        ...
        if value and value not in VALID_BODY_SUBJECT_MATCH:
            return (False, f"Invalid subject-match '{value}'. Must be one of: {', '.join(VALID_BODY_SUBJECT_MATCH)}")
    ...
    if 'subject-set' in payload:
        ...
        if value and value not in VALID_BODY_SUBJECT_SET:
            return (False, f"Invalid subject-set '{value}'. Must be one of: {', '.join(VALID_BODY_SUBJECT_SET)}")
    ...
    if 'cn-match' in payload:
        ...
        if value and value not in VALID_BODY_CN_MATCH:
            return (False, f"Invalid cn-match '{value}'. Must be one of: {', '.join(VALID_BODY_CN_MATCH)}")
    ...
    if 'cn-allow-multi' in payload:
        ...
        if value and value not in VALID_BODY_CN_ALLOW_MULTI:
            return (False, f"Invalid cn-allow-multi '{value}'. Must be one of: {', '.join(VALID_BODY_CN_ALLOW_MULTI)}")
    ...
    if 'strict-ocsp-check' in payload:
        ...
        if value and value not in VALID_BODY_STRICT_OCSP_CHECK:
            return (False, f"Invalid strict-ocsp-check '{value}'. Must be one of: {', '.join(VALID_BODY_STRICT_OCSP_CHECK)}")
    ...
    if 'cmp-save-extra-certs' in payload:
        ...
        if value and value not in VALID_BODY_CMP_SAVE_EXTRA_CERTS:
            return (False, f"Invalid cmp-save-extra-certs '{value}'. Must be one of: {', '.join(VALID_BODY_CMP_SAVE_EXTRA_CERTS)}")
    ...
    if 'cmp-key-usage-checking' in payload:
        ...
        if value and value not in VALID_BODY_CMP_KEY_USAGE_CHECKING:
            return (False, f"Invalid cmp-key-usage-checking '{value}'. Must be one of: {', '.join(VALID_BODY_CMP_KEY_USAGE_CHECKING)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100:
                    return (False, f"cert-expire-warning must be between 0 and 100")
            except (ValueError, TypeError):
                return (False, f"cert-expire-warning must be numeric, got: {value}")
