...
def validate_certificate_local_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'range' in payload:
        ...
        if value and value not in VALID_BODY_RANGE:
            return (False, f"Invalid range '{value}'. Must be one of: {', '.join(VALID_BODY_RANGE)}")
    
    # Validate source if present
    if 'source' in payload:
        value = payload.get('source')
        if value and value not in VALID_BODY_SOURCE:
            return (False, f"Invalid source '{value}'. Must be one of: {', '.join(VALID_BODY_SOURCE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"auto-regenerate-days must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"auto-regenerate-days must be numeric, got: {value}")
    ...
    if 'auto-regenerate-days-warning' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"auto-regenerate-days-warning must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"auto-regenerate-days-warning must be numeric, got: {value}")
    ...
    if 'name-encoding' in payload:
        ...
        if value and value not in VALID_BODY_NAME_ENCODING:
            return (False, f"Invalid name-encoding '{value}'. Must be one of: {', '.join(VALID_BODY_NAME_ENCODING)}")
    ...
    if 'ike-localid-type' in payload:
        ...
        if value and value not in VALID_BODY_IKE_LOCALID_TYPE:
            return (False, f"Invalid ike-localid-type '{value}'. Must be one of: {', '.join(VALID_BODY_IKE_LOCALID_TYPE)}")
    ...
    if 'enroll-protocol' in payload:
        ...
        if value and value not in VALID_BODY_ENROLL_PROTOCOL:
            return (False, f"Invalid enroll-protocol '{value}'. Must be one of: {', '.join(VALID_BODY_ENROLL_PROTOCOL)}")
    ...
    if 'private-key-retain' in payload:
        ...
        if value and value not in VALID_BODY_PRIVATE_KEY_RETAIN:
            return (False, f"Invalid private-key-retain '{value}'. Must be one of: {', '.join(VALID_BODY_PRIVATE_KEY_RETAIN)}")
    ...
    if 'cmp-regeneration-method' in payload:
        ...
        if value and value not in VALID_BODY_CMP_REGENERATION_METHOD:
            return (False, f"Invalid cmp-regeneration-method '{value}'. Must be one of: {', '.join(VALID_BODY_CMP_REGENERATION_METHOD)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 2048 or int_val > 4096:
                    return (False, f"acme-rsa-key-size must be between 2048 and 4096")
            except (ValueError, TypeError):
                return (False, f"acme-rsa-key-size must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 100:
                    return (False, f"acme-renew-window must be between 1 and 100")
            except (ValueError, TypeError):
                return (False, f"acme-renew-window must be numeric, got: {value}")
    ...
    if 'est-regeneration-method' in payload:
        ...
        if value and value not in VALID_BODY_EST_REGENERATION_METHOD:
            return (False, f"Invalid est-regeneration-method '{value}'. Must be one of: {', '.join(VALID_BODY_EST_REGENERATION_METHOD)}")
