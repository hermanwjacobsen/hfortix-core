...
def validate_interface_policy6_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'casb-profile-status' in payload:
        ...
        if value and value not in VALID_BODY_CASB_PROFILE_STATUS:
            return (False, f"Invalid casb-profile-status '{value}'. Must be one of: {', '.join(VALID_BODY_CASB_PROFILE_STATUS)}")
...
def validate_interface_policy6_delete(policyid: str | None = None) -> tuple[bool, str | None]:
    """
