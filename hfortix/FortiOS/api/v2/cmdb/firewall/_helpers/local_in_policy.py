...
def validate_local_in_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'ha-mgmt-intf-only' in payload:
        ...
        if value and value not in VALID_BODY_HA_MGMT_INTF_ONLY:
            return (False, f"Invalid ha-mgmt-intf-only '{value}'. Must be one of: {', '.join(VALID_BODY_HA_MGMT_INTF_ONLY)}")
    ...
    if 'virtual-patch' in payload:
        ...
        if value and value not in VALID_BODY_VIRTUAL_PATCH:
            return (False, f"Invalid virtual-patch '{value}'. Must be one of: {', '.join(VALID_BODY_VIRTUAL_PATCH)}")
...
def validate_local_in_policy_delete(policyid: str | None = None) -> tuple[bool, str | None]:
    """
