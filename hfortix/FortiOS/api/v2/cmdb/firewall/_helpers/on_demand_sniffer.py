...
def validate_on_demand_sniffer_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 4000:
                    return (False, f"max-packet-count must be between 1 and 4000")
            except (ValueError, TypeError):
                return (False, f"max-packet-count must be numeric, got: {value}")
    ...
    if 'non-ip-packet' in payload:
        ...
        if value and value not in VALID_BODY_NON_IP_PACKET:
            return (False, f"Invalid non-ip-packet '{value}'. Must be one of: {', '.join(VALID_BODY_NON_IP_PACKET)}")
...
def validate_on_demand_sniffer_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
