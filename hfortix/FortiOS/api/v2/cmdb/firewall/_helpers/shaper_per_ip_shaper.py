...
def validate_shaper_per_ip_shaper_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 80000000:
                    return (False, f"max-bandwidth must be between 0 and 80000000")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2097000:
                    return (False, f"max-concurrent-session must be between 0 and 2097000")
            except (ValueError, TypeError):
                return (False, f"max-concurrent-session must be numeric, got: {value}")
    ...
    if 'max-concurrent-tcp-session' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2097000:
                    return (False, f"max-concurrent-tcp-session must be between 0 and 2097000")
            except (ValueError, TypeError):
                return (False, f"max-concurrent-tcp-session must be numeric, got: {value}")
    ...
    if 'max-concurrent-udp-session' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2097000:
                    return (False, f"max-concurrent-udp-session must be between 0 and 2097000")
            except (ValueError, TypeError):
                return (False, f"max-concurrent-udp-session must be numeric, got: {value}")
...
def validate_shaper_per_ip_shaper_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
