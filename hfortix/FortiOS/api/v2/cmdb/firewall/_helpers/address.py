...
VALID_BODY_TYPE = ['ipmask', 'iprange', 'fqdn', 'geography', 'wildcard', 'dynamic', 'interface-subnet', 'mac', 'route-tag']
VALID_BODY_SUB_TYPE = ['sdn', 'clearpass-spt', 'fsso', 'rsso', 'ems-tag', 'fortivoice-tag', 'fortinac-tag', 'swc-tag', 'device-identification', 'external-resource', 'obsolete']
VALID_BODY_CLEARPASS_SPT = ['unknown', 'healthy', 'quarantine', 'checkup', 'transient', 'infected']
...
def validate_address_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'sub-type' in payload:
        ...
        if value and value not in VALID_BODY_SUB_TYPE:
            return (False, f"Invalid sub-type '{value}'. Must be one of: {', '.join(VALID_BODY_SUB_TYPE)}")
    ...
    if 'clearpass-spt' in payload:
        ...
        if value and value not in VALID_BODY_CLEARPASS_SPT:
            return (False, f"Invalid clearpass-spt '{value}'. Must be one of: {', '.join(VALID_BODY_CLEARPASS_SPT)}")
    ...
    if 'obj-type' in payload:
        ...
        if value and value not in VALID_BODY_OBJ_TYPE:
            return (False, f"Invalid obj-type '{value}'. Must be one of: {', '.join(VALID_BODY_OBJ_TYPE)}")
    ...
    if 'node-ip-only' in payload:
        ...
        if value and value not in VALID_BODY_NODE_IP_ONLY:
            return (False, f"Invalid node-ip-only '{value}'. Must be one of: {', '.join(VALID_BODY_NODE_IP_ONLY)}")
    ...
    if 'allow-routing' in payload:
        ...
        if value and value not in VALID_BODY_ALLOW_ROUTING:
            return (False, f"Invalid allow-routing '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOW_ROUTING)}")
...
def validate_address_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
