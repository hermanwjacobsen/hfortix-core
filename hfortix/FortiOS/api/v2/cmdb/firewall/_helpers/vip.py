...
VALID_BODY_TYPE = ['static-nat', 'load-balance', 'server-load-balance', 'dns-translation', 'fqdn', 'access-proxy']
VALID_BODY_SERVER_TYPE = ['http', 'https', 'tcp', 'udp', 'ip']
VALID_BODY_LDB_METHOD = ['static', 'round-robin', 'weighted', 'least-session', 'least-rtt', 'first-alive', 'http-host']
...
VALID_BODY_SSL_MIN_VERSION = ['ssl-3.0', 'tls-1.0', 'tls-1.1', 'tls-1.2', 'tls-1.3']
VALID_BODY_SSL_MAX_VERSION = ['ssl-3.0', 'tls-1.0', 'tls-1.1', 'tls-1.2', 'tls-1.3']
VALID_BODY_SSL_SERVER_MIN_VERSION = ['ssl-3.0', 'tls-1.0', 'tls-1.1', 'tls-1.2', 'tls-1.3', 'client']
VALID_BODY_SSL_SERVER_MAX_VERSION = ['ssl-3.0', 'tls-1.0', 'tls-1.1', 'tls-1.2', 'tls-1.3', 'client']
...
def validate_vip_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 604800:
                    return (False, f"dns-mapping-ttl must be between 0 and 604800")
            except (ValueError, TypeError):
                return (False, f"dns-mapping-ttl must be numeric, got: {value}")
    ...
    if 'ldb-method' in payload:
        ...
        if value and value not in VALID_BODY_LDB_METHOD:
            return (False, f"Invalid ldb-method '{value}'. Must be one of: {', '.join(VALID_BODY_LDB_METHOD)}")
    ...
    if 'src-vip-filter' in payload:
        ...
        if value and value not in VALID_BODY_SRC_VIP_FILTER:
            return (False, f"Invalid src-vip-filter '{value}'. Must be one of: {', '.join(VALID_BODY_SRC_VIP_FILTER)}")
    ...
    if 'h2-support' in payload:
        ...
        if value and value not in VALID_BODY_H2_SUPPORT:
            return (False, f"Invalid h2-support '{value}'. Must be one of: {', '.join(VALID_BODY_H2_SUPPORT)}")
    ...
    if 'h3-support' in payload:
        ...
        if value and value not in VALID_BODY_H3_SUPPORT:
            return (False, f"Invalid h3-support '{value}'. Must be one of: {', '.join(VALID_BODY_H3_SUPPORT)}")
    
    # Validate nat44 if present
    if 'nat44' in payload:
        value = payload.get('nat44')
        if value and value not in VALID_BODY_NAT44:
            return (False, f"Invalid nat44 '{value}'. Must be one of: {', '.join(VALID_BODY_NAT44)}")
    ...
    if 'add-nat46-route' in payload:
        ...
        if value and value not in VALID_BODY_ADD_NAT46_ROUTE:
            return (False, f"Invalid add-nat46-route '{value}'. Must be one of: {', '.join(VALID_BODY_ADD_NAT46_ROUTE)}")
    ...
    if 'arp-reply' in payload:
        ...
        if value and value not in VALID_BODY_ARP_REPLY:
            return (False, f"Invalid arp-reply '{value}'. Must be one of: {', '.join(VALID_BODY_ARP_REPLY)}")
    ...
    if 'http-redirect' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_REDIRECT:
            return (False, f"Invalid http-redirect '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_REDIRECT)}")
    ...
    if 'persistence' in payload:
        ...
        if value and value not in VALID_BODY_PERSISTENCE:
            return (False, f"Invalid persistence '{value}'. Must be one of: {', '.join(VALID_BODY_PERSISTENCE)}")
    ...
    if 'nat-source-vip' in payload:
        ...
        if value and value not in VALID_BODY_NAT_SOURCE_VIP:
            return (False, f"Invalid nat-source-vip '{value}'. Must be one of: {', '.join(VALID_BODY_NAT_SOURCE_VIP)}")
    ...
    if 'portforward' in payload:
        ...
        if value and value not in VALID_BODY_PORTFORWARD:
            return (False, f"Invalid portforward '{value}'. Must be one of: {', '.join(VALID_BODY_PORTFORWARD)}")
    ...
    if 'protocol' in payload:
        ...
        if value and value not in VALID_BODY_PROTOCOL:
            return (False, f"Invalid protocol '{value}'. Must be one of: {', '.join(VALID_BODY_PROTOCOL)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 5 or int_val > 8640000:
                    return (False, f"gratuitous-arp-interval must be between 5 and 8640000")
            except (ValueError, TypeError):
                return (False, f"gratuitous-arp-interval must be numeric, got: {value}")
    ...
    if 'portmapping-type' in payload:
        ...
        if value and value not in VALID_BODY_PORTMAPPING_TYPE:
            return (False, f"Invalid portmapping-type '{value}'. Must be one of: {', '.join(VALID_BODY_PORTMAPPING_TYPE)}")
    ...
    if 'empty-cert-action' in payload:
        ...
        if value and value not in VALID_BODY_EMPTY_CERT_ACTION:
            return (False, f"Invalid empty-cert-action '{value}'. Must be one of: {', '.join(VALID_BODY_EMPTY_CERT_ACTION)}")
    ...
    if 'user-agent-detect' in payload:
        ...
        if value and value not in VALID_BODY_USER_AGENT_DETECT:
            return (False, f"Invalid user-agent-detect '{value}'. Must be one of: {', '.join(VALID_BODY_USER_AGENT_DETECT)}")
    ...
    if 'client-cert' in payload:
        ...
        if value and value not in VALID_BODY_CLIENT_CERT:
            return (False, f"Invalid client-cert '{value}'. Must be one of: {', '.join(VALID_BODY_CLIENT_CERT)}")
    ...
    if 'http-cookie-domain-from-host' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_COOKIE_DOMAIN_FROM_HOST:
            return (False, f"Invalid http-cookie-domain-from-host '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_COOKIE_DOMAIN_FROM_HOST)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"http-cookie-generation must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"http-cookie-generation must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 525600:
                    return (False, f"http-cookie-age must be between 0 and 525600")
            except (ValueError, TypeError):
                return (False, f"http-cookie-age must be numeric, got: {value}")
    ...
    if 'http-cookie-share' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_COOKIE_SHARE:
            return (False, f"Invalid http-cookie-share '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_COOKIE_SHARE)}")
    ...
    if 'https-cookie-secure' in payload:
        ...
        if value and value not in VALID_BODY_HTTPS_COOKIE_SECURE:
            return (False, f"Invalid https-cookie-secure '{value}'. Must be one of: {', '.join(VALID_BODY_HTTPS_COOKIE_SECURE)}")
    ...
    if 'http-multiplex' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_MULTIPLEX:
            return (False, f"Invalid http-multiplex '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_MULTIPLEX)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2147483647:
                    return (False, f"http-multiplex-ttl must be between 0 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"http-multiplex-ttl must be numeric, got: {value}")
    ...
    if 'http-multiplex-max-request' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2147483647:
                    return (False, f"http-multiplex-max-request must be between 0 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"http-multiplex-max-request must be numeric, got: {value}")
    ...
    if 'http-multiplex-max-concurrent-request' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2147483647:
                    return (False, f"http-multiplex-max-concurrent-request must be between 0 and 2147483647")
            except (ValueError, TypeError):
                return (False, f"http-multiplex-max-concurrent-request must be numeric, got: {value}")
    ...
    if 'http-ip-header' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_IP_HEADER:
            return (False, f"Invalid http-ip-header '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_IP_HEADER)}")
    ...
    if 'outlook-web-access' in payload:
        ...
        if value and value not in VALID_BODY_OUTLOOK_WEB_ACCESS:
            return (False, f"Invalid outlook-web-access '{value}'. Must be one of: {', '.join(VALID_BODY_OUTLOOK_WEB_ACCESS)}")
    ...
    if 'weblogic-server' in payload:
        ...
        if value and value not in VALID_BODY_WEBLOGIC_SERVER:
            return (False, f"Invalid weblogic-server '{value}'. Must be one of: {', '.join(VALID_BODY_WEBLOGIC_SERVER)}")
    ...
    if 'websphere-server' in payload:
        ...
        if value and value not in VALID_BODY_WEBSPHERE_SERVER:
            return (False, f"Invalid websphere-server '{value}'. Must be one of: {', '.join(VALID_BODY_WEBSPHERE_SERVER)}")
    ...
    if 'ssl-mode' in payload:
        ...
        if value and value not in VALID_BODY_SSL_MODE:
            return (False, f"Invalid ssl-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_MODE)}")
    ...
    if 'ssl-dh-bits' in payload:
        ...
        if value and value not in VALID_BODY_SSL_DH_BITS:
            return (False, f"Invalid ssl-dh-bits '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_DH_BITS)}")
    ...
    if 'ssl-algorithm' in payload:
        ...
        if value and value not in VALID_BODY_SSL_ALGORITHM:
            return (False, f"Invalid ssl-algorithm '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_ALGORITHM)}")
    ...
    if 'ssl-server-algorithm' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_ALGORITHM:
            return (False, f"Invalid ssl-server-algorithm '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_ALGORITHM)}")
    
    # Validate ssl-pfs if present
    if 'ssl-pfs' in payload:
        value = payload.get('ssl-pfs')
        if value and value not in VALID_BODY_SSL_PFS:
            return (False, f"Invalid ssl-pfs '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_PFS)}")
    ...
    if 'ssl-min-version' in payload:
        ...
        if value and value not in VALID_BODY_SSL_MIN_VERSION:
            return (False, f"Invalid ssl-min-version '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_MIN_VERSION)}")
    ...
    if 'ssl-max-version' in payload:
        ...
        if value and value not in VALID_BODY_SSL_MAX_VERSION:
            return (False, f"Invalid ssl-max-version '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_MAX_VERSION)}")
    ...
    if 'ssl-server-min-version' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_MIN_VERSION:
            return (False, f"Invalid ssl-server-min-version '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_MIN_VERSION)}")
    ...
    if 'ssl-server-max-version' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_MAX_VERSION:
            return (False, f"Invalid ssl-server-max-version '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_MAX_VERSION)}")
    ...
    if 'ssl-accept-ffdhe-groups' in payload:
        ...
        if value and value not in VALID_BODY_SSL_ACCEPT_FFDHE_GROUPS:
            return (False, f"Invalid ssl-accept-ffdhe-groups '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_ACCEPT_FFDHE_GROUPS)}")
    ...
    if 'ssl-send-empty-frags' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SEND_EMPTY_FRAGS:
            return (False, f"Invalid ssl-send-empty-frags '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SEND_EMPTY_FRAGS)}")
    ...
    if 'ssl-client-fallback' in payload:
        ...
        if value and value not in VALID_BODY_SSL_CLIENT_FALLBACK:
            return (False, f"Invalid ssl-client-fallback '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_CLIENT_FALLBACK)}")
    ...
    if 'ssl-client-renegotiation' in payload:
        ...
        if value and value not in VALID_BODY_SSL_CLIENT_RENEGOTIATION:
            return (False, f"Invalid ssl-client-renegotiation '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_CLIENT_RENEGOTIATION)}")
    ...
    if 'ssl-client-session-state-type' in payload:
        ...
        if value and value not in VALID_BODY_SSL_CLIENT_SESSION_STATE_TYPE:
            return (False, f"Invalid ssl-client-session-state-type '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_CLIENT_SESSION_STATE_TYPE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 14400:
                    return (False, f"ssl-client-session-state-timeout must be between 1 and 14400")
            except (ValueError, TypeError):
                return (False, f"ssl-client-session-state-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 10000:
                    return (False, f"ssl-client-session-state-max must be between 1 and 10000")
            except (ValueError, TypeError):
                return (False, f"ssl-client-session-state-max must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 200 or int_val > 1048576:
                    return (False, f"ssl-client-rekey-count must be between 200 and 1048576")
            except (ValueError, TypeError):
                return (False, f"ssl-client-rekey-count must be numeric, got: {value}")
    ...
    if 'ssl-server-renegotiation' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_RENEGOTIATION:
            return (False, f"Invalid ssl-server-renegotiation '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_RENEGOTIATION)}")
    ...
    if 'ssl-server-session-state-type' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_SESSION_STATE_TYPE:
            return (False, f"Invalid ssl-server-session-state-type '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_SESSION_STATE_TYPE)}")
    ...
    if 'ssl-server-session-state-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 14400:
                    return (False, f"ssl-server-session-state-timeout must be between 1 and 14400")
            except (ValueError, TypeError):
                return (False, f"ssl-server-session-state-timeout must be numeric, got: {value}")
    ...
    if 'ssl-server-session-state-max' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 10000:
                    return (False, f"ssl-server-session-state-max must be between 1 and 10000")
            except (ValueError, TypeError):
                return (False, f"ssl-server-session-state-max must be numeric, got: {value}")
    ...
    if 'ssl-http-location-conversion' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HTTP_LOCATION_CONVERSION:
            return (False, f"Invalid ssl-http-location-conversion '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HTTP_LOCATION_CONVERSION)}")
    ...
    if 'ssl-http-match-host' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HTTP_MATCH_HOST:
            return (False, f"Invalid ssl-http-match-host '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HTTP_MATCH_HOST)}")
    ...
    if 'ssl-hpkp' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HPKP:
            return (False, f"Invalid ssl-hpkp '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HPKP)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 60 or int_val > 157680000:
                    return (False, f"ssl-hpkp-age must be between 60 and 157680000")
            except (ValueError, TypeError):
    ...
    if 'ssl-hpkp-include-subdomains' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HPKP_INCLUDE_SUBDOMAINS:
            return (False, f"Invalid ssl-hpkp-include-subdomains '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HPKP_INCLUDE_SUBDOMAINS)}")
    ...
    if 'ssl-hsts' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HSTS:
            return (False, f"Invalid ssl-hsts '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HSTS)}")
    ...
    if 'ssl-hsts-age' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 60 or int_val > 157680000:
                    return (False, f"ssl-hsts-age must be between 60 and 157680000")
            except (ValueError, TypeError):
    ...
    if 'ssl-hsts-include-subdomains' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HSTS_INCLUDE_SUBDOMAINS:
            return (False, f"Invalid ssl-hsts-include-subdomains '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HSTS_INCLUDE_SUBDOMAINS)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100000:
                    return (False, f"max-embryonic-connections must be between 0 and 100000")
            except (ValueError, TypeError):
                return (False, f"max-embryonic-connections must be numeric, got: {value}")
    ...
    if 'one-click-gslb-server' in payload:
        ...
        if value and value not in VALID_BODY_ONE_CLICK_GSLB_SERVER:
            return (False, f"Invalid one-click-gslb-server '{value}'. Must be one of: {', '.join(VALID_BODY_ONE_CLICK_GSLB_SERVER)}")
