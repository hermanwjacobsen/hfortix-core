...
VALID_BODY_TYPE = ['RSA', 'DSA', 'ECDSA', 'ED25519', 'RSA-CA', 'DSA-CA', 'ECDSA-CA', 'ED25519-CA']
...
def validate_ssh_host_key_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'nid' in payload:
        ...
        if value and value not in VALID_BODY_NID:
            return (False, f"Invalid nid '{value}'. Must be one of: {', '.join(VALID_BODY_NID)}")
    
    # Validate usage if present
    if 'usage' in payload:
        value = payload.get('usage')
        if value and value not in VALID_BODY_USAGE:
            return (False, f"Invalid usage '{value}'. Must be one of: {', '.join(VALID_BODY_USAGE)}")
...
def validate_ssh_host_key_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
