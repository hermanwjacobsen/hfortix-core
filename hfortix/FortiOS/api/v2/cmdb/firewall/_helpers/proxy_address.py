...
VALID_BODY_TYPE = ['host-regex', 'url', 'category', 'method', 'ua', 'header', 'src-advanced', 'dst-advanced', 'saas']
VALID_BODY_REFERRER = ['enable', 'disable']
VALID_BODY_METHOD = ['get', 'post', 'put', 'head', 'connect', 'trace', 'options', 'delete', 'update', 'patch', 'other']
...
def validate_proxy_address_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'referrer' in payload:
        ...
        if value and value not in VALID_BODY_REFERRER:
            return (False, f"Invalid referrer '{value}'. Must be one of: {', '.join(VALID_BODY_REFERRER)}")
    ...
    if 'ua' in payload:
        ...
        if value and value not in VALID_BODY_UA:
            return (False, f"Invalid ua '{value}'. Must be one of: {', '.join(VALID_BODY_UA)}")
    ...
    if 'case-sensitivity' in payload:
        ...
        if value and value not in VALID_BODY_CASE_SENSITIVITY:
            return (False, f"Invalid case-sensitivity '{value}'. Must be one of: {', '.join(VALID_BODY_CASE_SENSITIVITY)}")
...
def validate_proxy_address_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
