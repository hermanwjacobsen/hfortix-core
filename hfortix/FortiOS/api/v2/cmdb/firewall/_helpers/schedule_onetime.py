...
def validate_schedule_onetime_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 100:
                    return (False, f"expiration-days must be between 0 and 100")
            except (ValueError, TypeError):
                return (False, f"expiration-days must be numeric, got: {value}")
...
def validate_schedule_onetime_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
