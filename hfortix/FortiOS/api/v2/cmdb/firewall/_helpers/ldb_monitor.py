...
def validate_ldb_monitor_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 5:
                    return (False, f"http-max-redirects must be between 0 and 5")
            except (ValueError, TypeError):
                return (False, f"http-max-redirects must be numeric, got: {value}")
    ...
    if 'dns-protocol' in payload:
        ...
        if value and value not in VALID_BODY_DNS_PROTOCOL:
            return (False, f"Invalid dns-protocol '{value}'. Must be one of: {', '.join(VALID_BODY_DNS_PROTOCOL)}")
...
def validate_ldb_monitor_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
