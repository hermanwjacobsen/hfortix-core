...
def validate_security_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'enforce-default-app-port' in payload:
        ...
        if value and value not in VALID_BODY_ENFORCE_DEFAULT_APP_PORT:
            return (False, f"Invalid enforce-default-app-port '{value}'. Must be one of: {', '.join(VALID_BODY_ENFORCE_DEFAULT_APP_PORT)}")
    ...
    if 'learning-mode' in payload:
        ...
        if value and value not in VALID_BODY_LEARNING_MODE:
            return (False, f"Invalid learning-mode '{value}'. Must be one of: {', '.join(VALID_BODY_LEARNING_MODE)}")
...
def validate_security_policy_delete(policyid: str | None = None) -> tuple[bool, str | None]:
    """
