...
def validate_shaper_traffic_shaper_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 80000000:
                    return (False, f"guaranteed-bandwidth must be between 0 and 80000000")
            except (ValueError, TypeError):
                return (False, f"guaranteed-bandwidth must be numeric, got: {value}")
    ...
    if 'maximum-bandwidth' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 80000000:
                    return (False, f"maximum-bandwidth must be between 0 and 80000000")
            except (ValueError, TypeError):
                return (False, f"maximum-bandwidth must be numeric, got: {value}")
    ...
    if 'bandwidth-unit' in payload:
        ...
        if value and value not in VALID_BODY_BANDWIDTH_UNIT:
            return (False, f"Invalid bandwidth-unit '{value}'. Must be one of: {', '.join(VALID_BODY_BANDWIDTH_UNIT)}")
    ...
    if 'per-policy' in payload:
        ...
        if value and value not in VALID_BODY_PER_POLICY:
            return (False, f"Invalid per-policy '{value}'. Must be one of: {', '.join(VALID_BODY_PER_POLICY)}")
    ...
    if 'dscp-marking-method' in payload:
        ...
        if value and value not in VALID_BODY_DSCP_MARKING_METHOD:
            return (False, f"Invalid dscp-marking-method '{value}'. Must be one of: {', '.join(VALID_BODY_DSCP_MARKING_METHOD)}")
    ...
    if 'exceed-bandwidth' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 80000000:
                    return (False, f"exceed-bandwidth must be between 0 and 80000000")
            except (ValueError, TypeError):
                return (False, f"exceed-bandwidth must be numeric, got: {value}")
    ...
    if 'cos-marking' in payload:
        ...
        if value and value not in VALID_BODY_COS_MARKING:
            return (False, f"Invalid cos-marking '{value}'. Must be one of: {', '.join(VALID_BODY_COS_MARKING)}")
    ...
    if 'cos-marking-method' in payload:
        ...
        if value and value not in VALID_BODY_COS_MARKING_METHOD:
            return (False, f"Invalid cos-marking-method '{value}'. Must be one of: {', '.join(VALID_BODY_COS_MARKING_METHOD)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"exceed-class-id must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"exceed-class-id must be numeric, got: {value}")
...
def validate_shaper_traffic_shaper_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
