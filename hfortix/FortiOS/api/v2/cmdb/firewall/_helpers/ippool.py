...
VALID_BODY_TYPE = ['overload', 'one-to-one', 'fixed-port-range', 'port-block-allocation']
...
def validate_ippool_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
        ...
        if value is not None:
            try:
                ...
                if int_val < 32 or int_val > 60417:
                    return (False, f"port-per-user must be between 32 and 60417")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 128:
                    return (False, f"num-blocks-per-user must be between 1 and 128")
            except (ValueError, TypeError):
                return (False, f"num-blocks-per-user must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 600 or int_val > 86400:
                    return (False, f"pba-interim-log must be between 600 and 86400")
            except (ValueError, TypeError):
                return (False, f"pba-interim-log must be numeric, got: {value}")
    ...
    if 'client-prefix-length' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 128:
                    return (False, f"client-prefix-length must be between 1 and 128")
            except (ValueError, TypeError):
                return (False, f"client-prefix-length must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 2097000:
                    return (False, f"tcp-session-quota must be between 0 and 2097000")
            except (ValueError, TypeError):
                return (False, f"tcp-session-quota must be numeric, got: {value}")
    ...
    if 'udp-session-quota' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2097000:
                    return (False, f"udp-session-quota must be between 0 and 2097000")
            except (ValueError, TypeError):
                return (False, f"udp-session-quota must be numeric, got: {value}")
    ...
    if 'icmp-session-quota' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 2097000:
                    return (False, f"icmp-session-quota must be between 0 and 2097000")
            except (ValueError, TypeError):
                return (False, f"icmp-session-quota must be numeric, got: {value}")
    ...
    if 'privileged-port-use-pba' in payload:
        ...
        if value and value not in VALID_BODY_PRIVILEGED_PORT_USE_PBA:
            return (False, f"Invalid privileged-port-use-pba '{value}'. Must be one of: {', '.join(VALID_BODY_PRIVILEGED_PORT_USE_PBA)}")
    ...
    if 'subnet-broadcast-in-ippool' in payload:
        ...
        if value and value not in VALID_BODY_SUBNET_BROADCAST_IN_IPPOOL:
            return (False, f"Invalid subnet-broadcast-in-ippool '{value}'. Must be one of: {', '.join(VALID_BODY_SUBNET_BROADCAST_IN_IPPOOL)}")
