...
def validate_access_proxy_ssh_client_cert_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'source-address' in payload:
        ...
        if value and value not in VALID_BODY_SOURCE_ADDRESS:
            return (False, f"Invalid source-address '{value}'. Must be one of: {', '.join(VALID_BODY_SOURCE_ADDRESS)}")
    ...
    if 'permit-x11-forwarding' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_X11_FORWARDING:
            return (False, f"Invalid permit-x11-forwarding '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_X11_FORWARDING)}")
    ...
    if 'permit-agent-forwarding' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_AGENT_FORWARDING:
            return (False, f"Invalid permit-agent-forwarding '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_AGENT_FORWARDING)}")
    ...
    if 'permit-port-forwarding' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_PORT_FORWARDING:
            return (False, f"Invalid permit-port-forwarding '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_PORT_FORWARDING)}")
    ...
    if 'permit-pty' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_PTY:
            return (False, f"Invalid permit-pty '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_PTY)}")
    ...
    if 'permit-user-rc' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_USER_RC:
            return (False, f"Invalid permit-user-rc '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_USER_RC)}")
...
def validate_access_proxy_ssh_client_cert_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
