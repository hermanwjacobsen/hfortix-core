...
def validate_vendor_mac_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'mac-number' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"mac-number must be between 0 and 4294967295")
            except (ValueError, TypeError):
...
def validate_vendor_mac_delete(id: str | None = None) -> tuple[bool, str | None]:
    """
