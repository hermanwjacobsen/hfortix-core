...
def validate_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967294:
                    return (False, f"policyid must be between 0 and 4294967294")
    ...
    if 'nat64' in payload:
        ...
        if value and value not in VALID_BODY_NAT64:
            return (False, f"Invalid nat64 '{value}'. Must be one of: {', '.join(VALID_BODY_NAT64)}")
    
    # Validate nat46 if present
    if 'nat46' in payload:
        value = payload.get('nat46')
        if value and value not in VALID_BODY_NAT46:
            return (False, f"Invalid nat46 '{value}'. Must be one of: {', '.join(VALID_BODY_NAT46)}")
    ...
    if 'ztna-status' in payload:
        ...
        if value and value not in VALID_BODY_ZTNA_STATUS:
            return (False, f"Invalid ztna-status '{value}'. Must be one of: {', '.join(VALID_BODY_ZTNA_STATUS)}")
    ...
    if 'ztna-device-ownership' in payload:
        ...
        if value and value not in VALID_BODY_ZTNA_DEVICE_OWNERSHIP:
            return (False, f"Invalid ztna-device-ownership '{value}'. Must be one of: {', '.join(VALID_BODY_ZTNA_DEVICE_OWNERSHIP)}")
    ...
    if 'ztna-tags-match-logic' in payload:
        ...
        if value and value not in VALID_BODY_ZTNA_TAGS_MATCH_LOGIC:
            return (False, f"Invalid ztna-tags-match-logic '{value}'. Must be one of: {', '.join(VALID_BODY_ZTNA_TAGS_MATCH_LOGIC)}")
    ...
    if 'internet-service' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE:
            return (False, f"Invalid internet-service '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE)}")
    ...
    if 'internet-service-src' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE_SRC:
            return (False, f"Invalid internet-service-src '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE_SRC)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"reputation-minimum must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"reputation-minimum must be numeric, got: {value}")
    ...
    if 'reputation-direction' in payload:
        ...
        if value and value not in VALID_BODY_REPUTATION_DIRECTION:
            return (False, f"Invalid reputation-direction '{value}'. Must be one of: {', '.join(VALID_BODY_REPUTATION_DIRECTION)}")
    ...
    if 'internet-service6' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE6:
            return (False, f"Invalid internet-service6 '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE6)}")
    ...
    if 'internet-service6-src' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE6_SRC:
            return (False, f"Invalid internet-service6-src '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE6_SRC)}")
    ...
    if 'reputation-minimum6' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"reputation-minimum6 must be between 0 and 4294967295")
            except (ValueError, TypeError):
                return (False, f"reputation-minimum6 must be numeric, got: {value}")
    ...
    if 'reputation-direction6' in payload:
        ...
        if value and value not in VALID_BODY_REPUTATION_DIRECTION6:
            return (False, f"Invalid reputation-direction6 '{value}'. Must be one of: {', '.join(VALID_BODY_REPUTATION_DIRECTION6)}")
    
    # Validate rtp-nat if present
    if 'rtp-nat' in payload:
        value = payload.get('rtp-nat')
        if value and value not in VALID_BODY_RTP_NAT:
            return (False, f"Invalid rtp-nat '{value}'. Must be one of: {', '.join(VALID_BODY_RTP_NAT)}")
    ...
    if 'send-deny-packet' in payload:
        ...
        if value and value not in VALID_BODY_SEND_DENY_PACKET:
            return (False, f"Invalid send-deny-packet '{value}'. Must be one of: {', '.join(VALID_BODY_SEND_DENY_PACKET)}")
    ...
    if 'schedule-timeout' in payload:
        ...
        if value and value not in VALID_BODY_SCHEDULE_TIMEOUT:
            return (False, f"Invalid schedule-timeout '{value}'. Must be one of: {', '.join(VALID_BODY_SCHEDULE_TIMEOUT)}")
    ...
    if 'policy-expiry' in payload:
        ...
        if value and value not in VALID_BODY_POLICY_EXPIRY:
            return (False, f"Invalid policy-expiry '{value}'. Must be one of: {', '.join(VALID_BODY_POLICY_EXPIRY)}")
    ...
    if 'tos-negate' in payload:
        ...
        if value and value not in VALID_BODY_TOS_NEGATE:
            return (False, f"Invalid tos-negate '{value}'. Must be one of: {', '.join(VALID_BODY_TOS_NEGATE)}")
    ...
    if 'geoip-anycast' in payload:
        ...
        if value and value not in VALID_BODY_GEOIP_ANYCAST:
            return (False, f"Invalid geoip-anycast '{value}'. Must be one of: {', '.join(VALID_BODY_GEOIP_ANYCAST)}")
    ...
    if 'geoip-match' in payload:
        ...
        if value and value not in VALID_BODY_GEOIP_MATCH:
            return (False, f"Invalid geoip-match '{value}'. Must be one of: {', '.join(VALID_BODY_GEOIP_MATCH)}")
    ...
    if 'dynamic-shaping' in payload:
        ...
        if value and value not in VALID_BODY_DYNAMIC_SHAPING:
            return (False, f"Invalid dynamic-shaping '{value}'. Must be one of: {', '.join(VALID_BODY_DYNAMIC_SHAPING)}")
    ...
    if 'passive-wan-health-measurement' in payload:
        ...
        if value and value not in VALID_BODY_PASSIVE_WAN_HEALTH_MEASUREMENT:
            return (False, f"Invalid passive-wan-health-measurement '{value}'. Must be one of: {', '.join(VALID_BODY_PASSIVE_WAN_HEALTH_MEASUREMENT)}")
    ...
    if 'app-monitor' in payload:
        ...
        if value and value not in VALID_BODY_APP_MONITOR:
            return (False, f"Invalid app-monitor '{value}'. Must be one of: {', '.join(VALID_BODY_APP_MONITOR)}")
    ...
    if 'inspection-mode' in payload:
        ...
        if value and value not in VALID_BODY_INSPECTION_MODE:
            return (False, f"Invalid inspection-mode '{value}'. Must be one of: {', '.join(VALID_BODY_INSPECTION_MODE)}")
    ...
    if 'http-policy-redirect' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_POLICY_REDIRECT:
            return (False, f"Invalid http-policy-redirect '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_POLICY_REDIRECT)}")
    ...
    if 'ssh-policy-redirect' in payload:
        ...
        if value and value not in VALID_BODY_SSH_POLICY_REDIRECT:
            return (False, f"Invalid ssh-policy-redirect '{value}'. Must be one of: {', '.join(VALID_BODY_SSH_POLICY_REDIRECT)}")
    ...
    if 'ztna-policy-redirect' in payload:
        ...
        if value and value not in VALID_BODY_ZTNA_POLICY_REDIRECT:
            return (False, f"Invalid ztna-policy-redirect '{value}'. Must be one of: {', '.join(VALID_BODY_ZTNA_POLICY_REDIRECT)}")
    ...
    if 'profile-type' in payload:
        ...
        if value and value not in VALID_BODY_PROFILE_TYPE:
            return (False, f"Invalid profile-type '{value}'. Must be one of: {', '.join(VALID_BODY_PROFILE_TYPE)}")
    ...
    if 'profile-protocol-options' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 47:
            return (False, f"profile-protocol-options cannot exceed 47 characters")
    ...
    if 'diameter-filter-profile' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 47:
            return (False, f"diameter-filter-profile cannot exceed 47 characters")
    
    # Validate virtual-patch-profile if present
    if 'virtual-patch-profile' in payload:
        value = payload.get('virtual-patch-profile')
        if value and isinstance(value, str) and len(value) > 47:
            return (False, f"virtual-patch-profile cannot exceed 47 characters")
    ...
    if 'logtraffic-start' in payload:
        ...
        if value and value not in VALID_BODY_LOGTRAFFIC_START:
            return (False, f"Invalid logtraffic-start '{value}'. Must be one of: {', '.join(VALID_BODY_LOGTRAFFIC_START)}")
    ...
    if 'log-http-transaction' in payload:
        ...
        if value and value not in VALID_BODY_LOG_HTTP_TRANSACTION:
            return (False, f"Invalid log-http-transaction '{value}'. Must be one of: {', '.join(VALID_BODY_LOG_HTTP_TRANSACTION)}")
    ...
    if 'capture-packet' in payload:
        ...
        if value and value not in VALID_BODY_CAPTURE_PACKET:
            return (False, f"Invalid capture-packet '{value}'. Must be one of: {', '.join(VALID_BODY_CAPTURE_PACKET)}")
    ...
    if 'np-acceleration' in payload:
        ...
        if value and value not in VALID_BODY_NP_ACCELERATION:
            return (False, f"Invalid np-acceleration '{value}'. Must be one of: {', '.join(VALID_BODY_NP_ACCELERATION)}")
    ...
    if 'webproxy-forward-server' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 63:
            return (False, f"webproxy-forward-server cannot exceed 63 characters")
    ...
    if 'traffic-shaper-reverse' in payload:
        ...
        if value and isinstance(value, str) and len(value) > 35:
            return (False, f"traffic-shaper-reverse cannot exceed 35 characters")
    ...
    if 'nat' in payload:
        ...
        if value and value not in VALID_BODY_NAT:
            return (False, f"Invalid nat '{value}'. Must be one of: {', '.join(VALID_BODY_NAT)}")
    ...
    if 'pcp-outbound' in payload:
        ...
        if value and value not in VALID_BODY_PCP_OUTBOUND:
            return (False, f"Invalid pcp-outbound '{value}'. Must be one of: {', '.join(VALID_BODY_PCP_OUTBOUND)}")
    ...
    if 'pcp-inbound' in payload:
        ...
        if value and value not in VALID_BODY_PCP_INBOUND:
            return (False, f"Invalid pcp-inbound '{value}'. Must be one of: {', '.join(VALID_BODY_PCP_INBOUND)}")
    ...
    if 'permit-any-host' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_ANY_HOST:
            return (False, f"Invalid permit-any-host '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_ANY_HOST)}")
    ...
    if 'permit-stun-host' in payload:
        ...
        if value and value not in VALID_BODY_PERMIT_STUN_HOST:
            return (False, f"Invalid permit-stun-host '{value}'. Must be one of: {', '.join(VALID_BODY_PERMIT_STUN_HOST)}")
    ...
    if 'fixedport' in payload:
        ...
        if value and value not in VALID_BODY_FIXEDPORT:
            return (False, f"Invalid fixedport '{value}'. Must be one of: {', '.join(VALID_BODY_FIXEDPORT)}")
    ...
    if 'port-preserve' in payload:
        ...
        if value and value not in VALID_BODY_PORT_PRESERVE:
            return (False, f"Invalid port-preserve '{value}'. Must be one of: {', '.join(VALID_BODY_PORT_PRESERVE)}")
    ...
    if 'port-random' in payload:
        ...
        if value and value not in VALID_BODY_PORT_RANDOM:
            return (False, f"Invalid port-random '{value}'. Must be one of: {', '.join(VALID_BODY_PORT_RANDOM)}")
    
    # Validate ippool if present
    if 'ippool' in payload:
        value = payload.get('ippool')
        if value and value not in VALID_BODY_IPPOOL:
            return (False, f"Invalid ippool '{value}'. Must be one of: {', '.join(VALID_BODY_IPPOOL)}")
    ...
    if 'inbound' in payload:
        ...
        if value and value not in VALID_BODY_INBOUND:
            return (False, f"Invalid inbound '{value}'. Must be one of: {', '.join(VALID_BODY_INBOUND)}")
    ...
    if 'outbound' in payload:
        ...
        if value and value not in VALID_BODY_OUTBOUND:
            return (False, f"Invalid outbound '{value}'. Must be one of: {', '.join(VALID_BODY_OUTBOUND)}")
    ...
    if 'natinbound' in payload:
        ...
        if value and value not in VALID_BODY_NATINBOUND:
            return (False, f"Invalid natinbound '{value}'. Must be one of: {', '.join(VALID_BODY_NATINBOUND)}")
    ...
    if 'natoutbound' in payload:
        ...
        if value and value not in VALID_BODY_NATOUTBOUND:
            return (False, f"Invalid natoutbound '{value}'. Must be one of: {', '.join(VALID_BODY_NATOUTBOUND)}")
    
    # Validate fec if present
    if 'fec' in payload:
        value = payload.get('fec')
        if value and value not in VALID_BODY_FEC:
            return (False, f"Invalid fec '{value}'. Must be one of: {', '.join(VALID_BODY_FEC)}")
    ...
    if 'ntlm' in payload:
        ...
        if value and value not in VALID_BODY_NTLM:
            return (False, f"Invalid ntlm '{value}'. Must be one of: {', '.join(VALID_BODY_NTLM)}")
    ...
    if 'ntlm-guest' in payload:
        ...
        if value and value not in VALID_BODY_NTLM_GUEST:
            return (False, f"Invalid ntlm-guest '{value}'. Must be one of: {', '.join(VALID_BODY_NTLM_GUEST)}")
    ...
    if 'auth-path' in payload:
        ...
        if value and value not in VALID_BODY_AUTH_PATH:
            return (False, f"Invalid auth-path '{value}'. Must be one of: {', '.join(VALID_BODY_AUTH_PATH)}")
    ...
    if 'disclaimer' in payload:
        ...
        if value and value not in VALID_BODY_DISCLAIMER:
            return (False, f"Invalid disclaimer '{value}'. Must be one of: {', '.join(VALID_BODY_DISCLAIMER)}")
    ...
    if 'email-collect' in payload:
        ...
        if value and value not in VALID_BODY_EMAIL_COLLECT:
            return (False, f"Invalid email-collect '{value}'. Must be one of: {', '.join(VALID_BODY_EMAIL_COLLECT)}")
    ...
    if 'match-vip' in payload:
        ...
        if value and value not in VALID_BODY_MATCH_VIP:
            return (False, f"Invalid match-vip '{value}'. Must be one of: {', '.join(VALID_BODY_MATCH_VIP)}")
    ...
    if 'match-vip-only' in payload:
        ...
        if value and value not in VALID_BODY_MATCH_VIP_ONLY:
            return (False, f"Invalid match-vip-only '{value}'. Must be one of: {', '.join(VALID_BODY_MATCH_VIP_ONLY)}")
    ...
    if 'diffserv-copy' in payload:
        ...
        if value and value not in VALID_BODY_DIFFSERV_COPY:
            return (False, f"Invalid diffserv-copy '{value}'. Must be one of: {', '.join(VALID_BODY_DIFFSERV_COPY)}")
    ...
    if 'diffserv-forward' in payload:
        ...
        if value and value not in VALID_BODY_DIFFSERV_FORWARD:
            return (False, f"Invalid diffserv-forward '{value}'. Must be one of: {', '.join(VALID_BODY_DIFFSERV_FORWARD)}")
    ...
    if 'diffserv-reverse' in payload:
        ...
        if value and value not in VALID_BODY_DIFFSERV_REVERSE:
            return (False, f"Invalid diffserv-reverse '{value}'. Must be one of: {', '.join(VALID_BODY_DIFFSERV_REVERSE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 65535:
                    return (False, f"tcp-mss-sender must be between 0 and 65535")
            except (ValueError, TypeError):
    ...
    if 'tcp-mss-receiver' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 65535:
                    return (False, f"tcp-mss-receiver must be between 0 and 65535")
            except (ValueError, TypeError):
                return (False, f"tcp-mss-receiver must be numeric, got: {value}")
    ...
    if 'block-notification' in payload:
        ...
        if value and value not in VALID_BODY_BLOCK_NOTIFICATION:
            return (False, f"Invalid block-notification '{value}'. Must be one of: {', '.join(VALID_BODY_BLOCK_NOTIFICATION)}")
    ...
    if 'srcaddr-negate' in payload:
        ...
        if value and value not in VALID_BODY_SRCADDR_NEGATE:
            return (False, f"Invalid srcaddr-negate '{value}'. Must be one of: {', '.join(VALID_BODY_SRCADDR_NEGATE)}")
    ...
    if 'srcaddr6-negate' in payload:
        ...
        if value and value not in VALID_BODY_SRCADDR6_NEGATE:
            return (False, f"Invalid srcaddr6-negate '{value}'. Must be one of: {', '.join(VALID_BODY_SRCADDR6_NEGATE)}")
    ...
    if 'dstaddr-negate' in payload:
        ...
        if value and value not in VALID_BODY_DSTADDR_NEGATE:
            return (False, f"Invalid dstaddr-negate '{value}'. Must be one of: {', '.join(VALID_BODY_DSTADDR_NEGATE)}")
    ...
    if 'dstaddr6-negate' in payload:
        ...
        if value and value not in VALID_BODY_DSTADDR6_NEGATE:
            return (False, f"Invalid dstaddr6-negate '{value}'. Must be one of: {', '.join(VALID_BODY_DSTADDR6_NEGATE)}")
    ...
    if 'ztna-ems-tag-negate' in payload:
        ...
        if value and value not in VALID_BODY_ZTNA_EMS_TAG_NEGATE:
            return (False, f"Invalid ztna-ems-tag-negate '{value}'. Must be one of: {', '.join(VALID_BODY_ZTNA_EMS_TAG_NEGATE)}")
    ...
    if 'service-negate' in payload:
        ...
        if value and value not in VALID_BODY_SERVICE_NEGATE:
            return (False, f"Invalid service-negate '{value}'. Must be one of: {', '.join(VALID_BODY_SERVICE_NEGATE)}")
    ...
    if 'internet-service-negate' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE_NEGATE:
            return (False, f"Invalid internet-service-negate '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE_NEGATE)}")
    ...
    if 'internet-service-src-negate' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE_SRC_NEGATE:
            return (False, f"Invalid internet-service-src-negate '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE_SRC_NEGATE)}")
    ...
    if 'internet-service6-negate' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE6_NEGATE:
            return (False, f"Invalid internet-service6-negate '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE6_NEGATE)}")
    ...
    if 'internet-service6-src-negate' in payload:
        ...
        if value and value not in VALID_BODY_INTERNET_SERVICE6_SRC_NEGATE:
            return (False, f"Invalid internet-service6-src-negate '{value}'. Must be one of: {', '.join(VALID_BODY_INTERNET_SERVICE6_SRC_NEGATE)}")
    ...
    if 'timeout-send-rst' in payload:
        ...
        if value and value not in VALID_BODY_TIMEOUT_SEND_RST:
            return (False, f"Invalid timeout-send-rst '{value}'. Must be one of: {', '.join(VALID_BODY_TIMEOUT_SEND_RST)}")
    ...
    if 'captive-portal-exempt' in payload:
        ...
        if value and value not in VALID_BODY_CAPTIVE_PORTAL_EXEMPT:
            return (False, f"Invalid captive-portal-exempt '{value}'. Must be one of: {', '.join(VALID_BODY_CAPTIVE_PORTAL_EXEMPT)}")
    ...
    if 'dsri' in payload:
        ...
        if value and value not in VALID_BODY_DSRI:
            return (False, f"Invalid dsri '{value}'. Must be one of: {', '.join(VALID_BODY_DSRI)}")
    ...
    if 'radius-mac-auth-bypass' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_MAC_AUTH_BYPASS:
            return (False, f"Invalid radius-mac-auth-bypass '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_MAC_AUTH_BYPASS)}")
    ...
    if 'radius-ip-auth-bypass' in payload:
        ...
        if value and value not in VALID_BODY_RADIUS_IP_AUTH_BYPASS:
            return (False, f"Invalid radius-ip-auth-bypass '{value}'. Must be one of: {', '.join(VALID_BODY_RADIUS_IP_AUTH_BYPASS)}")
    ...
    if 'sgt-check' in payload:
        ...
        if value and value not in VALID_BODY_SGT_CHECK:
            return (False, f"Invalid sgt-check '{value}'. Must be one of: {', '.join(VALID_BODY_SGT_CHECK)}")
...
def validate_policy_delete(policyid: str | None = None) -> tuple[bool, str | None]:
    """
