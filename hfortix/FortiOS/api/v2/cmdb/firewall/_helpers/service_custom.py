...
VALID_BODY_PROTOCOL = ['TCP/UDP/UDP-Lite/SCTP', 'ICMP', 'ICMP6', 'IP', 'HTTP', 'FTP', 'CONNECT', 'SOCKS-TCP', 'SOCKS-UDP', 'ALL']
VALID_BODY_HELPER = ['auto', 'disable', 'ftp', 'tftp', 'ras', 'h323', 'tns', 'mms', 'sip', 'pptp', 'rtsp', 'dns-udp', 'dns-tcp', 'pmap', 'rsh', 'dcerpc', 'mgcp']
...
def validate_service_custom_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'helper' in payload:
        ...
        if value and value not in VALID_BODY_HELPER:
            return (False, f"Invalid helper '{value}'. Must be one of: {', '.join(VALID_BODY_HELPER)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 254:
                    return (False, f"protocol-number must be between 0 and 254")
            except (ValueError, TypeError):
                return (False, f"protocol-number must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"icmptype must be between 0 and 4294967295")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 86400:
                    return (False, f"tcp-halfclose-timer must be between 0 and 86400")
            except (ValueError, TypeError):
    ...
    if 'tcp-halfopen-timer' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 86400:
                    return (False, f"tcp-halfopen-timer must be between 0 and 86400")
            except (ValueError, TypeError):
    ...
    if 'udp-idle-timer' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 86400:
                    return (False, f"udp-idle-timer must be between 0 and 86400")
            except (ValueError, TypeError):
    ...
    if 'app-service-type' in payload:
        ...
        if value and value not in VALID_BODY_APP_SERVICE_TYPE:
            return (False, f"Invalid app-service-type '{value}'. Must be one of: {', '.join(VALID_BODY_APP_SERVICE_TYPE)}")
...
def validate_service_custom_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
