...
def validate_ssl_server_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'add-header-x-forwarded-proto' in payload:
        ...
        if value and value not in VALID_BODY_ADD_HEADER_X_FORWARDED_PROTO:
            return (False, f"Invalid add-header-x-forwarded-proto '{value}'. Must be one of: {', '.join(VALID_BODY_ADD_HEADER_X_FORWARDED_PROTO)}")
    ...
    if 'url-rewrite' in payload:
        ...
        if value and value not in VALID_BODY_URL_REWRITE:
            return (False, f"Invalid url-rewrite '{value}'. Must be one of: {', '.join(VALID_BODY_URL_REWRITE)}")
...
def validate_ssl_server_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
