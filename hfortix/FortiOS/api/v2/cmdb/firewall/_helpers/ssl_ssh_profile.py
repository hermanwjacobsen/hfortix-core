...
def validate_ssl_ssh_profile_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
    ...
    if 'allowlist' in payload:
        ...
        if value and value not in VALID_BODY_ALLOWLIST:
            return (False, f"Invalid allowlist '{value}'. Must be one of: {', '.join(VALID_BODY_ALLOWLIST)}")
    ...
    if 'block-blocklisted-certificates' in payload:
        ...
        if value and value not in VALID_BODY_BLOCK_BLOCKLISTED_CERTIFICATES:
            return (False, f"Invalid block-blocklisted-certificates '{value}'. Must be one of: {', '.join(VALID_BODY_BLOCK_BLOCKLISTED_CERTIFICATES)}")
    ...
    if 'server-cert-mode' in payload:
        ...
        if value and value not in VALID_BODY_SERVER_CERT_MODE:
            return (False, f"Invalid server-cert-mode '{value}'. Must be one of: {', '.join(VALID_BODY_SERVER_CERT_MODE)}")
    ...
    if 'use-ssl-server' in payload:
        ...
        if value and value not in VALID_BODY_USE_SSL_SERVER:
            return (False, f"Invalid use-ssl-server '{value}'. Must be one of: {', '.join(VALID_BODY_USE_SSL_SERVER)}")
    ...
    if 'ssl-exemption-ip-rating' in payload:
        ...
        if value and value not in VALID_BODY_SSL_EXEMPTION_IP_RATING:
            return (False, f"Invalid ssl-exemption-ip-rating '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_EXEMPTION_IP_RATING)}")
    ...
    if 'ssl-exemption-log' in payload:
        ...
        if value and value not in VALID_BODY_SSL_EXEMPTION_LOG:
            return (False, f"Invalid ssl-exemption-log '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_EXEMPTION_LOG)}")
    ...
    if 'ssl-anomaly-log' in payload:
        ...
        if value and value not in VALID_BODY_SSL_ANOMALY_LOG:
            return (False, f"Invalid ssl-anomaly-log '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_ANOMALY_LOG)}")
    ...
    if 'ssl-negotiation-log' in payload:
        ...
        if value and value not in VALID_BODY_SSL_NEGOTIATION_LOG:
            return (False, f"Invalid ssl-negotiation-log '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_NEGOTIATION_LOG)}")
    ...
    if 'ssl-server-cert-log' in payload:
        ...
        if value and value not in VALID_BODY_SSL_SERVER_CERT_LOG:
            return (False, f"Invalid ssl-server-cert-log '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_SERVER_CERT_LOG)}")
    ...
    if 'ssl-handshake-log' in payload:
        ...
        if value and value not in VALID_BODY_SSL_HANDSHAKE_LOG:
            return (False, f"Invalid ssl-handshake-log '{value}'. Must be one of: {', '.join(VALID_BODY_SSL_HANDSHAKE_LOG)}")
    ...
    if 'rpc-over-https' in payload:
        ...
        if value and value not in VALID_BODY_RPC_OVER_HTTPS:
            return (False, f"Invalid rpc-over-https '{value}'. Must be one of: {', '.join(VALID_BODY_RPC_OVER_HTTPS)}")
    ...
    if 'mapi-over-https' in payload:
        ...
        if value and value not in VALID_BODY_MAPI_OVER_HTTPS:
            return (False, f"Invalid mapi-over-https '{value}'. Must be one of: {', '.join(VALID_BODY_MAPI_OVER_HTTPS)}")
    ...
    if 'supported-alpn' in payload:
        ...
        if value and value not in VALID_BODY_SUPPORTED_ALPN:
            return (False, f"Invalid supported-alpn '{value}'. Must be one of: {', '.join(VALID_BODY_SUPPORTED_ALPN)}")
...
def validate_ssl_ssh_profile_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
