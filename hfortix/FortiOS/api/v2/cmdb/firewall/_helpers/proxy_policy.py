...
VALID_BODY_PROXY = ['explicit-web', 'transparent-web', 'ftp', 'ssh', 'ssh-tunnel', 'access-proxy', 'ztna-proxy']
...
def validate_proxy_policy_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"policyid must be between 0 and 4294967295")
    ...
    if 'proxy' in payload:
        ...
        if value and value not in VALID_BODY_PROXY:
            return (False, f"Invalid proxy '{value}'. Must be one of: {', '.join(VALID_BODY_PROXY)}")
    ...
    if 'device-ownership' in payload:
        ...
        if value and value not in VALID_BODY_DEVICE_OWNERSHIP:
            return (False, f"Invalid device-ownership '{value}'. Must be one of: {', '.join(VALID_BODY_DEVICE_OWNERSHIP)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 300 or int_val > 2764800:
                    return (False, f"session-ttl must be between 300 and 2764800")
            except (ValueError, TypeError):
    ...
    if 'http-tunnel-auth' in payload:
        ...
        if value and value not in VALID_BODY_HTTP_TUNNEL_AUTH:
            return (False, f"Invalid http-tunnel-auth '{value}'. Must be one of: {', '.join(VALID_BODY_HTTP_TUNNEL_AUTH)}")
    ...
    if 'transparent' in payload:
        ...
        if value and value not in VALID_BODY_TRANSPARENT:
            return (False, f"Invalid transparent '{value}'. Must be one of: {', '.join(VALID_BODY_TRANSPARENT)}")
    ...
    if 'https-sub-category' in payload:
        ...
        if value and value not in VALID_BODY_HTTPS_SUB_CATEGORY:
            return (False, f"Invalid https-sub-category '{value}'. Must be one of: {', '.join(VALID_BODY_HTTPS_SUB_CATEGORY)}")
    ...
    if 'detect-https-in-http-request' in payload:
        ...
        if value and value not in VALID_BODY_DETECT_HTTPS_IN_HTTP_REQUEST:
            return (False, f"Invalid detect-https-in-http-request '{value}'. Must be one of: {', '.join(VALID_BODY_DETECT_HTTPS_IN_HTTP_REQUEST)}")
...
def validate_proxy_policy_delete(policyid: str | None = None) -> tuple[bool, str | None]:
    """
