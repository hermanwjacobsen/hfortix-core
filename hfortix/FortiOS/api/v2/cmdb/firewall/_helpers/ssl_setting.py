...
def validate_ssl_setting_put(
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 60:
                    return (False, f"proxy-connect-timeout must be between 1 and 60")
            except (ValueError, TypeError):
                return (False, f"proxy-connect-timeout must be numeric, got: {value}")
    ...
    if 'no-matching-cipher-action' in payload:
        ...
        if value and value not in VALID_BODY_NO_MATCHING_CIPHER_ACTION:
            return (False, f"Invalid no-matching-cipher-action '{value}'. Must be one of: {', '.join(VALID_BODY_NO_MATCHING_CIPHER_ACTION)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 24 or int_val > 720:
                    return (False, f"cert-manager-cache-timeout must be between 24 and 720")
            except (ValueError, TypeError):
                return (False, f"cert-manager-cache-timeout must be numeric, got: {value}")
    ...
    if 'resigned-short-lived-certificate' in payload:
        ...
        if value and value not in VALID_BODY_RESIGNED_SHORT_LIVED_CERTIFICATE:
            return (False, f"Invalid resigned-short-lived-certificate '{value}'. Must be one of: {', '.join(VALID_BODY_RESIGNED_SHORT_LIVED_CERTIFICATE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 500:
                    return (False, f"cert-cache-capacity must be between 0 and 500")
            except (ValueError, TypeError):
                return (False, f"cert-cache-capacity must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 120:
                    return (False, f"cert-cache-timeout must be between 1 and 120")
            except (ValueError, TypeError):
                return (False, f"cert-cache-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 1000:
                    return (False, f"session-cache-capacity must be between 0 and 1000")
            except (ValueError, TypeError):
                return (False, f"session-cache-capacity must be numeric, got: {value}")
    ...
    if 'session-cache-timeout' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 1 or int_val > 60:
                    return (False, f"session-cache-timeout must be between 1 and 60")
            except (ValueError, TypeError):
                return (False, f"session-cache-timeout must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 512:
                    return (False, f"kxp-queue-threshold must be between 0 and 512")
            except (ValueError, TypeError):
                return (False, f"kxp-queue-threshold must be numeric, got: {value}")
    ...
    if 'ssl-queue-threshold' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 512:
                    return (False, f"ssl-queue-threshold must be between 0 and 512")
            except (ValueError, TypeError):
                return (False, f"ssl-queue-threshold must be numeric, got: {value}")
    ...
    if 'abbreviate-handshake' in payload:
        ...
        if value and value not in VALID_BODY_ABBREVIATE_HANDSHAKE:
            return (False, f"Invalid abbreviate-handshake '{value}'. Must be one of: {', '.join(VALID_BODY_ABBREVIATE_HANDSHAKE)}")
