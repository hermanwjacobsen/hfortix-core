...
VALID_BODY_SECURITY = ['OPEN', 'WPA2-Personal', 'WPA-WPA2-Personal', 'WPA3-SAE', 'WPA3-SAE-Transition', 'WPA2-Enterprise', 'WPA3-Enterprise-only', 'WPA3-Enterprise-transition', 'WPA3-Enterprise-192-bit']
...
def validate_extender_vap_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    """
        ...
        if value is not None:
            try:
                ...
                if int_val < 256 or int_val > 2347:
                    return (False, f"rts-threshold must be between 256 and 2347")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 65535:
                    return (False, f"auth-server-port must be between 1 and 65535")
            except (ValueError, TypeError):
                return (False, f"auth-server-port must be numeric, got: {value}")
...
def validate_extender_vap_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
