...
def validate_dataplan_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'modem-id' in payload:
        ...
        if value and value not in VALID_BODY_MODEM_ID:
            return (False, f"Invalid modem-id '{value}'. Must be one of: {', '.join(VALID_BODY_MODEM_ID)}")
    ...
    if 'slot' in payload:
        ...
        if value and value not in VALID_BODY_SLOT:
            return (False, f"Invalid slot '{value}'. Must be one of: {', '.join(VALID_BODY_SLOT)}")
    ...
    if 'pdn' in payload:
        ...
        if value and value not in VALID_BODY_PDN:
            return (False, f"Invalid pdn '{value}'. Must be one of: {', '.join(VALID_BODY_PDN)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 50 or int_val > 100:
                    return (False, f"signal-threshold must be between 50 and 100")
            except (ValueError, TypeError):
                return (False, f"signal-threshold must be numeric, got: {value}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 600 or int_val > 18000:
                    return (False, f"signal-period must be between 600 and 18000")
            except (ValueError, TypeError):
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 1000000:
                    return (False, f"monthly-fee must be between 0 and 1000000")
            except (ValueError, TypeError):
    ...
    if 'overage' in payload:
        ...
        if value and value not in VALID_BODY_OVERAGE:
            return (False, f"Invalid overage '{value}'. Must be one of: {', '.join(VALID_BODY_OVERAGE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 0 or int_val > 32:
                    return (False, f"preferred-subnet must be between 0 and 32")
            except (ValueError, TypeError):
                return (False, f"preferred-subnet must be numeric, got: {value}")
    ...
    if 'private-network' in payload:
        ...
        if value and value not in VALID_BODY_PRIVATE_NETWORK:
            return (False, f"Invalid private-network '{value}'. Must be one of: {', '.join(VALID_BODY_PRIVATE_NETWORK)}")
...
def validate_dataplan_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
