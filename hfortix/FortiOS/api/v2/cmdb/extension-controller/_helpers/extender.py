...
def validate_extender_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'authorized' in payload:
        ...
        if value and value not in VALID_BODY_AUTHORIZED:
            return (False, f"Invalid authorized '{value}'. Must be one of: {', '.join(VALID_BODY_AUTHORIZED)}")
    ...
    if 'device-id' in payload:
        ...
        if value is not None:
            try:
                int_val = int(value)
                if int_val < 0 or int_val > 4294967295:
                    return (False, f"device-id must be between 0 and 4294967295")
            except (ValueError, TypeError):
    ...
    if 'extension-type' in payload:
        ...
        if value and value not in VALID_BODY_EXTENSION_TYPE:
            return (False, f"Invalid extension-type '{value}'. Must be one of: {', '.join(VALID_BODY_EXTENSION_TYPE)}")
    ...
    if 'override-login-password-change' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE_LOGIN_PASSWORD_CHANGE:
            return (False, f"Invalid override-login-password-change '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_LOGIN_PASSWORD_CHANGE)}")
    ...
    if 'login-password-change' in payload:
        ...
        if value and value not in VALID_BODY_LOGIN_PASSWORD_CHANGE:
            return (False, f"Invalid login-password-change '{value}'. Must be one of: {', '.join(VALID_BODY_LOGIN_PASSWORD_CHANGE)}")
    ...
    if 'override-enforce-bandwidth' in payload:
        ...
        if value and value not in VALID_BODY_OVERRIDE_ENFORCE_BANDWIDTH:
            return (False, f"Invalid override-enforce-bandwidth '{value}'. Must be one of: {', '.join(VALID_BODY_OVERRIDE_ENFORCE_BANDWIDTH)}")
    ...
    if 'enforce-bandwidth' in payload:
        ...
        if value and value not in VALID_BODY_ENFORCE_BANDWIDTH:
            return (False, f"Invalid enforce-bandwidth '{value}'. Must be one of: {', '.join(VALID_BODY_ENFORCE_BANDWIDTH)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 1 or int_val > 16776000:
                    return (False, f"bandwidth-limit must be between 1 and 16776000")
            except (ValueError, TypeError):
                return (False, f"bandwidth-limit must be numeric, got: {value}")
...
def validate_extender_delete(name: str | None = None) -> tuple[bool, str | None]:
    """
