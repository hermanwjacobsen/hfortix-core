...
VALID_BODY_REPORT_SOURCE = ['forward-traffic', 'sniffer-traffic', 'local-deny-traffic']
...
def validate_setting_put(
    ...
    if 'pdf-report' in payload:
        ...
        if value and value not in VALID_BODY_PDF_REPORT:
            return (False, f"Invalid pdf-report '{value}'. Must be one of: {', '.join(VALID_BODY_PDF_REPORT)}")
    ...
    if 'fortiview' in payload:
        ...
        if value and value not in VALID_BODY_FORTIVIEW:
            return (False, f"Invalid fortiview '{value}'. Must be one of: {', '.join(VALID_BODY_FORTIVIEW)}")
    ...
    if 'report-source' in payload:
        ...
        if value and value not in VALID_BODY_REPORT_SOURCE:
            return (False, f"Invalid report-source '{value}'. Must be one of: {', '.join(VALID_BODY_REPORT_SOURCE)}")
        ...
        if value is not None:
            try:
                ...
                if int_val < 3 or int_val > 15:
                    return (False, f"web-browsing-threshold must be between 3 and 15")
            except (ValueError, TypeError):
                return (False, f"web-browsing-threshold must be numeric, got: {value}")
