...
VALID_BODY_OPTIONS = ['include-table-of-content', 'auto-numbering-heading', 'view-chart-as-heading', 'show-html-navbar-before-heading', 'dummy-option']
VALID_BODY_FORMAT = ['pdf']
VALID_BODY_SCHEDULE_TYPE = ['demand', 'daily', 'weekly']
VALID_BODY_DAY = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']
...
def validate_layout_post(payload: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if 'schedule-type' in payload:
        ...
        if value and value not in VALID_BODY_SCHEDULE_TYPE:
            return (False, f"Invalid schedule-type '{value}'. Must be one of: {', '.join(VALID_BODY_SCHEDULE_TYPE)}")
    ...
    if 'cutoff-option' in payload:
        ...
        if value and value not in VALID_BODY_CUTOFF_OPTION:
            return (False, f"Invalid cutoff-option '{value}'. Must be one of: {', '.join(VALID_BODY_CUTOFF_OPTION)}")
    ...
    if 'email-send' in payload:
        ...
        if value and value not in VALID_BODY_EMAIL_SEND:
            return (False, f"Invalid email-send '{value}'. Must be one of: {', '.join(VALID_BODY_EMAIL_SEND)}")
