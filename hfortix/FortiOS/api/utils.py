...
class Utils:
    ...
    def performance_test(
        ...
        if test_concurrency:
            ...
            print(
                "  ⚠ Note: Most FortiGates serialize requests - concurrency may not help"
            )
            try:
                ...
                print(
                    f"  Throughput: {results.concurrent_throughput:.2f} req/s"
                )
    ...
    def _test_concurrent_performance(
        ...
        print(
            "  ⚠ Skipping concurrent test - not yet implemented for reusing auth"
        )
