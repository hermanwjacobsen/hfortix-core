...
Monitor endpoints currently lack `response_fields` in their schema definitions:
...
result.switch_id         # ❓ No autocomplete, no type checking
result.snooping_entries  # ❓ Unknown field structure
```

### Desired Situation

With proper `response_fields` defined:
...
```python
# Enhanced developer experience with schema
result = fgt.api.monitor.switch_controller.managed_switch.dhcp_snooping.get()

# Type shows as DhcpSnoopingObject with full field definitions
result.switch_id         # ✅ Autocomplete: str
result.snooping_entries  # ✅ Autocomplete: list[DhcpSnoopingSnoopingEntriesItem]

for entry in result.snooping_entries:
    entry.mac   # ✅ Autocomplete: str
    entry.ip    # ✅ Autocomplete: str
    entry.vlan  # ✅ Autocomplete: str
    entry.port  # ✅ Autocomplete: str
```
...
1. `switch-controller/managed-switch/dhcp-snooping` - DHCP server detection
...
Response field definitions can be extracted from:

1. **API Responses**: Capture actual response JSON and infer types
2. **Swagger Documentation**: Use existing Swagger/OpenAPI definitions
