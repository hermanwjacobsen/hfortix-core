...
class SettingModel(BaseModel):
    ...
    # Model Fields
    # ========================================================================
    ocsp_status: Literal["enable", "mandatory", "disable"] | None = Field(default="disable", description="Enable/disable receiving certificates using the OCSP.")    ocsp_option: Literal["certificate", "server"] | None = Field(default="server", description="Specify whether the OCSP URL is from certificate or configured OCSP server.")    proxy: str | None = Field(max_length=127, default="", description="Proxy server FQDN or IP for OCSP/CA queries during certificate verification.")    proxy_port: int | None = Field(ge=1, le=65535, default=8080, description="Proxy server port (1 - 65535, default = 8080).")    proxy_username: str | None = Field(max_length=63, default="", description="Proxy server user name.")    proxy_password: Any = Field(max_length=128, default=None, description="Proxy server password.")    source_ip: str | None = Field(max_length=63, default="", description="Source IP address for dynamic AIA and OCSP queries.")    ocsp_default_server: str | None = Field(max_length=35, default="", description="Default OCSP server.")  # datasource: ['vpn.certificate.ocsp-server.name']    interface_select_method: Literal["auto", "sdwan", "specify"] | None = Field(default="auto", description="Specify how to select outgoing interface to reach server.")    interface: str = Field(max_length=15, default="", description="Specify outgoing interface to reach server.")  # datasource: ['system.interface.name']    vrf_select: int | None = Field(ge=0, le=511, default=0, description="VRF ID used for connection to server.")    check_ca_cert: Literal["enable", "disable"] | None = Field(default="enable", description="Enable/disable verification of the user certificate and pass authentication if any CA in the chain is trusted (default = enable).")    check_ca_chain: Literal["enable", "disable"] | None = Field(default="disable", description="Enable/disable verification of the entire certificate chain and pass authentication only if the chain is complete and all of the CAs in the chain are trusted (default = disable).")    subject_match: Literal["substring", "value"] | None = Field(default="substring", description="When searching for a matching certificate, control how to do RDN value matching with certificate subject name (default = substring).")    subject_set: Literal["subset", "superset"] | None = Field(default="subset", description="When searching for a matching certificate, control how to do RDN set matching with certificate subject name (default = subset).")    cn_match: Literal["substring", "value"] | None = Field(default="substring", description="When searching for a matching certificate, control how to do CN value matching with certificate subject name (default = substring).")    cn_allow_multi: Literal["disable", "enable"] | None = Field(default="enable", description="When searching for a matching certificate, allow multiple CN fields in certificate subject name (default = enable).")    crl_verification: list[SettingCrlVerification] = Field(default=None, description="CRL verification options.")    strict_ocsp_check: Literal["enable", "disable"] | None = Field(default="disable", description="Enable/disable strict mode OCSP checking.")    ssl_min_proto_version: SettingSslMinProtoVersionEnum | None = Field(default="default", description="Minimum supported protocol version for SSL/TLS connections (default is to follow system global setting).")    cmp_save_extra_certs: Literal["enable", "disable"] | None = Field(default="disable", description="Enable/disable saving extra certificates in CMP mode (default = disable).")    cmp_key_usage_checking: Literal["enable", "disable"] | None = Field(default="enable", description="Enable/disable server certificate key usage checking in CMP mode (default = enable).")    cert_expire_warning: int | None = Field(ge=0, le=100, default=14, description="Number of days before a certificate expires to send a warning. Set to 0 to disable sending of the warning (0 - 100, default = 14).")    certname_rsa1024: str = Field(max_length=35, default="Fortinet_SSL_RSA1024", description="1024 bit RSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_rsa2048: str = Field(max_length=35, default="Fortinet_SSL_RSA2048", description="2048 bit RSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_rsa4096: str = Field(max_length=35, default="Fortinet_SSL_RSA4096", description="4096 bit RSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_dsa1024: str = Field(max_length=35, default="Fortinet_SSL_DSA1024", description="1024 bit DSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_dsa2048: str = Field(max_length=35, default="Fortinet_SSL_DSA2048", description="2048 bit DSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_ecdsa256: str = Field(max_length=35, default="Fortinet_SSL_ECDSA256", description="256 bit ECDSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_ecdsa384: str = Field(max_length=35, default="Fortinet_SSL_ECDSA384", description="384 bit ECDSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_ecdsa521: str = Field(max_length=35, default="Fortinet_SSL_ECDSA521", description="521 bit ECDSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_ed25519: str = Field(max_length=35, default="Fortinet_SSL_ED25519", description="253 bit EdDSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    certname_ed448: str = Field(max_length=35, default="Fortinet_SSL_ED448", description="456 bit EdDSA key certificate for re-signing server certificates for SSL inspection.")  # datasource: ['vpn.certificate.local.name']    # ========================================================================
    # Custom Validators
    # ========================================================================
    ...
    async def validate_all_references(self, client: Any) -> list[str]:
        ...
        all_errors.extend(errors)        errors = await self.validate_certname_rsa1024_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_rsa2048_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_rsa4096_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_dsa1024_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_dsa2048_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_ecdsa256_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_ecdsa384_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_ecdsa521_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_ed25519_references(client)
        all_errors.extend(errors)        errors = await self.validate_certname_ed448_references(client)
...
# Schema: 1.7.0
# Generated: 2026-01-14T15:56:36.135483Z
# ============================================================================
