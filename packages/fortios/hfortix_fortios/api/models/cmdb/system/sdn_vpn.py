...
class SdnVpnModel(BaseModel):
    ...
    # Model Fields
    # ========================================================================
    name: str | None = Field(max_length=35, default="", description="Public cloud VPN name.")    sdn: str = Field(max_length=35, default="", description="SDN connector name.")  # datasource: ['system.sdn-connector.name']    remote_type: Literal["vgw", "tgw"] = Field(default="vgw", description="Type of remote device.")    routing_type: Literal["static", "dynamic"] = Field(default="dynamic", description="Type of routing.")    vgw_id: str = Field(max_length=63, default="", description="Virtual private gateway id.")    tgw_id: str = Field(max_length=63, default="", description="Transit gateway id.")    subnet_id: str | None = Field(max_length=63, default="", description="AWS subnet id for TGW route propagation.")    bgp_as: int = Field(ge=1, le=4294967295, default=65000, description="BGP Router AS number.")    cgw_gateway: str = Field(default="0.0.0.0", description="Public IP address of the customer gateway.")    nat_traversal: Literal["disable", "enable"] | None = Field(default="enable", description="Enable/disable use for NAT traversal. Please enable if your FortiGate device is behind a NAT/PAT device.")    tunnel_interface: str = Field(max_length=15, default="", description="Tunnel interface with public IP.")  # datasource: ['system.interface.name']    internal_interface: str = Field(max_length=15, default="", description="Internal interface with local subnet.")  # datasource: ['system.interface.name']    local_cidr: str = Field(default="0.0.0.0 0.0.0.0", description="Local subnet address and subnet mask.")    remote_cidr: str = Field(default="0.0.0.0 0.0.0.0", description="Remote subnet address and subnet mask.")    cgw_name: str | None = Field(max_length=35, default="", description="AWS customer gateway name to be created.")    psksecret: Any = Field(default=None, description="Pre-shared secret for PSK authentication. Auto-generated if not specified")    type: int | None = Field(ge=0, le=65535, default=0, description="SDN VPN type.")    status: int | None = Field(ge=0, le=255, default=0, description="SDN VPN status.")    code: int | None = Field(ge=0, le=255, default=0, description="SDN VPN error code.")    # ========================================================================
    # Custom Validators
    # ========================================================================
    ...
    async def validate_all_references(self, client: Any) -> list[str]:
        ...
        all_errors.extend(errors)        errors = await self.validate_tunnel_interface_references(client)
        all_errors.extend(errors)        errors = await self.validate_internal_interface_references(client)
...
# Schema: 1.7.0
# Generated: 2026-01-14T15:56:37.324870Z
# ============================================================================
