...
class DebugSession:
    ...
    Captures detailed information about all API requests made within the session,
    including timing, connection stats, and request/response details.
    
    Args:
        client: FortiOS client instance to debug
        capture_response_data: Whether to capture full response bodies (default: False)
        print_on_exit: Whether to print summary when exiting context (default: True)
    ...
    def stats_delta(self) -> dict[str, Any] | None:
        ...
        return {
            "requests_made": (
                self._final_stats["total_requests"] - self._initial_stats["total_requests"]
            ),
            "pool_exhaustions": (
    ...
    def print_summary(self, format: Literal["text", "json"] = "text") -> None:
        ...
        if summary["stats_delta"]:
            ...
            print(f"  Requests Made: {summary['stats_delta']['requests_made']}")
            print(f"  Pool Exhaustions: {summary['stats_delta']['pool_exhaustions']}")
            print(f"  Active Requests Change: {summary['stats_delta']['active_requests_delta']}")
            
        if summary["final_stats"]:
            print("\nFinal Connection Stats:")
            print(f"  Active Requests: {summary['final_stats']['active_requests']}")
            print(f"  Total Requests: {summary['final_stats']['total_requests']}")
            print(f"  Pool Exhaustion Count: {summary['final_stats']['pool_exhaustion_count']}")
        ...
        for i, req in enumerate(self._requests, 1):
            ...
            if verbose:
                ...
                if req.get("response_data"):
                    print(f"   Response: {json.dumps(req['response_data'], indent=6)}")
